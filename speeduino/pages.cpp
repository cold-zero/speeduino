...
constexpr const uint16_t PROGMEM ini_page_sizes[] = {0,   128, 288, 288, 128, 288, 128, 240,
                                                     384, 192, 192, 288, 192, 128, 288, 256};
...
class offset_to_table
{
   ...
   private:
    inline byte &get_value_value(void) const { return _pTable->values.value_at((uint8_t)_table_offset); }
    ...
    enum table_location
    {
        ...
        table_location_xaxis,
        table_location_yaxis
    };
...
#define CTA_GET_TABLE_VALUE(size, xDomain, yDomain, pTable, offset)         \
    return *offset_to_table<TABLE3D_TYPENAME_BASE(size, xDomain, yDomain)>( \
        (TABLE3D_TYPENAME_BASE(size, xDomain, yDomain) *)pTable, offset);
#define CTA_GET_TABLE_VALUE_DEFAULT ({ return 0U; })
    CONCRETE_TABLE_ACTION(entity.table_key, CTA_GET_TABLE_VALUE, CTA_GET_TABLE_VALUE_DEFAULT, entity.pData,
                          (offset - entity.start));
}
...
#define CTA_SET_TABLE_VALUE(size, xDomain, yDomain, pTable, offset, new_value)        \
    offset_to_table<TABLE3D_TYPENAME_BASE(size, xDomain, yDomain)>(                   \
        (TABLE3D_TYPENAME_BASE(size, xDomain, yDomain) *)pTable, offset) = new_value; \
    break;
#define CTA_SET_TABLE_VALUE_DEFAULT ({})
    CONCRETE_TABLE_ACTION(entity.table_key, CTA_SET_TABLE_VALUE, CTA_SET_TABLE_VALUE_DEFAULT, entity.pData,
                          (offset - entity.start), new_value);
}
...
inline const page_iterator_t create_table_iterator(void *pTable, table_type_t key, uint8_t pageNum, uint16_t start,
                                                   uint16_t size)
{
    return page_iterator_t{
        .pData = pTable,
...
#define CHECK_TABLE(pageNum, offset, pTable, entityNum)                                                    \
    if (offset < ENTITY_START_VAR(entityNum) + get_table_axisy_end(pTable))                                \
        {                                                                                                  \
            return create_table_iterator(pTable, (pTable)->type_key, pageNum, ENTITY_START_VAR(entityNum), \
                                         get_table_axisy_end(pTable));                                     \
        }                                                                                                  \
