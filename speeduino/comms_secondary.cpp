...
void secondserial_Command(void)
{
    ...
    if (serialSecondaryStatusFlag == SERIAL_INACTIVE)
        {
            currentSecondaryCommand = secondarySerial.read();
        }
    ...
    switch (currentSecondaryCommand)
        {
            case 'A':
                ...
                if (configPage9.secondarySerialProtocol == SECONDARY_SERIAL_PROTO_GENERIC_FIXED)
                    {
                        sendValues(0, CAN_PACKET_SIZE, 0x31, secondarySerial, serialSecondaryStatusFlag,
                                   &getLegacySecondarySerialLogEntry);
                    }  // Send values using the legacy fixed byte order
                else
                    {
                        sendValues(0, CAN_PACKET_SIZE, 0x31, secondarySerial, serialSecondaryStatusFlag);
                    }  // send values to serial3 using the order in the ini file
                break;
                ...
                if (secondarySerial.available() >= 9)
                    {
                        ...
                        if (cmdSuccessful != 0)
                            {  // read all 8 bytes of data.
                                ...
                                for (byte Gx = 0; Gx < 8;
                                     Gx++)  // first two are the can address the data is from. next two are the can
                                            // address the data is for.then next 1 or two bytes of data
                                    {
                                ...
                                if ((BIT_CHECK(configPage9.caninput_source_num_bytes, destcaninchannel) >
                                     0))  // if true then num bytes is 2
                                    {
                                        if ((configPage9.caninput_source_start_byte[destcaninchannel] & 7) <
                                            8)  // you can't have a 2 byte value starting at byte 7(8 on the list)
                                            {
                                                Ghigh = Gdata[(
                                                    (configPage9.caninput_source_start_byte[destcaninchannel] & 7) +
                                                    1)];
                                            }
                                        else
                                            {
            ...
            case 'L':
                {
                    ...
                    /*
                    Unclear what the below is trying to achieve. Commenting out for now to avoid compiler warnings for
                    unused variables Llength = secondarySerial.read();             // next the number of bytes expected
                    value uint8_t Lbuffer[8];                     //8 byte buffer to store incoming can data
            ...
            case 'n':  // sends the bytes of realtime values from the NEW CAN list
                // sendValues(0, NEW_CAN_PACKET_SIZE, 0x32, secondarySerial, serialSecondaryStatusFlag); //send values
                // to serial3
                if (configPage9.secondarySerialProtocol == SECONDARY_SERIAL_PROTO_GENERIC_FIXED)
                    {
                        sendValues(0, NEW_CAN_PACKET_SIZE, 0x32, secondarySerial, serialSecondaryStatusFlag,
                                   &getLegacySecondarySerialLogEntry);
                    }  // Send values using the legacy fixed byte order
                else
                    {
            ...
            case 'S':  // send code version
                if (configPage9.secondarySerialProtocol == SECONDARY_SERIAL_PROTO_MSDROID)
                    {
                        legacySerialHandler('Q', secondarySerial, serialSecondaryStatusFlag);
                    }  // Note 'Q', this is a workaround for msDroid
                else
                    {
                        legacySerialHandler(currentSecondaryCommand, secondarySerial, serialSecondaryStatusFlag);
                    }
...
// this routine sends a request(either "0" for a "G" , "1" for a "L" , "2" for a "R" to the Can interface or "3" sends
// the request via the actual local canbus
    ...
    switch (cmdtype)
        {
            ...
            case 2:  // requests via serial3
                secondarySerial.print(
                    "R");  // send "R" to request data from the sourcecanAddress whose value is sent next
