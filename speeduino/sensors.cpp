...
unsigned long MAPrunningValue;  // Used for tracking either the total of all MAP readings in this cycle (Event average)
                                // or the lowest value detected in this cycle (event minimum)
...
// These variables are used for tracking the number of running sensors values that appear to be errors. Once a threshold
// is reached, the sensor reading will go to default value and assume the sensor is faulty
...
ISR(ADC_vect)
{
    ...
    if (nChannel == 15)
        {
            ...
            BIT_CLEAR(ADCSRA, ADIE);  // Disable interrupt as we're at the end of a full ADC cycle. This will be
                                      // re-enabled in the main loop
        }
    ...
    // ADMUX always appears to be one ahead of the actual channel value that is in ADCL/ADCH. Subtract 1 from it to get
...
void initialiseADC(void)
{
    ...
    defined(__AVR_ATmega2561__)  // AVR chips use the ISR for this
...
#ifndef ADFR
#define ADFR \
    5  // Looks like this is now defined. Retain this for compatibility with earlier versions of Arduino IDE that did
       // not have this.
#endif
...
#else
    // This sets the ADC (Analog to Digital Converter) to run at 1Mhz, greatly reducing analog read times (MAP/TPS) when
    // using the standard analogRead() function 1Mhz is the fastest speed permitted by the CPU without affecting
    // accuracy Please see chapter 11 of 'Practical Arduino'
    // (books.google.com.au/books?id=HsTxON1L6D4C&printsec=frontcover#v=onepage&q&f=false) for more detail
    BIT_SET(ADCSRA, ADPS2);
    ...
    for (byte AuxinChan = 0; AuxinChan < 16; AuxinChan++)
        {
            ...
            if (((configPage9.caninput_sel[currentStatus.current_caninchannel] & 12) == 4) &&
                ...
                {  // if current input channel is enabled as external input in caninput_selxb(bits 2:3) and secondary
                   // serial or internal canbus is enabled(and is mcu supported)
                    ...
                    auxIsEnabled = true;
                     ...
                     (((configPage9.enable_secondarySerial == 0) && (configPage9.enable_intcan == 0)) &&
                      ((configPage9.caninput_sel[currentStatus.current_caninchannel] & 3) == 2)))
                {  // if current input channel is enabled as analog local pin check caninput_selxb(bits 2:3) with &12
                   // and caninput_selxa(bits 0:1) with &3
                    ...
                    if (pinIsUsed(pinNumber))
                        {
                            ...
                            BIT_SET(currentStatus.engineProtectStatus,
                                    PROTECT_IO_ERROR);  // Tell user that there is problem by lighting up the I/O error
                                                        // indicator
                        }
                    else
                        {
                     ...
                     (((configPage9.enable_secondarySerial == 0) && (configPage9.enable_intcan == 0)) &&
                      ((configPage9.caninput_sel[currentStatus.current_caninchannel] & 3) == 3)))
                {  // if current input channel is enabled as digital local pin check caninput_selxb(bits 2:3) with &12
                   // and caninput_selxa(bits 0:1) with &3
    ...
    // Sanity checks to ensure none of the filter values are set above 240 (Which would include the 255 value which is
    // the default on a new arduino) If an invalid value is detected, it's reset to the default the value and burned to
    // EEPROM. Each sensor has it's own default value
...
// Instantaneous MAP readings
    ...
    if (currentStatus.initialisationComplete == true)
        {
    ...
    currentStatus.MAP =
        fastMap10Bit(currentStatus.mapADC, configPage2.mapMin, configPage2.mapMax);  // Get the current MAP value
    ...
    switch (configPage2.mapSample)
        {
            ...
            case 1:
                ...
                if ((currentStatus.RPMdiv100 > configPage2.mapSwitchPoint) &&
                    ...
                    (currentStatus.startRevolutions >
                     1))  // If the engine isn't running and RPM below switch point, fall back to instantaneous reads
                    {
                        if ((MAPcurRev == currentStatus.startRevolutions) ||
                            ((MAPcurRev + 1) ==
                             currentStatus.startRevolutions))  // 2 revolutions are looked at for 4 stroke. 2 stroke not
                                                               // currently catered for.
                            {
                                ...
                                if ((tempReading < VALID_MAP_MAX) && (tempReading > VALID_MAP_MIN))
                                    {
                                        currentStatus.mapADC =
                                            ADC_FILTER(tempReading, configPage4.ADCFILTER_MAP, currentStatus.mapADC);
                                        MAPrunningValue +=
                                            currentStatus.mapADC;  // Add the current reading onto the total
                                        MAPcount++;
                                    }
                                else
                                    {
                                ...
                                if (configPage6.useEMAP == true)
                                    {
                                        ...
                                        if ((tempReading < VALID_MAP_MAX) && (tempReading > VALID_MAP_MIN))
                                            {
                                                currentStatus.EMAPADC = ADC_FILTER(
                                                    tempReading, configPage4.ADCFILTER_MAP, currentStatus.EMAPADC);
                                                EMAPrunningValue +=
                                                    currentStatus.EMAPADC;  // Add the current reading onto the total
                                            }
                                        else
                                            {
                                                mapErrorCount += 1;
                            ...
                            {
                                // Reaching here means that the last cycle has completed and the MAP value should be
                                // calculated Sanity check
                                if ((MAPrunningValue != 0) && (MAPcount != 0))
                                    {
                                        // Update the calculation times and last value. These are used by the MAP based
                                        // Accel enrich
                                        MAPlast = currentStatus.MAP;
                                        MAPlast_time = MAP_time;
                                        MAP_time = micros();
                                        ...
                                        currentStatus.MAP =
                                            fastMap10Bit(currentStatus.mapADC, configPage2.mapMin,
                                                         configPage2.mapMax);  // Get the current MAP value
                                        validateMAP();

                                        // If EMAP is enabled, the process is identical to the above
                                        if (configPage6.useEMAP == true)
                                            {
                                                currentStatus.EMAPADC =
                                                    udiv_32_16(EMAPrunningValue,
                                                               MAPcount);  // Note that the MAP count can be reused here
                                                                           // as it will always be the same count.
                                                currentStatus.EMAP = fastMap10Bit(
                                                    currentStatus.EMAPADC, configPage2.EMAPMin, configPage2.EMAPMax);
                                                if (currentStatus.EMAP < 0)
                                                    {
                                                        currentStatus.EMAP = 0;
                                                    }  // Sanity check
                    ...
                    {
                        instanteneousMAPReading();
                        MAPrunningValue = currentStatus.mapADC;  // Keep updating the MAPrunningValue to give it head
                                                                 // start when switching to cycle average.
                        if (configPage6.useEMAP == true)
                            {
            ...
            case 2:
                ...
                if (currentStatus.RPMdiv100 >
                    configPage2.mapSwitchPoint)  // If the engine isn't running and RPM below switch point, fall back to
                                                 // instantaneous reads
                    {
                        if ((MAPcurRev == currentStatus.startRevolutions) ||
                            ((MAPcurRev + 1) ==
                        ...
                        else
                            {
                                ...
                                // Update the calculation times and last value. These are used by the MAP based Accel
                                // enrich
                                MAPlast = currentStatus.MAP;
                                MAPlast_time = MAP_time;
                                MAP_time = micros();
                                ...
                                currentStatus.MAP = fastMap10Bit(currentStatus.mapADC, configPage2.mapMin,
                                                                 configPage2.mapMax);  // Get the current MAP value
                                MAPcurRev = currentStatus.startRevolutions;            // Reset the current rev count
                                MAPrunningValue =
                                    1023;  // Reset the latest value so the next reading will always be lower
                ...
                else
                    {
                        instanteneousMAPReading();
                        MAPrunningValue = currentStatus.mapADC;  // Keep updating the MAPrunningValue to give it head
                                                                 // start when switching to cycle minimum.
                    }
                break;
            ...
            case 3:
                ...
                if ((currentStatus.RPMdiv100 > configPage2.mapSwitchPoint) &&
                    ...
                    (!currentStatus
                          .engineProtectStatus))  // If the engine isn't running, fall back to instantaneous reads
                    {
                        if ((MAPcurRev == ignitionCount))  // Watch for a change in the ignition counter to determine
                                                           // whether we're still on the same event
                            {
#if defined(ANALOG_ISR_MAP)
                                tempReading = AnChannel[pinMAP - A0];
#else
                            ...
                            {
                                // Reaching here means that the  next ignition event has occurred and the MAP value
                                // should be calculated Sanity check
                ...
                else
                    {
                        instanteneousMAPReading();
                        MAPrunningValue = currentStatus.mapADC;  // Keep updating the MAPrunningValue to give it head
                                                                 // start when switching to ignition event average.
                        MAPcount = 1;
                    }
                break;
...
void readTPS(bool useFilter)
{
...
#if defined(ANALOG_ISR)
    byte tempTPS =
        fastMap1023toX(AnChannel[pinTPS - A0], 255);  // Get the current raw TPS ADC value and map it into a byte
#else
    analogRead(pinTPS);
    byte tempTPS = fastMap1023toX(analogRead(pinTPS), 255);  // Get the current raw TPS ADC value and map it into a byte
#endif
    // The use of the filter can be overridden if required. This is used on startup to disable priming pulse if flood
    // clear is wanted
    if (useFilter == true)
        {
    ...
    byte tempADC = currentStatus.tpsADC;  // The tempADC value is used in order to allow TunerStudio to recover and redo
                                          // the TPS calibration if this somehow gets corrupted

    if (configPage2.tpsMax > configPage2.tpsMin)
        {
            // Check that the ADC values fall within the min and max ranges (Should always be the case, but noise can
            // cause these to fluctuate outside the defined range).
            ...
            currentStatus.TPS =
                map(tempADC, configPage2.tpsMin, configPage2.tpsMax, 0,
                    200);  // Take the raw TPS ADC value and convert it into a TPS% based on the calibrated values
        }
    else
        {
            // This case occurs when the TPS +5v and gnd are wired backwards, but the user wishes to retain this
            // configuration. In such a case, tpsMin will be greater then tpsMax and hence checks and mapping needs to
            // be reversed
...
void readCLT(bool useFilter)
{
...
#endif
    // The use of the filter can be overridden if required. This is used on startup so there can be an immediately
    // accurate coolant value for priming
    if (useFilter == true)
        {
    ...
    currentStatus.coolant =
        ...
        CALIBRATION_TEMPERATURE_OFFSET;  // Temperature calibration values are stored as positive bytes. We subtract 40
...
// readings
            ...
            currentStatus.baro = fastMap10Bit(currentStatus.baroADC, configPage2.baroMin,
                                              configPage2.baroMax);  // Get the current MAP value
        }
    else
        {
            /*
             * If no dedicated baro sensor is available, attempt to get a reading from the MAP sensor. This can only be
             * done if the engine is not running.
            ...
            if ((lastBaro >= BARO_MIN) &&
                (lastBaro <= BARO_MAX))  // Make sure it's not invalid (Possible on first run etc)
                {
            ...
            // Verify the engine isn't running by confirming RPM is 0 and it has been at least 1 second since the last
            // tooth was detected
            ...
            if ((currentStatus.RPM == 0) && (timeToLastTooth > MICROS_PER_SEC))
                {
                    ...
                    /*
                     * The highest sea-level pressure on Earth occurs in Siberia, where the Siberian High often attains
                     * a sea-level pressure above 105 kPa; with record highs close to 108.5 kPa. The lowest possible
                     * baro reading is based on an altitude of 3500m above sea level.
                     */
                    if ((currentStatus.MAP >= BARO_MIN) &&
                        (currentStatus.MAP <=
                         BARO_MAX))  // Safety check to ensure the baro reading is within the physical limits
                        {
...
void readO2(void)
{
    // An O2 read is only performed if an O2 sensor type is selected. This is to prevent potentially dangerous use of
    // the O2 readings prior to proper setup/calibration
...
void readBat(void)
{
...
#if defined(ANALOG_ISR)
    tempReading =
        fastMap1023toX(AnChannel[pinBat - A0],
                       245);  // Get the current raw Battery value. Permissible values are from 0v to 24.5v (245)
#else
    tempReading = analogRead(pinBat);
    tempReading = fastMap1023toX(
        analogRead(pinBat), 245);  // Get the current raw Battery value. Permissible values are from 0v to 24.5v (245)
#endif
    ...
    // If this occurs, it's very likely that the system has gone from being powered by USB to being powered from the 12v
    // power source. Should that happen, we re-trigger the fuel pump priming and idle homing (If using a stepper)
...
uint16_t getSpeed(void)
{
    ...
    if (configPage2.vssMode == 1)
        {
            ...
            tempSpeed =
                ADC_FILTER(tempSpeed, configPage2.vssSmoothing, currentStatus.vss);  // Apply speed smoothing factor
        }
    // Interrupt driven mode
    else if (configPage2.vssMode > 1)
        {
            ...
            // Add up the time between the teeth. Note that the total number of gaps is equal to the number of samples
            // minus 1
            ...
            if ((micros() - vssTimes[vssIndex]) > MICROS_PER_SEC)
                {
                    tempSpeed = 0;
                }  // Check that the car hasn't come to a stop. Is true if last pulse was more than 1 second ago
            else
                {
                    tempSpeed =
                        MICROS_PER_HOUR / (pulseTime * configPage2.vssPulsesPerKm);  // Convert the pulse gap into km/h
    ...
    if (currentStatus.vss > 0)
        {
            ...
            uint16_t pulsesPer1000rpm =
                udiv_32_16(currentStatus.vss * 10000UL,
                           currentStatus.RPM);  // Gives the current pulses per 1000RPM, multiplied by 10 (10x is the
                                                // multiplication factor for the ratios in TS)
...
// Perform ADC read
            ...
            tempFuelPressure = ADC_FILTER(tempFuelPressure, ADCFILTER_PSI_DEFAULT,
...
// Perform ADC read
            ...
            tempOilPressure = ADC_FILTER(tempOilPressure, ADCFILTER_PSI_DEFAULT,
