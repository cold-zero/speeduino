...
// These are the page numbers that the Tuner Studio serial protocol uses to transverse the different map and config
// pages.
#define veMapPage 2
#define veSetPage \
    1  // Note that this and the veMapPage were swapped in Feb 2019 as the 'algorithm' field must be declared in the ini
       // before it's used in the fuel table
#define ignMapPage 3
...
byte getPageValue(byte pageNum,   /**< [in] The page number to retrieve data from. */
                  uint16_t offset /**< [in] The address in the page that should be returned. This is as per the page
                                     definition in the ini. */
);
