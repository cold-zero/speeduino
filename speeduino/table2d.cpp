...
ie: Given a value on the X axis, it returns a Y value that corresponds to the point on the curve between the nearest two
defined X values
...
int table2D_getValue(struct table2D *fromTable, int X_in)
{
    ...
    // Finally if none of that is found
        ...
        {
            fromTable->cacheTime = getCacheTime();  // As we're not using the cache value, set the current secl value to
                                                    // track when this new value was calculated
            ...
            if ((X <= xMaxValue) && (X > xMinValue))
            ...
            else
                {
                    ...
                    xMaxValue = table2D_getAxisValue(fromTable,
                                                     fromTable->xSize - 1);  // init xMaxValue in preparation for loop.
                    for (int x = fromTable->xSize - 1; x > 0; x--)
                        {
                            ...
                            if (X == xMaxValue)
                                {
                                    returnValue =
                                        table2D_getRawValue(fromTable, x);  // Simply return the corresponding value
                                    valueFound = true;
                                    break;
                                }
                            else if (X > xMinValue)
...
int16_t table2D_getAxisValue(struct table2D *fromTable, byte X_in)
{
    ...
    if (fromTable->axisSize == SIZE_INT)
        {
            returnValue = ((int16_t *)fromTable->axisX)[X_in];
        }
    else if (fromTable->axisSize == SIZE_BYTE)
        {
            returnValue = ((uint8_t *)fromTable->axisX)[X_in];
        }
    else if (fromTable->axisSize == SIZE_SIGNED_BYTE)
        {
            returnValue = ((int8_t *)fromTable->axisX)[X_in];
        }
...
int16_t table2D_getRawValue(struct table2D *fromTable, byte X_index)
{
    ...
    if (fromTable->valueSize == SIZE_INT)
        {
            returnValue = ((int16_t *)fromTable->values)[X_index];
        }
    else if (fromTable->valueSize == SIZE_BYTE)
        {
            returnValue = ((uint8_t *)fromTable->values)[X_index];
        }
    else if (fromTable->valueSize == SIZE_SIGNED_BYTE)
        {
            returnValue = ((int8_t *)fromTable->values)[X_index];
        }
