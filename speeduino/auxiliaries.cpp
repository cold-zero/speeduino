...
// integerPID boostPID(&MAPx100, &boost_pwm_target_value, &boostTargetx100, configPage6.boostKP, configPage6.boostKI,
// configPage6.boostKD, DIRECT);
integerPID_ideal boostPID(&currentStatus.MAP, &currentStatus.boostDuty, &currentStatus.boostTarget,
                          &configPage10.boostSens, &configPage10.boostIntv, configPage6.boostKP, configPage6.boostKI,
                          configPage6.boostKD,
                          DIRECT);  // This is the PID object if that algorithm is used. Needs to be global as it
                                    // maintains state outside of each function call
integerPID vvtPID(&vvt_pid_current_angle, &currentStatus.vvt1Duty, &vvt_pid_target_angle, configPage10.vvtCLKP,
                  configPage10.vvtCLKI, configPage10.vvtCLKD,
                  configPage6.vvtPWMdir);  // This is the PID object if that algorithm is used. Needs to be global as it
                                           // maintains state outside of each function call
integerPID vvt2PID(&vvt2_pid_current_angle, &currentStatus.vvt2Duty, &vvt2_pid_target_angle, configPage10.vvtCLKP,
                   configPage10.vvtCLKI, configPage10.vvtCLKD,
                   configPage4.vvt2PWMdir);  // This is the PID object if that algorithm is used. Needs to be global as
                                             // it maintains state outside of each function call
...
{
    if ((configPage15.airConEnable) == 1 && pinAirConRequest != 0 && pinAirConComp != 0)
        {
    ...
    if (acIsEnabled == true)
        {
            ...
            if (READ_AIRCON_REQUEST() == true && waitedAfterCranking == true &&
                ...
                {
                    // Set the BIT_AIRCON_TURNING_ON bit to notify the idle system to idle up & the cooling fan to start
                    // (if enabled)
...
bool READ_AIRCON_REQUEST(void)
{
    ...
    bool acReqPinStatus = (((configPage15.airConReqPol) == 1) ? !!(*aircon_req_pin_port & aircon_req_pin_mask)
...
static inline void checkAirConCoolantLockout(void)
{
    ...
    if (currentStatus.coolant > offTemp)
    ...
    else if (currentStatus.coolant < (offTemp - 1))
        {
            ...
            // Yes, it is 2 degrees (not 1 degree or 3 degrees) because we go "> offTemp" to enable and "< (offtemp-1)"
            // to disable, e.g. if offTemp is 100, it needs to go GREATER than 100 to enable, i.e. 101, and then 98 to
            // disable, because the coolant temp is an integer. So 98.5 degrees to 100.5 degrees is the analog null zone
            // where nothing happens, depending on sensor calibration and table interpolation. Hopefully offTemp wasn't
            // -40... otherwise underflow... but that would be ridiculous
    ...
    if ((currentStatus.RPM < (configPage15.airConMinRPMdiv10 * 10)) ||
        (currentStatus.RPMdiv100 > configPage15.airConMaxRPMdiv100))
        {
            ...
            acRPMLockoutDelay = 0;
             ...
             (currentStatus.RPMdiv100 <= configPage15.airConMaxRPMdiv100))
        {
            // No need to add hysteresis as we have the stand-down delay period after the high/low RPM condition goes
            // away.
    ...
    if (configPage2.fanEnable == 2)  // PWM Fan control
        {
            ...
            fan_pwm_max_count =
                (uint16_t)(MICROS_PER_SEC / (32U * configPage6.fanFreq *
                                             2U));  // Converts the frequency in Hz to the number of ticks (at 16uS) it
                                                    // takes to complete 1 cycle. Note that the frequency is divided by
                                                    // 2 coming from TS to allow for up to 512hz
#endif
            fan_pwm_value = 0;
        }
#endif
    ...
    if (configPage2.fanEnable == 1)  // regular on/off fan control
        {
            ...
            if ((fanPermit == true) && ((currentStatus.coolant >= onTemp) ||
                                        ((configPage15.airConTurnsFanOn) == 1 &&
                                         BIT_CHECK(currentStatus.airConStatus, BIT_AIRCON_TURNING_ON) == true)))
                {
                    // Fan needs to be turned on - either by high coolant temp, or from an A/C request (to ensure there
                    // is airflow over the A/C radiator).
            ...
            if (fanPermit == true)
                {
                    if (BIT_CHECK(currentStatus.engine, BIT_ENGINE_CRANK) && (configPage2.fanWhenCranking == 0))
                        {
                            currentStatus.fanDuty =
                                0;  // If the user has elected to disable the fan during cranking, make sure it's off
                            BIT_CLEAR(currentStatus.status4, BIT_STATUS4_FAN);
#if defined(PWM_FAN_AVAILABLE)  // PWM fan not available on Arduino MEGA
                            DISABLE_FAN_TIMER();
#endif
                        ...
                        {
                            byte tempFanDuty = table2D_getValue(
                                &fanPWMTable,
                                currentStatus.coolant + CALIBRATION_TEMPERATURE_OFFSET);  // In normal situation read
                                                                                          // PWM duty from the table
                            if ((configPage15.airConTurnsFanOn) == 1 &&
                                BIT_CHECK(currentStatus.airConStatus, BIT_AIRCON_TURNING_ON) == true)
                                {
                                    // Clamp the fan duty to airConPwmFanMinDuty or above, to ensure there is airflow
                                    // over the A/C radiator
...
#if defined(PWM_FAN_AVAILABLE)
                            fan_pwm_value =
                                halfPercentage(currentStatus.fanDuty, fan_pwm_max_count);  // update FAN PWM value last
                ...
                {
                    currentStatus.fanDuty = 0;  ////If the user has elected to disable the fan when engine is not
                                                ///running, make sure it's off
                    BIT_CLEAR(currentStatus.status4, BIT_STATUS4_FAN);
                }
...
void initialiseAuxPWM(void)
{
    ...
    // This is a safety check that will be true if the board is uninitialised. This prevents hangs on a new board that
    // could otherwise try to write to an invalid pin port/mask (Without this a new Teensy 4.x hangs on startup) The
    // n2o_minTPS variable is capped at 100 by TS, so 255 indicates a new board.
...
#if defined(CORE_AVR)
            vvt_pwm_max_count =
                (uint16_t)(MICROS_PER_SEC / (16U * configPage6.vvtFreq *
                                             2U));  // Converts the frequency in Hz to the number of ticks (at 16uS) it
                                                    // takes to complete 1 cycle. Note that the frequency is divided by
                                                    // 2 coming from TS to allow for up to 512hz
#elif defined(CORE_TEENSY35)
            vvt_pwm_max_count =
                (uint16_t)(MICROS_PER_SEC / (32U * configPage6.vvtFreq *
                                             2U));  // Converts the frequency in Hz to the number of ticks (at 16uS) it
                                                    // takes to complete 1 cycle. Note that the frequency is divided by
                                                    // 2 coming from TS to allow for up to 512hz
#elif defined(CORE_TEENSY41)
            vvt_pwm_max_count =
                (uint16_t)(MICROS_PER_SEC / (2U * configPage6.vvtFreq *
                                             2U));  // Converts the frequency in Hz to the number of ticks (at 2uS) it
                                                    // takes to complete 1 cycle. Note that the frequency is divided by
                                                    // 2 coming from TS to allow for up to 512hz
#endif
                    ...
                    switch (currentStatus.gear)
                        {
                            case 1:
                                combinedBoost = (((uint16_t)configPage9.boostByGear1 *
                                                  (uint16_t)get3DTableValue(&boostTable, (currentStatus.TPS * 2),
                                                                            currentStatus.RPM)))
                                                << 2;
                                ...
                                break;
                            case 2:
                                combinedBoost = (((uint16_t)configPage9.boostByGear2 *
                                                  (uint16_t)get3DTableValue(&boostTable, (currentStatus.TPS * 2),
                                                                            currentStatus.RPM)))
                                                << 2;
                                if (combinedBoost <= 10000)
                                ...
                                break;
                            case 3:
                                combinedBoost = (((uint16_t)configPage9.boostByGear3 *
                                                  (uint16_t)get3DTableValue(&boostTable, (currentStatus.TPS * 2),
                                                                            currentStatus.RPM)))
                                                << 2;
                                if (combinedBoost <= 10000)
                                ...
                                break;
                            case 4:
                                combinedBoost = (((uint16_t)configPage9.boostByGear4 *
                                                  (uint16_t)get3DTableValue(&boostTable, (currentStatus.TPS * 2),
                                                                            currentStatus.RPM)))
                                                << 2;
                                if (combinedBoost <= 10000)
                                ...
                                break;
                            case 5:
                                combinedBoost = (((uint16_t)configPage9.boostByGear5 *
                                                  (uint16_t)get3DTableValue(&boostTable, (currentStatus.TPS * 2),
                                                                            currentStatus.RPM)))
                                                << 2;
                                if (combinedBoost <= 10000)
                                ...
                                break;
                            case 6:
                                combinedBoost = (((uint16_t)configPage9.boostByGear6 *
                                                  (uint16_t)get3DTableValue(&boostTable, (currentStatus.TPS * 2),
                                                                            currentStatus.RPM)))
                                                << 2;
                                if (combinedBoost <= 10000)
    ...
    if (configPage6.boostEnabled == 1)
        {
                    ...
                    if (currentStatus.boostDuty == 0)
                    ...
                    else
                        {
                            boost_pwm_target_value =
                                ...
                                10000;  // Convert boost duty (Which is a % multiplied by 100) to a pwm count
                        }
                }
            else if (configPage4.boostType == CLOSED_LOOP_BOOST)
                {
                            ...
                            if ((configPage9.boostByGearEnabled > 0) && (configPage2.vssMode > 1))
                            ...
                            else
                                {
                                    currentStatus.boostTarget =
                                        get3DTableValue(&boostTable, (currentStatus.TPS * 2), currentStatus.RPM) << 1;
                    ...
                    if (((configPage15.boostControlEnable == EN_BOOST_CONTROL_BARO) &&
                        ...
                        ((configPage15.boostControlEnable == EN_BOOST_CONTROL_FIXED) &&
                         (currentStatus.MAP >=
                          configPage15.boostControlEnableThreshold)))  // Only enables boost control above baro pressure
                                                                       // or above user defined threshold (User defined
                                                                       // level is usually set to boost with wastegate
                                                                       // actuator only boost level)
                        {
                            // If flex fuel is enabled, there can be an adder to the boost target based on ethanol
                            // content
                            ...
                            if (currentStatus.boostTarget > 0)
                                {
                                    ...
                                    bool PIDcomputed = boostPID.Compute(
                                        get3DTableValue(&boostTableLookupDuty, currentStatus.boostTarget,
                                                        currentStatus.RPM) *
                                        100 /
                                        2);  // Compute() returns false if the required interval has not yet passed.
                                    if (currentStatus.boostDuty == 0)
                                        {
                                            DISABLE_BOOST_TIMER();
                                            BOOST_PIN_LOW();
                    ...
                    else
                        {
                            ...
                            // Boost control needs to have a high duty cycle if control is below threshold (baro or
                            // fixed value). This ensures the waste gate is closed as much as possible, this build boost
                            // as fast as possible.
                            currentStatus.boostDuty = configPage15.boostDCWhenDisabled * 100;
                            boost_pwm_target_value =
                                ((unsigned long)(currentStatus.boostDuty) * boost_pwm_max_count) /
                                10000;             // Convert boost duty (Which is a % multiplied by 100) to a pwm count
                            ENABLE_BOOST_TIMER();  // Turn on the compare unit (ie turn on the interrupt) if boost duty
                                                   // >0
                            if (currentStatus.boostDuty == 0)
                                {
                                    boostDisable();
            ...
            if (currentStatus.boostDuty >= 10000)
                {
                    DISABLE_BOOST_TIMER();  // Turn off the compare unit (ie turn off the interrupt) if boost duty is
                                            // 100%
...
void vvtControl(void)
{
    if ((configPage6.vvtEnabled == 1) &&
        ...
        (BIT_CHECK(currentStatus.engine, BIT_ENGINE_RUN)))
        {
            ...
            if ((vvtIsHot == true) ||
                ((runSecsX10 - vvtWarmTime) >= (configPage4.vvtDelay * VVT_TIME_DELAY_MULTIPLIER)))
                {
                    vvtIsHot = true;
                    ...
                    if ((configPage6.vvtMode == VVT_MODE_OPEN_LOOP) || (configPage6.vvtMode == VVT_MODE_ONOFF))
                        {
                            ...
                            if (configPage6.vvtLoadSource == VVT_LOAD_TPS)
                                {
                                    currentStatus.vvt1Duty =
                                        get3DTableValue(&vvtTable, (currentStatus.TPS * 2), currentStatus.RPM);
                                }
                            else
                                {
                                    currentStatus.vvt1Duty =
                            ...
                            // VVT table can be used for controlling on/off switching. If this is turned on, then
                            // disregard any interpolation or non-binary values
                            ...
                            if (configPage10.vvt2Enabled == 1)  // same for VVT2 if it's enabled
                                {
                                    ...
                                    if (configPage6.vvtLoadSource == VVT_LOAD_TPS)
                                        {
                                            currentStatus.vvt2Duty =
                                                get3DTableValue(&vvt2Table, (currentStatus.TPS * 2), currentStatus.RPM);
                                        }
                                    else
                                        {
                                            currentStatus.vvt2Duty =
                                                get3DTableValue(&vvt2Table, (currentStatus.MAP), currentStatus.RPM);
                                        }
                    ...
                    else if ((configPage6.vvtMode == VVT_MODE_CLOSED_LOOP))
                        {
                            ...
                            if ((vvtCounter & 31) == 1)
                                {
                                    vvtPID.SetTunings(
                                        ...
                                        configPage10
                                            .vvtCLKD);  // This only needs to be run very infrequently, once every 32
                                                        // calls to vvtControl(). This is approx. once per second
                                    vvtPID.SetControllerDirection(configPage6.vvtPWMdir);
                                }

                            // safety check that the cam angles are ok. The engine will be totally undriveable if the
                            // cam sensor is faulty and giving wrong cam angles, so if that happens, default to 0 duty.
                            ...
                            // Check that we're not already at the angle we want to be
                            ...
                            else
                                {
                                    ...
                                    // if (currentStatus.vvt1Angle > currentStatus.vvt1TargetAngle) {
                                    // vvt_pwm_target_value = 0; }
                                    if (PID_compute == true)
                                        {
                                            vvt1_pwm_value = halfPercentage(currentStatus.vvt1Duty, vvt_pwm_max_count);
                                        }
                            ...
                            if (configPage10.vvt2Enabled == 1)  // same for VVT2 if it's enabled
                                {
                                    ...
                                    if ((vvtCounter & 31) == 1)
                                        {
                                            vvt2PID.SetTunings(
                                                configPage10.vvtCLKP, configPage10.vvtCLKI,
                                                configPage10.vvtCLKD);  // This only needs to be run very infrequently,
                                    ...
                                    // if the cam sensor is faulty and giving wrong cam angles, so if that happens,
                                    // default to 0 duty. This also prevents using zero or negative current angle values
                                    // for PID adjustment, because those don't work in integer PID.
    ...
    if (configPage10.n2o_enable > 0)
        {
            ...
            if ((isArmed == true) &&
                ...
                (currentStatus.TPS > configPage10.n2o_minTPS) && (currentStatus.O2 < configPage10.n2o_maxAFR) &&
                (currentStatus.MAP < (uint16_t)(configPage10.n2o_maxMAP * 2)))
                {
                    // Config page values are divided by 100 to fit within a byte. Multiply them back out to real
                    // values.
                            ...
                            switch (configPage10.wmiMode)
                                {
                                    ...
                                    case WMI_MODE_PROPORTIONAL:
                                        // Proportional Mode - Output PWM is proportionally controlled between two MAP
                                        // values - MAP Value 1 = PWM:0% / MAP Value 2 = PWM:100%
                                        wmiPW = map(currentStatus.MAP / 2, configPage10.wmiMAP, configPage10.wmiMAP2, 0,
                                                    200);
                                        break;
                                    case WMI_MODE_OPENLOOP:
                                        //  Mapped open loop - Output PWM follows 2D map value (RPM vs MAP) Cell value
                                        //  contains desired PWM% [range 0-100%]
                                    ...
                                    case WMI_MODE_CLOSEDLOOP:
                                        ...
                                        // correction map applied (RPM vs MAP). Cell value contains correction value%
                                        // [nom 100%]
                                        wmiPW = max(0, ((int)currentStatus.PW1 + configPage10.wmiOffset)) *
                                                get3DTableValue(&wmiTable, currentStatus.MAP, currentStatus.RPM) / 200;
                                        break;
                                    default:
                                        // Wrong mode
                                        wmiPW = 0;
            ...
            if (nextVVT == 0)
                {
                    ...
                    if (vvt2_pwm_state == true)
                        {
                            SET_COMPARE(VVT_TIMER_COMPARE,
                                        VVT_TIMER_COUNTER + (vvt2_pwm_cur_value - vvt1_pwm_cur_value));
                        }
                    else
                        {
                            SET_COMPARE(VVT_TIMER_COMPARE,
                                        VVT_TIMER_COUNTER + (vvt_pwm_max_count - vvt1_pwm_cur_value));
            ...
            else if (nextVVT == 1)
                {
                    ...
                    if (vvt1_pwm_state == true)
                        {
                            SET_COMPARE(VVT_TIMER_COMPARE,
                                        VVT_TIMER_COUNTER + (vvt1_pwm_cur_value - vvt2_pwm_cur_value));
                        }
                    else
                        {
                            SET_COMPARE(VVT_TIMER_COMPARE,
                                        VVT_TIMER_COUNTER + (vvt_pwm_max_count - vvt2_pwm_cur_value));
                            nextVVT = 2;  // Next event is for both PWM
                        }
                }
            else
