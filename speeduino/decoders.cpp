 ...
 * - **triggerPri_xxxx** - Called each time the primary (No. 1) crank/cam signal is triggered (Called as an interrupt,
 * so variables must be declared volatile)
 * - **triggerSec_xxxx** - Called each time the secondary (No. 2) crank/cam signal is triggered (Called as an interrupt,
 * so variables must be declared volatile)
 ...
 * - Relying on JAVADOC_AUTOBRIEF (in Doxyfile, essentially automatic @brief), the first sentence (ending with period)
 * becomes the longer description (second column following name) on (Doxygen) "Modules" page (old Desc: ... could be
 * this sentence)
 * - All the content after first sentence (like old Note:...) is visible on the page linked from the name (1st col) on
 * "Modules" page
 * - To group all decoders together add 1) @defgroup dec Decoders (on top) and 2) "@ingroup dec" to each decoder (under
 * @defgroup)
...
void (*triggerHandler)(void) =
    nullTriggerHandler;  /// Pointer for the trigger function (Gets pointed to the relevant decoder)
void (*triggerSecondaryHandler)(void) =
    nullTriggerHandler;  /// Pointer for the secondary trigger function (Gets pointed to the relevant decoder)
void (*triggerTertiaryHandler)(void) =
    nullTriggerHandler;  /// Pointer for the tertiary trigger function (Gets pointed to the relevant decoder)
...
int (*getCrankAngle)(void) =
    nullGetCrankAngle;  /// Pointer to the getCrank Angle function (Gets pointed to the relevant decoder)
void (*triggerSetEndTeeth)(void) =
    triggerSetEndTeeth_missingTooth;  /// Pointer to the triggerSetEndTeeth function of each decoder
...
unsigned long MAX_STALL_TIME =
    MICROS_PER_SEC /
    2U;  // The maximum time (in uS) that the system will continue to function before the engine is considered
         // stalled/stopped. This is unique to each decoder, depending on the number of teeth etc. 500000 (half a
         // second) is used as the default value, most decoders will be much less.
volatile uint16_t toothCurrentCount =
    0;  // The current number of teeth (Once sync has been achieved, this can never actually be 0
volatile byte toothSystemCount =
    0;  // Used for decoders such as Audi 135 where not every tooth is used for calculating crank angle. This variable
        // stores the actual number of teeth, not the number being used to calculate crank angle
volatile unsigned long toothSystemLastToothTime =
    0;  // As below, but used for decoders where not every tooth count is used for calculation
volatile unsigned long toothLastToothTime = 0;  // The time (micros()) that the last tooth was registered
volatile unsigned long toothLastSecToothTime =
    0;  // The time (micros()) that the last tooth was registered on the secondary input
volatile unsigned long toothLastThirdToothTime =
    0;  // The time (micros()) that the last tooth was registered on the second cam input
volatile unsigned long toothLastMinusOneToothTime =
    0;  // The time (micros()) that the tooth before the last tooth was registered
volatile unsigned long toothLastMinusOneSecToothTime =
    0;  // The time (micros()) that the tooth before the last tooth was registered on secondary input
volatile unsigned long toothLastToothRisingTime =
    0;  // The time (micros()) that the last tooth rose (used by special decoders to determine missing teeth polarity)
volatile unsigned long toothLastSecToothRisingTime =
    0;  // The time (micros()) that the last tooth rose on the secondary input (used by special decoders to determine
        // missing teeth polarity)
...
volatile bool revolutionOne =
    0;  // For sequential operation, this tracks whether the current revolution is 1 or 2 (not 1)
volatile bool revolutionLastOne = 0;  // used to identify in the rover pattern which has a non unique primary trigger
                                      // something unique - has the secondary tooth changed.

volatile unsigned int secondaryToothCount;  // Used for identifying the current secondary (Usually cam) tooth for
                                            // patterns with multiple secondary teeth
volatile unsigned int secondaryLastToothCount =
    0;  // used to identify in the rover pattern which has a non unique primary trigger something unique - has the
        // secondary tooth changed.
volatile unsigned long secondaryLastToothTime = 0;  // The time (micros()) that the last tooth was registered (Cam
                                                    // input)
volatile unsigned long secondaryLastToothTime1 =
    0;  // The time (micros()) that the last tooth was registered (Cam input)

volatile unsigned int thirdToothCount;  // Used for identifying the current third (Usually exhaust cam - used for VVT2)
                                        // tooth for patterns with multiple secondary teeth
...
volatile unsigned long
    triggerFilterTime;  // The shortest time (in uS) that pulses will be accepted (Used for debounce filtering)
volatile unsigned long triggerSecFilterTime;    // The shortest time (in uS) that pulses will be accepted (Used for
                                                // debounce filtering) for the secondary input
volatile unsigned long triggerThirdFilterTime;  // The shortest time (in uS) that pulses will be accepted (Used for
                                                // debounce filtering) for the Third input
...
byte checkSyncToothCount;  // How many teeth must've been seen on this revolution before we try to confirm sync (Useful
                           // for missing tooth type decoders)
...
int16_t toothAngles[24];  // An array for storing fixed tooth angles. Currently sized at 24 for the GM 24X decoder, but
                          // may grow later if there are other decoders that use this style
            ...
            if (currentStatus.toothLogEnabled == true)
            ...
            else if (currentStatus.compositeTriggerUsed > 0)
                {
                    ...
                    if (currentStatus.compositeTriggerUsed == 4)
                        {
                            // we want to display both cams so swap the values round to display primary as cam1 and
                            // secondary as cam2, include the crank in the data as the third output
                            ...
                            if (currentStatus.compositeTriggerUsed == 3)
                                {
                                    ...
                                    if (READ_THIRD_TRIGGER() == true)
                                        {
                                            BIT_SET(compositeLogHistory[toothHistoryIndex], COMPOSITE_LOG_SEC);
                                        }  // only the COMPOSITE_LOG_SEC value is visualised hence the swapping of the
                                           // data
                                    if (READ_SEC_TRIGGER() == true)
                                        {
                                            BIT_SET(compositeLogHistory[toothHistoryIndex], COMPOSITE_LOG_THIRD);
...
{
    BIT_CLEAR(decoderState,
              BIT_DECODER_VALID_TRIGGER);  // This value will be set to the return value of the decoder function,
    ...
    */
    if (((primaryTriggerEdge == RISING) && (READ_PRI_TRIGGER() == HIGH)) ||
        ((primaryTriggerEdge == FALLING) && (READ_PRI_TRIGGER() == LOW)) || (primaryTriggerEdge == CHANGE))
        {
            triggerHandler();
            validEdge = true;
        }
...
void loggerSecondaryISR(void)
{
    ...
    */
    if (((secondaryTriggerEdge == RISING) && (READ_SEC_TRIGGER() == HIGH)) ||
        ((secondaryTriggerEdge == FALLING) && (READ_SEC_TRIGGER() == LOW)) || (secondaryTriggerEdge == CHANGE))
        {
...
void loggerTertiaryISR(void)
{
    ...
    */
    ...
    if (((tertiaryTriggerEdge == RISING) && (READ_THIRD_TRIGGER() == HIGH)) ||
        ((tertiaryTriggerEdge == FALLING) && (READ_THIRD_TRIGGER() == LOW)) || (tertiaryTriggerEdge == CHANGE))
        {
    ...
    if (revTime != revolutionTime)
        {
            ...
            degreesPerMicro =
                (uint16_t)UDIV_ROUND_CLOSEST((UINT32_C(360) << degreesPerMicro_Shift), revolutionTime, uint32_t);
            return true;
        }
    return false;
}
...
{
    noInterrupts();
    bool updatedRevTime = HasAnySync(currentStatus) && !IsCranking(currentStatus) &&
                          (toothOneMinusOneTime != UINT32_C(0)) &&
                          (toothOneTime > toothOneMinusOneTime)
                          // The time in uS that one revolution would take at current speed (The time tooth 1 was last
                          // seen, minus the time it was seen prior to that)
...
static inline uint16_t clampRpm(uint16_t rpm) { return rpm >= MAX_RPM ? currentStatus.RPM : rpm; }
...
static inline uint16_t RpmFromRevolutionTimeUs(uint32_t revTime)
{
    if (revTime < UINT16_MAX)
        ...
        {
            return clampRpm((uint16_t)UDIV_ROUND_CLOSEST(
                MICROS_PER_MIN, revTime, uint32_t));  // Calc RPM based on last full revolution time (Faster as /)
        }
}

/** Compute RPM.
 * As nearly all the decoders use a common method of determining RPM (The time the last full revolution took) A common
 * function is simpler.
 * @param degreesOver - the number of crank degrees between tooth #1s. Some patterns have a tooth #1 every crank rev,
 * others are every cam rev.
 * @return RPM
 */
...
/**
This is a special case of RPM measure that is based on the time between the last 2 teeth rather than the time of the
last full revolution. This gives much more volatile reading, but is quite useful during cranking, particularly on low
resolution patterns. It can only be used on patterns where the teeth are evenly spaced. It takes an argument of the full
(COMPLETE) number of teeth per revolution. For a missing tooth wheel, this is the number if the tooth had NOT been
missing (Eg 36-1 = 36)
*/
static __attribute__((noinline)) int crankingGetRPM(byte totalTeeth, bool isCamTeeth)
{
    if ((currentStatus.startRevolutions >= configPage4.StgCycles) &&
        ((currentStatus.hasSync == true) || BIT_CHECK(currentStatus.status3, BIT_STATUS3_HALFSYNC)))
        {
            if ((toothLastMinusOneToothTime > 0) && (toothLastToothTime > toothLastMinusOneToothTime))
                {
                    noInterrupts();
                    bool newRevtime = SetRevolutionTime(
                        ((toothLastToothTime - toothLastMinusOneToothTime) * totalTeeth) >> (isCamTeeth ? 1U : 0U));
                    interrupts();
                    if (newRevtime)
                        {
                            return RpmFromRevolutionTimeUs(revolutionTime);
...
/**
On decoders that are enabled for per tooth based timing adjustments, this function performs the timer compare changes on
the schedules themselves For each ignition channel, a check is made whether we're at the relevant tooth and whether that
ignition schedule is currently running Only if both these conditions are met will the schedule be updated with the
latest timing information. If it's the correct tooth, but the schedule is not yet started, calculate and an end compare
value (This situation occurs when both the start and end of the ignition pulse happen after the end tooth, but before
the next tooth)
*/
...
void triggerSetup_missingTooth(void)
{
    ...
    triggerActualTeeth = configPage4.triggerTeeth -
                         configPage4.triggerMissingTeeth;  // The number of physical teeth on the wheel. Doing this here
                                                           // saves us a calculation each time in the interrupt
    triggerFilterTime =
        (MICROS_PER_SEC /
         (MAX_RPM / 60U * configPage4.triggerTeeth));  // Trigger filter time is the shortest possible time (in uS) that
                                                       // there can be between crank teeth (ie at max RPM). Any pulses
                                                       // that occur faster than this time will be discarded as noise
    ...
    MAX_STALL_TIME =
        ((MICROS_PER_DEG_1_RPM / 50U) * triggerToothAngle *
         (configPage4.triggerMissingTeeth + 1U));  // Minimum 50rpm. (3333uS is the time per degree at 50rpm)

    if ((configPage4.TrigSpeed == CRANK_SPEED) &&
        ((configPage4.sparkMode == IGN_MODE_SEQUENTIAL) || (configPage2.injLayout == INJ_SEQUENTIAL)))
        {
...
void triggerPri_missingTooth(void)
{
    ...
    if (curGap >= triggerFilterTime)  // Pulses should never be less than triggerFilterTime, so if they are it means a
                                      // false trigger. (A 36-1 wheel at 8000pm will have triggers approx. every 200uS)
        {
            toothCurrentCount++;  // Increment the tooth counter
            BIT_SET(decoderState,
                    BIT_DECODER_VALID_TRIGGER);  // Flag this pulse as being a valid trigger (ie that it passed filters)
            ...
            if ((toothLastToothTime > 0) && (toothLastMinusOneToothTime > 0))
                {
                    ...
                    2. We have sync and are in the final 1/4 of the wheel (Missing tooth will/should never occur in the
                    first 3/4)
                    3. RPM is under 2000. This is to ensure that we don't interfere with strange timing when cranking or
                    idling. Optimisation not really required at these speeds anyway
                    */
                    if ((currentStatus.hasSync == false) || (currentStatus.RPM < 2000) ||
                        (toothCurrentCount >= (3 * triggerActualTeeth >> 2)))
                        {
                            // Begin the missing tooth detection
                            // If the time between the current tooth and the last is greater than 1.5x the time between
                            // the last tooth and the tooth before that, we make the assertion that we must be at the
                            // first tooth after the gap
                            if (configPage4.triggerMissingTeeth == 1)
                                {
                                    targetGap = (3 * (toothLastToothTime - toothLastMinusOneToothTime)) >> 1;
                                }  // Multiply by 1.5 (Checks for a gap 1.5x greater than the last one) (Uses bitshift
                                   // to multiply by 3 then divide by 2. Much faster than multiplying by 1.5)
                            else
                                {
                                    targetGap = ((toothLastToothTime - toothLastMinusOneToothTime)) *
                                                configPage4.triggerMissingTeeth;
                            ...
                            if ((curGap > targetGap) || (toothCurrentCount > triggerActualTeeth))
                                {
                                    ...
                                    if ((toothCurrentCount < triggerActualTeeth) && (currentStatus.hasSync == true))
                                        {
                                            ...
                                            // This indicates a signal issue so we flag lost sync so this will attempt
                                            // to resync on the next revolution.
                                            currentStatus.hasSync = false;
                                            BIT_CLEAR(
                                                currentStatus.status3,
                                                BIT_STATUS3_HALFSYNC);  // No sync at all, so also clear HalfSync bit.
                                            currentStatus.syncLossCounter++;
                                        }
                                    // This is to handle a special case on startup where sync can be obtained and the
                                    // system immediately thinks the revs have jumped: else if (currentStatus.hasSync ==
                                    // false && toothCurrentCount < checkSyncToothCount ) { triggerFilterTime = 0; }
                                    else
                                        {
                                            if ((currentStatus.hasSync == true) ||
                                                BIT_CHECK(currentStatus.status3, BIT_STATUS3_HALFSYNC))
                                                {
                                                    ...
                                                    if (configPage4.TrigSpeed == CAM_SPEED)
                                                        ...
                                                        }  // Add an extra revolution count if we're running at cam
                                                           // speed
                                                }
                                            else
                                                {
                                            ...
                                            toothCurrentCount = 1;
                                            if (configPage4.trigPatternSec ==
                                                SEC_TRIGGER_POLL)  // at tooth one check if the cam sensor is high or
                                                                   // low in poll level mode
                                                {
                                            ...
                                            if ((configPage4.sparkMode == IGN_MODE_SEQUENTIAL) ||
                                                ...
                                                {
                                                    // If either fuel or ignition is sequential, only declare sync if
                                                    // the cam tooth has been seen OR if the missing wheel is on the cam
                                                    if ((secondaryToothCount > 0) ||
                                                        ...
                                                        (configPage2.strokes == TWO_STROKE))
                                                        {
                                                            ...
                                                            BIT_CLEAR(
                                                                currentStatus.status3,
                                                                BIT_STATUS3_HALFSYNC);  // the engine is fully synced so
                                                                                        // clear the Half Sync bit
                                                        }
                                                    else if (currentStatus.hasSync != true)
                                                        {
                                                            BIT_SET(currentStatus.status3, BIT_STATUS3_HALFSYNC);
                                                        }  // If there is primary trigger but no secondary we only have
                                                           // half sync.
                                                }
                                            else
                                                {
                                                ...
                                                }  // If nothing is using sequential, we have sync and also clear half
                                                   // sync bit
                                            if (configPage4.trigPatternSec == SEC_TRIGGER_SINGLE ||
                                                configPage4.trigPatternSec ==
                                                    SEC_TRIGGER_TOYOTA_3)  // Reset the secondary tooth counter to
                                                                           // prevent it overflowing, done outside of
                                                                           // sequental as v6 & v8 engines could be
                                                                           // batch firing with VVT that needs the cam
                                                                           // resetting
                                                {
                                                    secondaryToothCount = 0;
                                                }
                                            ...
                                            triggerFilterTime =
                                                0;  // This is used to prevent a condition where serious intermittent
                                                    // signals (Eg someone furiously plugging the sensor wire in and
                                                    // out) can leave the filter in an unrecoverable state
                                            toothLastMinusOneToothTime = toothLastToothTime;
                                            toothLastToothTime = curTime;
                                            BIT_CLEAR(decoderState,
                                                      BIT_DECODER_TOOTH_ANG_CORRECT);  // The tooth angle is double at
                                                                                       // this point
                                        }
                                }
                        }
            ...
            switch (configPage4.trigPatternSec)
                {
                    case SEC_TRIGGER_4_1:
                        targetGap2 = (3 * (toothLastSecToothTime - toothLastMinusOneSecToothTime)) >>
                                     1;  // If the time between the current tooth and the last is greater than 1.5x the
                                         // time between the last tooth and the tooth before that, we make the assertion
                                         // that we must be at the first tooth after the gap
                        ...
                        if ((curGap2 >= targetGap2) || (secondaryToothCount > 3))
                            {
                                ...
                                triggerSecFilterTime =
                                    0;  // This is used to prevent a condition where serious intermittent signals (Eg
                                        // someone furiously plugging the sensor wire in and out) can leave the filter
                            ...
                            {
                                triggerSecFilterTime =
                                    curGap2 >> 2;  // Set filter at 25% of the current speed. Filter can only be
                                                   // recalc'd for the regular teeth, not the missing one.
                                secondaryToothCount++;
                            }
                        break;
                    ...
                    case SEC_TRIGGER_TOYOTA_3:
                        ...
                        // Next secondary filter is 25% the current gap, done here so we don't get a great big gap for
                        // the 1st tooth
    ...
    if (curGap3 >= triggerThirdFilterTime)
        {
            ...
            // currentStatus.vvt2Angle = int8_t (curAngle); //vvt1Angle is only int8, but +/-127 degrees is enough for
            // VVT control
            ...
            if (toothCurrentCount != 1)
                {
                    tempRPM = crankingGetRPM(configPage4.triggerTeeth,
                                             configPage4.TrigSpeed == CAM_SPEED);  // Account for cam speed
                }
            else
                {
...
{
    // This is the current angle ATDC the engine is at. This is the last known position based on what tooth was last
    // 'seen'. It is only accurate to the resolution of the trigger wheel (Eg 36-1 is 10 degrees)
    ...
    int crankAngle = ((tempToothCurrentCount - 1) * triggerToothAngle) +
                     configPage4.triggerAngle;  // Number of teeth that have passed since tooth 1, multiplied by the
                                                // angle each tooth represents, plus the angle that tooth 1 is ATDC.
...
/** Dual wheels - 2 wheels located either both on the crank or with the primary on the crank and the secondary on the
cam. Note: There can be no missing teeth on the primary wheel.
    ...
    triggerFilterTime =
        (MICROS_PER_SEC /
         (MAX_RPM / 60U * configPage4.triggerTeeth));  // Trigger filter time is the shortest possible time (in uS) that
                                                       ...
                                                       // that occur faster than this time will be discarded as noise
    triggerSecFilterTime =
        (MICROS_PER_SEC / (MAX_RPM / 60U * 2U)) /
        2U;  // Same as above, but fixed at 2 teeth on the secondary input and divided by 2 (for cam speed)
    BIT_CLEAR(decoderState, BIT_DECODER_2ND_DERIV);
    BIT_SET(decoderState, BIT_DECODER_IS_SEQUENTIAL);
    BIT_SET(decoderState, BIT_DECODER_TOOTH_ANG_CORRECT);  // This is always true for this pattern
    BIT_SET(decoderState, BIT_DECODER_HAS_SECONDARY);
    MAX_STALL_TIME =
        ((MICROS_PER_DEG_1_RPM / 50U) * triggerToothAngle);  // Minimum 50rpm. (3333uS is the time per degree at 50rpm)
#ifdef USE_LIBDIVIDE
    divTriggerToothAngle = libdivide::libdivide_s16_gen(triggerToothAngle);
#endif
}
...
void triggerSec_DualWheel(void)
{
    ...
    if (curGap2 >= triggerSecFilterTime)
            ...
            if ((currentStatus.hasSync == false) || (currentStatus.startRevolutions <= configPage4.StgCycles))
                {
                    ...
                    toothLastMinusOneToothTime =
                        micros() -
                        ((MICROS_PER_MIN / 10U) /
                         configPage4.triggerTeeth);  // Fixes RPM at 10rpm until a full revolution has taken place
                    toothCurrentCount = configPage4.triggerTeeth;
                    triggerFilterTime = 0;  // Need to turn the filter off here otherwise the first primary tooth after
        ...
        {
            triggerSecFilterTime =
                revolutionTime >> 1;  // Set filter at 25% of the current cam speed. This needs to be performed here to
                                      // prevent a situation where the RPM and triggerSecFilterTime get out of alignment
                                      // and curGap2 never exceeds the filter value
        }  // Trigger filter
}
/** Dual Wheel - Get RPM.
 *
...
{
    // The toothAdder variable is used for when a setup is running sequentially, but the primary wheel is running at
    // crank speed. This way the count of teeth will go up to 2* the number of primary teeth to allow for a sequential
    // count.
    byte toothAdder = 0;
    if ((configPage4.sparkMode == IGN_MODE_SEQUENTIAL) && (configPage4.TrigSpeed == CRANK_SPEED))
        {
...
void triggerSetup_BasicDistributor(void)
{
    ...
    if (configPage2.nCylinders <= 4U)
        ...
        }  // Minimum 90rpm. (1851uS is the time per degree at 90rpm). This uses 90rpm rather than 50rpm due to the
           // potentially very high stall time on a 4 cylinder if we wait that long.
    else
        {
    ...
    if ((curGap >= triggerFilterTime))
        {
            if (currentStatus.hasSync == true)
                ...
                {
                    triggerFilterTime = 0;
                }  // If we don't yet have sync, ensure that the filter won't prevent future valid pulses from being
                   // ignored.

            if ((toothCurrentCount == triggerActualTeeth) ||
                (currentStatus.hasSync == false))  // Check if we're back to the beginning of a revolution
                {
...
uint16_t getRPM_BasicDistributor(void)
{
    ...
    MAX_STALL_TIME = revolutionTime << 1;  // Set the stall time to be twice the current RPM. This is a safe figure as
 ...
 * Note: Within the decoder code pf GM7X, the sync tooth is referred to as tooth #3 rather than tooth #7. This makes for
 * simpler angle calculations (See: http://www.speeduino.com/forum/download/file.php?id=4743 ).
    ...
    if ((toothLastToothTime > 0) && (toothLastMinusOneToothTime > 0))
        {
            if (toothCurrentCount > 7)
            ...
            else
                {
                    ...
                    if (curGap < targetGap)  // If the gap between this tooth and the last one is less than half of the
                                             // previous gap, then we are very likely at the magical 3rd tooth
                        {
                            toothCurrentCount = 3;
                            currentStatus.hasSync = true;
                            BIT_CLEAR(decoderState,
                                      BIT_DECODER_TOOTH_ANG_CORRECT);  // The tooth angle is double at this point
                            currentStatus.startRevolutions++;          // Counter
                        }
                    else
                        {
            ...
            if (toothCurrentCount != 3)  // Never do the check on the extra tooth. It's not needed anyway
                {
                    ...
                    if (toothCurrentCount < 3)
                        {
                            crankAngle =
                                ((toothCurrentCount - 1) * triggerToothAngle) + 42 +
                                configPage4
                                    .triggerAngle;  // Number of teeth that have passed since tooth 1, multiplied by the
                                                    // angle each tooth represents, plus the angle that tooth 1 is ATDC.
                                                    // This gives accuracy only to the nearest tooth.
                        }
                    else
                        {
                            crankAngle =
                                ((toothCurrentCount - 2) * triggerToothAngle) + 42 +
                                configPage4
                                    .triggerAngle;  // Number of teeth that have passed since tooth 1, multiplied by the
                                                    // angle each tooth represents, plus the angle that tooth 1 is ATDC.
                                                    // This gives accuracy only to the nearest tooth.
...
uint16_t getRPM_GM7X(void) { return stdGetRPM(CRANK_SPEED); }
int getCrankAngle_GM7X(void)
{
    ...
    // Check if the last tooth seen was the reference tooth (Number 3). All others can be calculated, but tooth 3 has a
    // unique angle
    int crankAngle;
...
void triggerSetup_4G63(void)
{
    ...
    if (currentStatus.initialisationComplete == false)
        {
            toothLastToothTime = micros();
        }  // Set a startup value here to avoid filter errors when starting. This MUST have the initial check to prevent
           // the fuel pump just staying on all the time
    ...
    triggerSecFilterTime =
        (int)(MICROS_PER_SEC / (MAX_RPM / 60U * 2U)) /
        2U;  // Same as above, but fixed at 2 teeth on the secondary input and divided by 2 (for cam speed)
    ...
    if ((curGap >= triggerFilterTime) || (currentStatus.startRevolutions == 0))
        {
            BIT_SET(decoderState,
                    BIT_DECODER_VALID_TRIGGER);  // Flag this pulse as being a valid trigger (ie that it passed filters)
            triggerFilterTime = curGap >> 2;     // This only applies during non-sync conditions. If there is sync then
            ...
            if ((toothCurrentCount == 1) ||
                (toothCurrentCount > triggerActualTeeth))  // Trigger is on CHANGE, hence 4 pulses = 1 crank rev (or 6
                                                           // pulses for 6 cylinders)
                {
                    toothCurrentCount = 1;  // Reset the counter
                    toothOneMinusOneTime = toothOneTime;
                    toothOneTime = curTime;
            ...
            if (currentStatus.hasSync == true)
                {
                    if (BIT_CHECK(currentStatus.engine, BIT_ENGINE_CRANK) && configPage4.ignCranklock &&
                        (currentStatus.startRevolutions >= configPage4.StgCycles))
                        {
                            if (configPage2.nCylinders == 4)
                                {
                                    // This operates in forced wasted spark mode during cranking to align with crank
                                    // teeth
                    ...
                    // Whilst this is an uneven tooth pattern, if the specific angle between the last 2 teeth is
                    // specified, 1st deriv prediction can be used
                    if ((configPage4.triggerFilter == 1) || (currentStatus.RPM < 1400))
                        {
                            // Lite filter
                            if ((toothCurrentCount == 1) || (toothCurrentCount == 3) || (toothCurrentCount == 5) ||
                                (toothCurrentCount == 7) || (toothCurrentCount == 9) || (toothCurrentCount == 11))
                                {
                                    if (configPage2.nCylinders == 4)
                                        {
                                            triggerToothAngle = 70;
                                            triggerFilterTime =
                                                curGap;  // Trigger filter is set to whatever time it took to do 70
                                                         // degrees (Next trigger is 110 degrees away)
                                        }
                                    else if (configPage2.nCylinders == 6)
                                        {
                                            triggerToothAngle = 70;
                                            triggerFilterTime =
                                                (curGap >> 2);  // Trigger filter is set to (70/4)=17.5=17 degrees (Next
                                                                // trigger is 50 degrees away).
                                        }
                                }
                            else
                                {
                                    if (configPage2.nCylinders == 4)
                                        {
                                            ...
                                            triggerFilterTime =
                                                (curGap * 3) >> 3;  // Trigger filter is set to (110*3)/8=41.25=41
                                                                    // degrees (Next trigger is 70 degrees away).
                                        }
                                    else if (configPage2.nCylinders == 6)
                                        {
                                            triggerToothAngle = 50;
                                            triggerFilterTime = curGap >> 1;  // Trigger filter is set to 25 degrees
                            ...
                            else
                                {
                                    if (configPage2.nCylinders == 4)
                                    ...
                                    else
                                        {
                                            triggerToothAngle = 50;
                                            triggerFilterTime =
                                                (curGap * 3) >> 2;  // Trigger filter is set to (50*3)/4=37.5=37 degrees
                                                                    // (Next trigger is 70 degrees away).
                                        }
                                }
                        }
                            ...
                            if ((toothCurrentCount == 1) || (toothCurrentCount == 3) || (toothCurrentCount == 5) ||
                                (toothCurrentCount == 7) || (toothCurrentCount == 9) || (toothCurrentCount == 11))
                                {
                                    ...
                                    if (configPage2.nCylinders == 4)
                                        {
                                            triggerFilterTime =
                                                (curGap * 11) >> 3;  // 96.26 degrees with a target of 110
                                        }
                                    else
                                        {
                                            triggerFilterTime = curGap >> 1;  // 35 degrees with a target of 50
                    ...
                    // EXPERIMENTAL!
                    // New ignition mode is ONLY available on 4g63 when the trigger angle is set to the stock value of
                    // 0.
    ...
    if ((curGap2 >= triggerSecFilterTime))  //|| (currentStatus.startRevolutions == 0) )
        {
            ...
            if ((currentStatus.hasSync == false))
                {
                    triggerFilterTime = 1500;  // If this is removed, can have trouble getting sync again after the
                                               // engine is turned off (but ECU not reset).
                    triggerSecFilterTime =
                        triggerSecFilterTime >>
                        1;  // Divide the secondary filter time by 2 again, making it 25%. Only needed when cranking
                    if (READ_PRI_TRIGGER() == true)
                        {
                            if (configPage2.nCylinders == 4)
                                {
            ...
            if ((currentStatus.RPM < currentStatus.crankRPM))
                {
                    ...
                    if ((toothLastToothTime == 0) || (toothLastMinusOneToothTime == 0))
                    ...
                    else
                        {
                            ...
                            toothTime = (toothLastToothTime -
                                         toothLastMinusOneToothTime);  // Note that trigger tooth angle changes between
                                                                       // 70 and 110 depending on the last tooth that
                                                                       // was seen (or 70/50 for 6 cylinders)
                            interrupts();
            ...
            else
                {
                    ...
                    MAX_STALL_TIME = revolutionTime
                                     << 1;  // Set the stall time to be twice the current RPM. This is a safe figure as
                                            // there should be no single revolution where this changes more than this
                    if (MAX_STALL_TIME < 366667UL)
                        {
                            MAX_STALL_TIME = 366667UL;
...
int getCrankAngle_4G63(void)
{
    ...
    if (currentStatus.hasSync == true)
        {
            // This is the current angle ATDC the engine is at. This is the last known position based on what tooth was
            // last 'seen'. It is only accurate to the resolution of the trigger wheel (Eg 36-1 is 10 degrees)
            unsigned long tempToothLastToothTime;
            int tempToothCurrentCount;
            // Grab some variables that are used in the trigger code and assign them to temp variables.
            noInterrupts();
            ...
            crankAngle = toothAngles[(tempToothCurrentCount - 1)] +
                         configPage4.triggerAngle;  // Perform a lookup of the fixed toothAngles array to find what the
                                                    // angle of the last tooth passed was.
...
Provided that the cam signal is used, this decoder simply counts the teeth and then looks their angles up against a
lookup table. The cam signal is used to determine tooth #1
    ...
    if (currentStatus.initialisationComplete == false)
        {
            ...
            toothLastToothTime = micros();
        }  // Set a startup value here to avoid filter errors when starting. This MUST have the init check to prevent
           // the fuel pump just staying on all the time
    BIT_CLEAR(decoderState, BIT_DECODER_2ND_DERIV);
    BIT_SET(decoderState, BIT_DECODER_IS_SEQUENTIAL);
    BIT_SET(decoderState, BIT_DECODER_TOOTH_ANG_CORRECT);
            ...
            if (toothCurrentCount == 0)
            ...
            else
                {
                    ...
                    triggerToothAngle =
                        ...
                        toothAngles[(toothCurrentCount - 2)];  // Calculate the last tooth gap in degrees
                }
...
{
    toothCurrentCount = 0;  // All we need to do is reset the tooth count back to zero, indicating that we're at the
                            // beginning of a new revolution
    revolutionOne = 1;  // Sequential revolution reset
}

uint16_t getRPM_24X(void) { return stdGetRPM(CRANK_SPEED); }
int getCrankAngle_24X(void)
{
    ...
    if (tempToothCurrentCount == 0)
        ...
        }  // This is the special case to handle when the 'last tooth' seen was the cam tooth. 0 is the angle at which
           // the crank tooth goes high (Within 360 degrees).
    else
        {
...
void triggerSetup_Jeep2000(void)
{
    ...
    MAX_STALL_TIME =
        ((MICROS_PER_DEG_1_RPM / 50U) *
         60U);  // Minimum 50rpm. (3333uS is the time per degree at 50rpm). Largest gap between teeth is 60 degrees.
    if (currentStatus.initialisationComplete == false)
        {
            ...
            if (curGap >= triggerFilterTime)
                {
                    if (toothCurrentCount == 0)
                        {
                            ...
                            currentStatus.hasSync = true;
                            currentStatus.startRevolutions++;  // Counter
                            triggerToothAngle = 60;  // There are groups of 4 pulses (Each 20 degrees apart), with each
                                                     // group being 60 degrees apart. Hence #1 is always 60
                        }
                    else
                        {
                            toothCurrentCount++;  // Increment the tooth counter
                    ...
                    BIT_SET(decoderState, BIT_DECODER_VALID_TRIGGER);  // Flag this pulse as being a valid trigger (ie
                                                                       // that it passed filters)
...
uint16_t getRPM_Jeep2000(void) { return stdGetRPM(CRANK_SPEED); }
int getCrankAngle_Jeep2000(void)
{
    ...
    if (toothCurrentCount == 0)
        ...
        }  // This is the special case to handle when the 'last tooth' seen was the cam tooth. Since  the tooth timings
           // were taken on the previous crank tooth, the previous crank tooth angle is used here, not cam angle.
    else
        {
            crankAngle = toothAngles[(tempToothCurrentCount - 1)] + configPage4.triggerAngle;
        }  // Perform a lookup of the fixed toothAngles array to find what the angle of the last tooth passed was.
 ...
 * only every 3rd crank tooth is used in calculating the crank angle. This effectively makes it a 45 tooth dual wheel
 * setup.
...
void triggerSetup_Audi135(void)
{
    ...
    triggerFilterTime =
        (unsigned long)(MICROS_PER_SEC /
                        (MAX_RPM / 60U * 135UL));  // Trigger filter time is the shortest possible time (in uS) that
                                                   // there can be between crank teeth (ie at max RPM). Any pulses that
                                                   // occur faster than this time will be discarded as noise
    triggerSecFilterTime =
        (int)(MICROS_PER_SEC / (MAX_RPM / 60U * 2U)) /
        2U;  // Same as above, but fixed at 2 teeth on the secondary input and divided by 2 (for cam speed)
    MAX_STALL_TIME =
                    ...
                    if (toothSystemCount >= 3)
                        {
                            ...
                            BIT_SET(decoderState, BIT_DECODER_VALID_TRIGGER);  // Flag this pulse as being a valid
...
uint16_t getRPM_Audi135(void) { return stdGetRPM(CRANK_SPEED); }
...
void triggerPri_HondaD17(void)
{
    ...
    if ((toothCurrentCount == 13) && (currentStatus.hasSync == true))
    ...
    else
        {
            // 13th tooth
            targetGap = (lastGap) >> 1;  // The target gap is set at half the last tooth gap
            if (curGap < targetGap)  // If the gap between this tooth and the last one is less than half of the previous
                                     // gap, then we are very likely at the magical 13th tooth
                {
                    toothCurrentCount = 0;
                    currentStatus.hasSync = true;
                }
                ...
                {
                    // The tooth times below don't get set on tooth 13(The magical 13th tooth should not be considered
                    // for any calculations that use those times)
                    toothLastMinusOneToothTime = toothLastToothTime;
                    toothLastToothTime = curTime;
                }
        }
...
{
    return;
}  // The 4+1 signal on the cam is yet to be supported. If this ever changes, update BIT_DECODER_HAS_SECONDARY in the
   // setup() function
uint16_t getRPM_HondaD17(void) { return stdGetRPM(CRANK_SPEED); }
int getCrankAngle_HondaD17(void)
{
    ...
    // Check if the last tooth seen was the reference tooth 13 (Number 0 here). All others can be calculated, but tooth
    // 3 has a unique angle
    int crankAngle;
    if (tempToothCurrentCount == 0)
        {
            crankAngle =
                (11 * triggerToothAngle) +
                configPage4.triggerAngle;  // if temptoothCurrentCount is 0, the last tooth seen was the 13th one. Based
                                           // on this, ignore the 13th tooth and use the 12th one as the last reference.
        }
    else
        {
            crankAngle = ((tempToothCurrentCount - 1) * triggerToothAngle) +
                         configPage4.triggerAngle;  // Number of teeth that have passed since tooth 1, multiplied by the
                                                    // angle each tooth represents, plus the angle that tooth 1 is ATDC.
                                                    // This gives accuracy only to the nearest tooth.
        }
    ...
    if ((curGap >= triggerFilterTime) || (currentStatus.startRevolutions == 0))
        {
            ...
            if (currentStatus.hasSync == true)
                {
                    ...
                    if ((configPage4.triggerFilter == 1) || (currentStatus.RPM < 1400))
                        {
                            ...
                            if ((toothCurrentCount == 1) || (toothCurrentCount == 3) || (toothCurrentCount == 5) ||
                                ...
                                }  // Trigger filter is set to whatever time it took to do 70 degrees (Next trigger is
                                   // 110 degrees away)
                            else
                                {
                                    triggerToothAngle = 110;
                                    triggerFilterTime = (curGap * 3) >> 3;
                                }  // Trigger filter is set to (110*3)/8=41.25=41 degrees (Next trigger is 70 degrees
                                   // away).
                        }
                    else if (configPage4.triggerFilter == 2)
                        {
                    ...
                    // EXPERIMENTAL!
                    // New ignition mode is ONLY available on 9905 when the trigger angle is set to the stock value of
                    // 0.
                    if ((configPage2.perToothIgn == true) && (configPage4.triggerAngle == 0) &&
            ...
            if ((currentStatus.RPM < (currentStatus.crankRPM + 30)) &&
                (configPage4.ignCranklock))  // The +30 here is a safety margin. When switching from fixed timing to
                                             // normal, there can be a situation where a pulse started when fixed and
                                             // ending when in normal mode causes problems. This prevents that.
                {
                    if ((toothCurrentCount == 1) || (toothCurrentCount == 5))
                        {
                            endCoil1Charge();
...
void triggerSec_Miata9905(void)
{
    ...
    if (BIT_CHECK(currentStatus.engine, BIT_ENGINE_CRANK) || (currentStatus.hasSync == false))
        {
            triggerFilterTime = 1500;  // If this is removed, can have trouble getting sync again after the engine is
                                       // turned off (but ECU not reset).
        }
    ...
    if ((currentStatus.RPM < currentStatus.crankRPM) && (currentStatus.hasSync == true))
        {
            if ((toothLastToothTime == 0) || (toothLastMinusOneToothTime == 0))
            ...
            else
                {
                    ...
                    tempToothAngle = triggerToothAngle;
                    toothTime = (toothLastToothTime -
                                 toothLastMinusOneToothTime);  // Note that trigger tooth angle changes between 70 and
                                                               // 110 depending on the last tooth that was seen
                    interrupts();
                    toothTime = toothTime * 36;
                    tempRPM = ((unsigned long)tempToothAngle * (MICROS_PER_MIN / 10U)) / toothTime;
                    SetRevolutionTime((10UL * toothTime) / tempToothAngle);
    ...
    else
        {
            tempRPM = stdGetRPM(CAM_SPEED);
            MAX_STALL_TIME = revolutionTime
                             << 1;  // Set the stall time to be twice the current RPM. This is a safe figure as there
                                    // should be no single revolution where this changes more than this
            if (MAX_STALL_TIME < 366667UL)
                {
                    MAX_STALL_TIME = 366667UL;
                }  // Check for 50rpm minimum
...
int getCamAngle_Miata9905(void)
{
    ...
    // All cam angles in in BTDC, so the actual advance angle is 370 - timeToAngleDegPerMicroSec(lastVVTtime) - <the
    // angle of the cam at 0 advance>
...
{
    triggerToothAngle = 108;  // The number of degrees that passes from tooth to tooth (primary). This is the maximum
                              // gap
    toothCurrentCount = 99;   // Fake tooth count represents no sync
    ...
    if (curGap >= triggerFilterTime)
        {
            ...
            toothCurrentCount++;
            if ((toothCurrentCount == 1) ||
                (toothCurrentCount == 5))  // Trigger is on CHANGE, hence 4 pulses = 1 crank rev
                {
                    toothCurrentCount = 1;  // Reset the counter
                    toothOneMinusOneTime = toothOneTime;
                    toothOneTime = curTime;
                    ...
                    if ((toothCurrentCount == 1) || (toothCurrentCount == 3))
                        {
                            ...
                            triggerFilterTime = curGap;
                        }  // Trigger filter is set to whatever time it took to do 72 degrees (Next trigger is 108
                           // degrees away)
                    else
                        {
...
void triggerSec_MazdaAU(void)
{
    ...
    if (currentStatus.hasSync == false)
        {
            // we find sync by looking for the 2 teeth that are close together. The next crank tooth after that is the
            // one we're looking for. For the sake of this decoder, the lone cam tooth will be designated #1
            if (secondaryToothCount == 2)
                {
                    toothCurrentCount = 1;
                    currentStatus.hasSync = true;
            ...
            else
                {
                    ...
                    targetGap = (lastGap) >> 1;  // The target gap is set at half the last tooth gap
                    if (curGap2 < targetGap)  // If the gap between this tooth and the last one is less than half of the
                                              // previous gap, then we are very likely at the extra (3rd) tooth on the
                                              // cam). This tooth is located at 421 crank degrees (aka 61 degrees) and
                                              // therefore the last crank tooth seen was number 1 (At 350 degrees)
                        {
            ...
            if (currentStatus.RPM < currentStatus.crankRPM)
                {
                    ...
                    tempToothAngle = triggerToothAngle;
                    SetRevolutionTime(
                        36 * (toothLastToothTime -
                              toothLastMinusOneToothTime));  // Note that trigger tooth angle changes between 72 and 108
                                                             // depending on the last tooth that was seen
                    interrupts();
...
/** Non-360 Dual wheel with 2 wheels located either both on the crank or with the primary on the crank and the secondary
on the cam. There can be no missing teeth on the primary wheel.
...
{
    triggerToothAngle =
        (360U * configPage4.TrigAngMul) / configPage4.triggerTeeth;  // The number of degrees that passes from tooth to
                                                                     // tooth multiplied by the additional multiplier
    toothCurrentCount = 255;  // Default value
    triggerFilterTime =
        (MICROS_PER_SEC /
         (MAX_RPM / 60U * configPage4.triggerTeeth));  // Trigger filter time is the shortest possible time (in uS) that
...
int getCrankAngle_non360(void)
{
    ...
    // Number of teeth that have passed since tooth 1, multiplied by the angle each tooth represents, plus the angle
    // that tooth 1 is ATDC. This gives accuracy only to the nearest tooth.
    int crankAngle = (tempToothCurrentCount - 1) * triggerToothAngle;
    crankAngle = (crankAngle / configPage4.TrigAngMul) +
                 configPage4.triggerAngle;  // Have to divide by the multiplier to get back to actual crank angle.
...
void triggerSetup_Nissan360(void)
{
    triggerFilterTime =
        (MICROS_PER_SEC /
         (MAX_RPM / 60U *
          360UL));  // Trigger filter time is the shortest possible time (in uS) that there can be between crank teeth
                    // (ie at max RPM). Any pulses that occur faster than this time will be discarded as noise
    triggerSecFilterTime =
        (int)(MICROS_PER_SEC / (MAX_RPM / 60U * 2U)) /
        2U;  // Same as above, but fixed at 2 teeth on the secondary input and divided by 2 (for cam speed)
...
void triggerSec_Nissan360(void)
{
    ...
    if ((secondaryToothCount == 0) || (READ_SEC_TRIGGER() == trigEdge))
    ...
    else
        {
            ...
            byte secondaryDuration =
                toothCurrentCount -
                secondaryToothCount;  // How many primary teeth have passed during the duration of this secondary window
                    ...
                    if (configPage2.nCylinders == 4)
                        {
                            ...
                            if ((secondaryDuration >= 15) &&
                                (secondaryDuration <= 17))  // Duration of window = 16 primary teeth
                                {
                                    toothCurrentCount =
                                        16;  // End of first window (The longest) occurs 16 teeth after TDC
                                    currentStatus.hasSync = true;
                                }
                            else if ((secondaryDuration >= 11) &&
                                     (secondaryDuration <= 13))  // Duration of window = 12 primary teeth
                                {
                                    toothCurrentCount = 102;  // End of second window is after 90+12 primary teeth
                                    currentStatus.hasSync = true;
                                }
                            else if ((secondaryDuration >= 7) &&
                                     (secondaryDuration <= 9))  // Duration of window = 8 primary teeth
                                {
                                    toothCurrentCount = 188;  // End of third window is after 90+90+8 primary teeth
                                    currentStatus.hasSync = true;
                                }
                            else if ((secondaryDuration >= 3) &&
                                     (secondaryDuration <= 5))  // Duration of window = 4 primary teeth
                                {
                    ...
                    else if (configPage2.nCylinders == 8)
                        {
                            ...
                            if ((secondaryDuration >= 6) &&
                                (secondaryDuration <= 8))  // Duration of window = 16 primary teeth
                                {
                                    toothCurrentCount = 56;  // End of the shortest of the individual windows. Occurs at
                                                             // 102 crank degrees.
                                    currentStatus.hasSync = true;
                                }
                        }
            ...
            if (currentStatus.startRevolutions < 2)
                {
                    ...
                    SetRevolutionTime(
                        ...
                        180);  // Each tooth covers 2 crank degrees, so multiply by 180 to get a full revolution time.
                    interrupts();
                }
            else
                {
                    ...
                    SetRevolutionTime((toothOneTime - toothOneMinusOneTime) >>
                                      1);  // The time in uS that one revolution would take at current speed (The time
                                           // tooth 1 was last seen, minus the time it was seen prior to that)
                    interrupts();
                }
            tempRPM =
                RpmFromRevolutionTimeUs(revolutionTime);  // Calc RPM based on last full revolution time (Faster as /)
            MAX_STALL_TIME = revolutionTime
                             << 1;  // Set the stall time to be twice the current RPM. This is a safe figure as there
                                    // should be no single revolution where this changes more than this
        }
...
{
    // As each tooth represents 2 crank degrees, we only need to determine whether we're more or less than halfway
    // between teeth to know whether to add another 1 degrees
    int crankAngle = 0;
...
/** Subaru 6/7 Trigger pattern decoder for 6 tooth (irregularly spaced) crank and 7 tooth (also fairly irregular) cam
wheels (eg late 90's Impreza 2.2). This seems to be present in late 90's Subaru. In 2001 Subaru moved to 36-2-2-2 (See:
http://www.vems.hu/wiki/index.php?page=InputTrigger%2FSubaruTrigger ).
...
void triggerPri_Subaru67(void)
{
    ...
    toothCurrentCount++;  // Increment the tooth counter
    toothSystemCount++;   // Used to count the number of primary pulses that have occurred since the last secondary. Is
                         // part of the noise filtering system.
    BIT_SET(decoderState,
            BIT_DECODER_VALID_TRIGGER);  // Flag this pulse as being a valid trigger (ie that it passed filters)
            ...
            case 1:
                ...
                if (toothCurrentCount == 5 || toothCurrentCount == 11)
                ...
                else
                    {
                        currentStatus.hasSync = false;
                        currentStatus.syncLossCounter++;
                        toothCurrentCount = 5;  // we don't know if its 5 or 11, but we'll be right 50% of the time and
            ...
            if (toothSystemCount > 3)
                {
                    toothSystemCount = 0;
                    secondaryToothCount = 1;
                    currentStatus.hasSync = false;  // impossible to have more than 3 crank teeth between cam teeth -
...
void triggerSetup_Daihatsu(void)
{
    ...
    MAX_STALL_TIME = ((MICROS_PER_DEG_1_RPM / 90U) * triggerToothAngle) *
                     4U;  // Minimum 90rpm. (1851uS is the time per degree at 90rpm). This uses 90rpm rather than 50rpm
                          // due to the potentially very high stall time on a 4 cylinder if we wait that long.
                        ...
                        if (configPage2.nCylinders == 3)
                            {
                                targetTime = (toothLastToothTime - toothLastMinusOneToothTime) /
                                             4;  // Teeth are 240 degrees apart for 3 cylinder. 240/4 = 60
                            }
                        else
                            {
                                targetTime = ((toothLastToothTime - toothLastMinusOneToothTime) * 3) /
                                             8;  // Teeth are 180 degrees apart for 4 cylinder. (180*3)/8 = 67
                            }
            ...
            if (currentStatus.hasSync == true)
                {
                    if (toothCurrentCount == 2)
                        ...
                        {
                            noInterrupts();
                            SetRevolutionTime((toothLastToothTime - toothLastMinusOneToothTime) *
                                              (triggerActualTeeth - 1));
                            interrupts();
...
int getCrankAngle_Daihatsu(void)
{
    ...
    crankAngle =
        toothAngles[tempToothCurrentCount - 1] + configPage4.triggerAngle;  // Crank angle of the last tooth seen
...
void triggerPri_Harley(void)
{
    ...
    if (curGap > triggerFilterTime)
        {
            if (READ_PRI_TRIGGER() ==
                HIGH)  // Has to be the same as in main() trigger-attach, for readability we do it this way.
                {
                    BIT_SET(decoderState, BIT_DECODER_VALID_TRIGGER);  // Flag this pulse as being a valid trigger (ie
                                                                       // that it passed filters)
                    ...
                    if ((toothLastToothTime == 0) || (toothLastMinusOneToothTime == 0))
                    ...
                    else
                        {
                            ...
                            /* High-res mode
                              ...
                              else { tempToothAngle = toothAngles[toothCurrentCount-1] -
                              toothAngles[toothCurrentCount-2]; }
                            */
                            SetRevolutionTime(toothOneTime -
                                              toothOneMinusOneTime);  // The time in uS that one revolution would take
                                                                      // at current speed (The time tooth 1 was last
                                                                      // seen, minus the time it was seen prior to that)
                            toothTime =
                                (toothLastToothTime -
                                 toothLastMinusOneToothTime);  // Note that trigger tooth angle changes between 129 and
...
int getCrankAngle_Harley(void)
{
    ...
    if ((tempToothCurrentCount == 1) || (tempToothCurrentCount == 3))
        {
            crankAngle = 0 + configPage4.triggerAngle;  // Number of teeth that have passed since tooth 1, multiplied by
                                                        // the angle each tooth represents, plus the angle that tooth 1
                                                        // is ATDC. This gives accuracy only to the nearest tooth.
        }
    else
        {
 ...
 * Note: This decoder supports both the H4 version (13-missing-16-missing-1-missing) and the H6 version of 36-2-2-2
 * (19-missing-10-missing-1-missing). The decoder checks which pattern is selected in order to determine the tooth
 * number Note: www.thefactoryfiveforum.com/attachment.php?attachmentid=34279&d=1412431418
 *
...
void triggerSetup_ThirtySixMinus222(void)
{
    ...
    triggerActualTeeth = 30;  // The number of physical teeth on the wheel. Doing this here saves us a calculation each
                              // time in the interrupt
    triggerFilterTime =
        (int)(MICROS_PER_SEC /
              (MAX_RPM / 60U *
               36));  // Trigger filter time is the shortest possible time (in uS) that there can be between crank teeth
                      // (ie at max RPM). Any pulses that occur faster than this time will be discarded as noise
    BIT_CLEAR(decoderState, BIT_DECODER_2ND_DERIV);
    BIT_CLEAR(decoderState, BIT_DECODER_IS_SEQUENTIAL);
    BIT_SET(decoderState, BIT_DECODER_HAS_SECONDARY);
    ...
    toothOneMinusOneTime = 0;
    MAX_STALL_TIME = ((MICROS_PER_DEG_1_RPM / 50U) * triggerToothAngle *
                      2U);  // Minimum 50rpm. (3333uS is the time per degree at 50rpm)
}
...
void triggerPri_ThirtySixMinus222(void)
{
    ...
    if (curGap >= triggerFilterTime)  // Pulses should never be less than triggerFilterTime, so if they are it means a
                                      // false trigger. (A 36-1 wheel at 8000pm will have triggers approx. every 200uS)
        {
            ...
            // Begin the missing tooth detection
            // If the time between the current tooth and the last is greater than 2x the time between the last tooth and
            // the tooth before that, we make the assertion that we must be at the first tooth after a gap
            // toothSystemCount is used to keep track of which missed tooth we're on. It will be set to 1 if that last
            // tooth seen was the middle one in the -2-2 area. At all other times it will be 0
                ...
                {
                    {
                        if (toothSystemCount == 1)
                            ...
                            {
                                // We've seen a missing tooth set, but do not yet know whether it is the single one or
                                // the double one.
                                toothSystemCount = 1;
                                toothCurrentCount++;
                                toothCurrentCount++;  // Accurately reflect the actual tooth count, including the
                                                      // skipped ones
                            }
                        BIT_CLEAR(decoderState,
                                  BIT_DECODER_TOOTH_ANG_CORRECT);  // The tooth angle is double at this point
                        triggerFilterTime = 0;  // This is used to prevent a condition where serious intermittent
                                                // signals (Eg someone furiously plugging the sensor wire in and out)
...
uint16_t getRPM_ThirtySixMinus222(void)
{
    ...
    if (currentStatus.RPM < currentStatus.crankRPM)
        {
            if ((configPage2.nCylinders == 4) && (toothCurrentCount != 19) && (toothCurrentCount != 16) &&
                (toothCurrentCount != 34) && (BIT_CHECK(decoderState, BIT_DECODER_TOOTH_ANG_CORRECT)))
                {
                    tempRPM = crankingGetRPM(36, CRANK_SPEED);
                }
            else if ((configPage2.nCylinders == 6) && (toothCurrentCount != 9) && (toothCurrentCount != 12) &&
                     (toothCurrentCount != 33) && (BIT_CHECK(decoderState, BIT_DECODER_TOOTH_ANG_CORRECT)))
                {
                    tempRPM = crankingGetRPM(36, CRANK_SPEED);
                }
            else
...
/** 36-2-1 / Mistsubishi 4B11 - A crank based trigger with a nominal 36 teeth, but with 1 single and 1 double missing
 * tooth.
...
{
    triggerToothAngle = 10;   // The number of degrees that passes from tooth to tooth
    triggerActualTeeth = 33;  // The number of physical teeth on the wheel. Doing this here saves us a calculation each
                              // time in the interrupt. Not Used
    triggerFilterTime =
        (MICROS_PER_SEC /
         (MAX_RPM / 60U *
          36));  // Trigger filter time is the shortest possible time (in uS) that there can be between crank teeth (ie
                 // at max RPM). Any pulses that occur faster than this time will be discarded as noise
    BIT_CLEAR(decoderState, BIT_DECODER_2ND_DERIV);
    BIT_CLEAR(decoderState, BIT_DECODER_IS_SEQUENTIAL);
    BIT_SET(decoderState, BIT_DECODER_HAS_SECONDARY);
    checkSyncToothCount = (configPage4.triggerTeeth) >> 1;  // 50% of the total teeth.
...
void triggerPri_ThirtySixMinus21(void)
{
    ...
    if (curGap >= triggerFilterTime)  // Pulses should never be less than triggerFilterTime, so if they are it means a
                                      // false trigger. (A 36-1 wheel at 8000pm will have triggers approx. every 200uS)
        {
            ...
            targetGap2 =
                (3 * (toothLastToothTime -
                      toothLastMinusOneToothTime));  // Multiply by 3 (Checks for a gap 3x greater than the last one)
            targetGap = targetGap2 >> 1;  // Multiply by 1.5 (Checks for a gap 1.5x greater than the last one) (Uses
                                          // bitshift to divide by 2 as in the missing tooth decoder)
            ...
            if ((curGap > targetGap))
                {
                    ...
                    triggerFilterTime = 0;  // This is used to prevent a condition where serious intermittent signals
                                            // (Eg someone furiously plugging the sensor wire in and out) can leave the
                                            // filter in an unrecoverable state
                }
        }
    else
        {
 ...
 * Sync is determined by watching for a falling edge on the secondary signal and checking if the primary signal is high
 * then. https://github.com/noisymime/speeduino/issues/133
...
uint16_t getRPM_420a(void)
{
    ...
    if (currentStatus.RPM < currentStatus.crankRPM)
        {
            // Possibly look at doing special handling for cranking in the future, but for now just use the standard
            // method
            tempRPM = stdGetRPM(CAM_SPEED);
        }
    else
...
int getCrankAngle_420a(void)
{
    ...
    int crankAngle;
    crankAngle = toothAngles[(tempToothCurrentCount - 1)] +
                 configPage4.triggerAngle;  // Perform a lookup of the fixed toothAngles array to find what the angle of
    ...
    if (curGap2 >= triggerSecFilterTime)
        {
            ...
            if ((secondaryToothCount == 2) && (checkSyncToothCount == 3))
                    ...
                    if (currentStatus.hasSync == false)
                        {
                            toothLastToothTime = micros();
                            toothLastMinusOneToothTime =
                                micros() - 1500000;  // Fixes RPM at 10rpm until a full revolution has taken place
            ...
            else if ((currentStatus.hasSync == false) && (toothCurrentCount >= 3) && (secondaryToothCount == 0))
                {
                    ...
                    currentStatus.hasSync = true;
                }  // First start, between gaps on CAM pulses have 2 teeth, sync on first CAM pulse if seen 3 teeth or
                   // more
            else
                {
...
void triggerSetup_FordST170(void)
{
    ...
    triggerSecFilterTime = MICROS_PER_MIN / MAX_RPM / 8U /
                           2U;  // Cam pattern is 8-3, so 2 nearest teeth are 90 deg crank angle apart. Cam can be
                                // advanced by 60 deg, so going from fully retarded to fully advanced closes the gap to
                                // 30 deg. Zetec cam pulleys aren't keyed from factory, so I subtracted additional 10
                                // deg to avoid filter to be too aggressive. And there you have it 720/20=36.
    ...
    toothOneMinusOneTime = 0;
    MAX_STALL_TIME = ((MICROS_PER_DEG_1_RPM / 50U) * triggerToothAngle *
                      (1U + 1U));  // Minimum 50rpm. (3333uS is the time per degree at 50rpm)
#ifdef USE_LIBDIVIDE
    divTriggerToothAngle = libdivide::libdivide_s16_gen(triggerToothAngle);
#endif
}
...
void triggerSec_FordST170(void)
{
    ...
    if (curGap2 >= triggerSecFilterTime)
        {
            targetGap2 = (3 * (toothLastSecToothTime - toothLastMinusOneSecToothTime)) >>
                         1;  // If the time between the current tooth and the last is greater than 1.5x the time between
                             // the last tooth and the tooth before that, we make the assertion that we must be at the
                             // first tooth after the gap
            toothLastMinusOneSecToothTime = toothLastSecToothTime;
            if ((curGap2 >= targetGap2) || (secondaryToothCount == 5))
                {
                    secondaryToothCount = 1;
                    revolutionOne = 1;         // Sequential revolution reset
                    triggerSecFilterTime = 0;  // This is used to prevent a condition where serious intermittent signals
                                               // (Eg someone furiously plugging the sensor wire in and out) can leave
                                               // the filter in an unrecoverable state
                }
            else
                {
                    triggerSecFilterTime = curGap2 >> 2;  // Set filter at 25% of the current speed. Filter can only be
                                                          // recalculated for the regular teeth, not the missing one.
                    secondaryToothCount++;
                }
    ...
    triggerFilterTime =
        (MICROS_PER_SEC /
         (MAX_RPM / 60U * configPage4.triggerTeeth));  // Trigger filter time is the shortest possible time (in uS) that
                                                       ...
                                                       // that occur faster than this time will be discarded as noise
    triggerSecFilterTime =
        (MICROS_PER_SEC / (MAX_RPM / 60U * 2U));  // Same as above, but fixed at 2 teeth on the secondary input
    BIT_CLEAR(decoderState, BIT_DECODER_2ND_DERIV);
    BIT_SET(decoderState, BIT_DECODER_IS_SEQUENTIAL);
    BIT_SET(decoderState, BIT_DECODER_TOOTH_ANG_CORRECT);  // This is always true for this pattern
    BIT_SET(decoderState, BIT_DECODER_HAS_SECONDARY);
...
void triggerSetup_NGC(void)
{
    ...
    triggerToothAngle = 10;         // The number of degrees that passes from tooth to tooth
    triggerFilterTime = MICROS_PER_SEC / (MAX_RPM / 60U) /
                        (360U / triggerToothAngle);  // Trigger filter time is the shortest possible time (in uS) that
                                                     // there can be between crank teeth (ie at max RPM). Any pulses
                                                     // that occur faster than this time will be discarded as noise
    toothCurrentCount = 0;
    toothOneTime = 0;
    ...
    if (configPage2.nCylinders == 4)
        {
            triggerSecFilterTime = (MICROS_PER_SEC / (MAX_RPM / 60U) / (360U / 36U) *
                                    2U);  // Two nearest edges are 36 degrees apart. Multiply by 2 for half cam speed.
        }
    else
        {
            triggerSecFilterTime = (MICROS_PER_SEC / (MAX_RPM / 60U) / (360U / 21U) *
                                    2U);  // Two nearest edges are 21 degrees apart. Multiply by 2 for half cam speed.
        }
    secondaryToothCount = 0;
    toothSystemCount = 0;
    ...
    if (curGap >= triggerFilterTime)  // Pulses should never be less than triggerFilterTime, so if they are it means a
                                      // false trigger.
        {
            ...
            if (toothLastToothTime > 0 && toothLastMinusOneToothTime > 0)
                {  // Make sure we haven't enough tooth information to calculate missing tooth length
                    ...
                    if (toothCurrentCount == 17 || toothCurrentCount == 35 ||
                        (currentStatus.hasSync == false &&
                         BIT_CHECK(currentStatus.status3, BIT_STATUS3_HALFSYNC) == false))
                        {
                            // If the time between the current tooth and the last is greater than 2x the time between
                            // the last tooth and the tooth before that, we make the assertion that we must be at the
                            // first tooth after the gap
                            if (curGap > ((toothLastToothTime - toothLastMinusOneToothTime) * 2))
                                {
                                    ...
                                    triggerFilterTime =
                                        0;  // This is used to prevent a condition where serious intermittent signals
                                            // (Eg someone furiously plugging the sensor wire in and out) can leave the
                                            // filter in an unrecoverable state
                                    BIT_CLEAR(
                                        decoderState,
                                    ...
                                    // Figure out the polarity of the missing tooth by comparing how far ago the last
                                    // tooth rose
                                    if ((toothLastToothRisingTime - toothLastToothTime) <
                                        (curTime - toothLastToothRisingTime))
                                        {
                                    ...
                                    // If Sequential fuel or ignition is in use, further checks are needed before
                                    // determining sync
                                    if ((configPage4.sparkMode == IGN_MODE_SEQUENTIAL) ||
                                        (configPage2.injLayout == INJ_SEQUENTIAL))
                                        {
                                            // Verify the tooth counters are valid and use this to determine current
                                            // revolution
                                            if ((configPage2.nCylinders == 4 &&
                                                 ((toothCurrentCount == 1 &&
                                                   (secondaryToothCount == 1 || secondaryToothCount == 2)) ||
                                                  (toothCurrentCount == 19 && secondaryToothCount == 4))) ||
                                                (configPage2.nCylinders == 6 &&
                                                 ((toothCurrentCount == 1 &&
                                                   (toothSystemCount == 1 || toothSystemCount == 2)) ||
                                                  (toothCurrentCount == 19 &&
                                                   (toothSystemCount == 2 || toothSystemCount == 3)))) ||
                                                (configPage2.nCylinders == 8 &&
                                                 ((toothCurrentCount == 1 &&
                                                   (toothSystemCount == 1 || toothSystemCount == 2)) ||
                                                  (toothCurrentCount == 19 &&
                                                   (toothSystemCount == 3 || toothSystemCount == 4)))))
                                                {
                                                    revolutionOne = false;
                                                    currentStatus.hasSync = true;
                                                    BIT_CLEAR(currentStatus.status3,
                                                      ...
                                                      ((toothCurrentCount == 1 && secondaryToothCount == 5) ||
                                                       (toothCurrentCount == 19 && secondaryToothCount == 7))) ||
                                                     (configPage2.nCylinders == 6 &&
                                                      ((toothCurrentCount == 1 &&
                                                        (toothSystemCount == 4 || toothSystemCount == 5)) ||
                                                       (toothCurrentCount == 19 &&
                                                        (toothSystemCount == 5 || toothSystemCount == 6)))) ||
                                                     (configPage2.nCylinders == 8 &&
                                                      ((toothCurrentCount == 1 &&
                                                        (toothSystemCount == 5 || toothSystemCount == 6)) ||
                                                       (toothCurrentCount == 19 &&
                                                        (toothSystemCount == 7 || toothSystemCount == 8)))))
                                                {
                                                    revolutionOne = true;
                                                    currentStatus.hasSync = true;
                                                    BIT_CLEAR(currentStatus.status3,
                                            ...
                                            // If tooth counters are not valid, set half sync bit
                                            else
                                                {
                                                    ...
                                                    BIT_SET(
                                                        currentStatus.status3,
                                                        BIT_STATUS3_HALFSYNC);  // If there is primary trigger but no
                                                                                // secondary we only have half sync.
                                                }
                                        }
                                    else
                                        {
                                ...
                                {
                                    // If we have found a missing tooth and don't get the next one at the correct tooth
                                    // we end up here -> Resync
                                    if (currentStatus.hasSync == true)
                                        {
                                            currentStatus.syncLossCounter++;
                                        }
            ...
            if (isMissingTooth == true)
                {  // If we have a missing tooth, copy the gap from the previous tooth as that is the correct normal
                   // tooth length
    ...
    if (curGap2 > triggerSecFilterTime)
        {
            if (toothLastSecToothTime > 0 &&
                toothLastMinusOneSecToothTime >
                    0)  // Make sure we have enough tooth information to calculate tooth lengths
                {
                    if (secondaryToothCount > 0)
                        {
                            secondaryToothCount++;
                    ...
                    if (curGap2 >= ((3 * (toothLastSecToothTime - toothLastMinusOneSecToothTime)) >>
                                    1))  // Check if we have a bigger gap, that is a long tooth
                        {
                            // Check long tooth polarity
                            if ((toothLastSecToothRisingTime - toothLastSecToothTime) <
                                (curTime2 - toothLastSecToothRisingTime))
                                {
                                    ...
                                    if (secondaryToothCount == 0 || secondaryToothCount == 8)
                                        ...
                                        {
                                            secondaryToothCount = 0;
                                        }  // Any other number of teeth seen means we missed something or something
                                           // extra was seen so attempt resync.
                                }
                            else
                                {
                                    // Just passed the first tooth after the LOW missing tooth
                            ...
                            triggerSecFilterTime = 0;  // This is used to prevent a condition where serious intermittent
                                                       // signals (Eg someone furiously plugging the sensor wire in and
                                                       // out) can leave the filter in an unrecoverable state
                        }
                    else if (secondaryToothCount > 0)
                        {
                            triggerSecFilterTime =
                                curGap2 >> 2;  // Set filter at 25% of the current speed. Filter can only be recalc'd
...
void triggerSec_NGC68(void)
{
    ...
    if (curGap2 > triggerSecFilterTime)
        {
            if (toothLastSecToothTime > 0 && toothLastToothTime > 0 &&
                toothLastMinusOneToothTime >
                    0)  // Make sure we have enough tooth information to calculate tooth lengths
                {
                    /* Cam wheel can have a single tooth in a group which can screw up the "targetgap" calculations
                       Instead use primary wheel tooth gap as comparison as those values are always correct. 2.1 primary
                       teeth are the same duration as one secondary tooth. */
                    if (curGap2 >=
                        (3 * (toothLastToothTime -
                              toothLastMinusOneToothTime)))  // Check if we have a bigger gap, that is missing teeth
                        {
                            // toothSystemCount > 0 means we have cam sync and identifies which group we have synced
                            // with toothAngles is reused to store the cam pattern
                            if (secondaryToothCount > 0 && secondaryToothLastCount > 0)
                                {  // Only check for cam sync if we have actually detected two groups and can get cam
                                   // sync
                                    if (toothSystemCount > 0 &&
                                        ...
                                        {  // Check for a pair of matching groups which tells us which group we are at,
                                           // this should only happen when we don't have cam sync
                                            toothSystemCount =
                                                0;  // We either haven't got cam sync yet or we lost cam sync
...
Within the decoder code, the sync tooth is referred to as tooth #1. Derived from Harley and made to work on the Yamah
Vmax. Trigger is based on 'CHANGE' so we get a signal on the up and downward edges of the lobe. This is required to
identify the wide lobe.
...
void triggerSetup_Vmax(void)
{
    ...
    if (currentStatus.initialisationComplete == false)
        {
            toothLastToothTime = micros();
        }  // Set a startup value here to avoid filter errors when starting. This MUST have the initi check to prevent
           // the fuel pump just staying on all the time
    triggerFilterTime = 1500;
    BIT_SET(decoderState,
            BIT_DECODER_VALID_TRIGGER);  // We must start with a valid trigger or we cannot start measuring the lobe
                                         // width. We only have a false trigger on the lobe up event when it doesn't
                                         // pass the filter. Then, the lobe width will also not be beasured.
...
// We measure the width of a lobe so on the end of a lobe, but want to trigger on the beginning. Variable
// toothCurrentCount tracks the downward events, and secondaryToothCount updates on the upward events. Ideally, it
// should be the other way round but the engine stall routine resets secondaryToothCount, so it would not sync again
// after an engine stall.

void triggerPri_Vmax(void)
{
    curTime = micros();
    if (READ_PRI_TRIGGER() == primaryTriggerEdge)
        {  // Forwarded from the config page to setup the primary trigger edge (rising or falling). Inverting
           // VR-conditioners require FALLING, non-inverting VR-conditioners require RISING in the Trigger edge setup.
            curGap2 = curTime;
            curGap = curTime - toothLastToothTime;
            if ((curGap >= triggerFilterTime))
                {
                    ...
                    if (toothCurrentCount > 0)  // We have sync based on the tooth width.
                        {
                            ...
                            if (toothCurrentCount == 1)
                                {
                                    secondaryToothCount = 1;
                                    triggerToothAngle = 70;  // Has to be equal to Angle Routine, and describe the delta
                                                             // between two teeth.
                                    toothOneMinusOneTime = toothOneTime;
                                    toothOneTime = curTime;
                                    currentStatus.hasSync = true;
                                    // setFilter((curGap/1.75));//Angle to this tooth is 70, next is in 40,
                                    // compensating.
                                    setFilter(
                                        ((curGap * 4) / 7));  // Angle to this tooth is 70, next is in 40, compensating.
                                    currentStatus.startRevolutions++;  // Counter
                                }
                            else if (toothCurrentCount == 2)
                                {
                                    ...
                                    // setFilter((curGap*1.75));//Angle to this tooth is 40, next is in 70,
                                    // compensating.
                                    setFilter(
                                        ((curGap * 7) / 4));  // Angle to this tooth is 40, next is in 70, compensating.
                                }
                            else if (toothCurrentCount == 3)
                                {
                                    ...
                                    triggerToothAngle = 70;
                                    setFilter(
                                        curGap);  // Angle to this tooth is 70, next is in 70. No need to compensate.
                                }
                            else if (toothCurrentCount == 4)
                                {
    ...
    else if (BIT_CHECK(decoderState,
                       BIT_DECODER_VALID_TRIGGER))  // Inverted due to vr conditioner. So this is the falling lobe. We
                                                    // only process if there was a valid trigger.
        {
            ...
            if (curGapLocal > (lastGap * 2))
                {  // Small lobe is 5 degrees, big lobe is 45 degrees. So this should be the wide lobe.
                    if (toothCurrentCount == 0 || toothCurrentCount == 6)
                        {  // Wide should be seen with toothCurrentCount = 0, when there is no sync yet, or
                           // toothCurrentCount = 6 when we have done a full revolution.
                            currentStatus.hasSync = true;
                        }
                    else
        ...
        {
            BIT_SET(decoderState,
                    BIT_DECODER_VALID_TRIGGER);  // We reset this every time to ensure we only filter when needed.
        }
}
 ...
 * Renix trigger wheel doesn't decode into 360 degrees nicely (360/44 = 8.18 degrees or 360/66 = 5.454545). Speeduino
 * can't handle any teeth that have a decimal point. Solution is to count teeth, every 11 teeth = a proper angle. For 66
 * tooth decoder its 60 degrees per 11 teeth, for 44 tooth decoder its 90 degrees per 11 teeth. This means the system
 * sees 4 teeth on the 44 tooth wheel and 6 teeth on the 66 tooth wheel. Double missing tooth in the pattern is actually
 * a large tooth and a large gap. If the trigger is set to rising you'll see the start of the large tooth then the gap.
 * If its not set to rising the code won't work due to seeing two gaps
 *
 *
...
void triggerSetup_Renix(void)
{
    if (configPage2.nCylinders == 4)
        {
            triggerToothAngle = 90;  // The number of degrees that passes from tooth to tooth (primary) this changes
                                     // between 41 and 49 degrees
            configPage4.triggerTeeth = 4;  // wheel has 44 teeth but we use these to work out which tooth angle to use,
            ...
            triggerFilterTime =
                (MICROS_PER_SEC /
                 (MAX_RPM / 60U * 44U));  // Trigger filter time is the shortest possible time (in uS) that there can be
    ...
    else if (configPage2.nCylinders == 6)
        {
            ...
            configPage4.triggerTeeth = 6;  // wheel has 44 teeth but we use these to work out which tooth angle to use,
            ...
            triggerFilterTime =
                (MICROS_PER_SEC /
                 (MAX_RPM / 60U * 66U));  // Trigger filter time is the shortest possible time (in uS) that there can be
                                          // between crank teeth (ie at max RPM). Any pulses that occur faster than this
                                          // time will be discarded as noise
        }

    MAX_STALL_TIME = ((MICROS_PER_DEG_1_RPM / 50U) *
                      triggerToothAngle);  // Minimum 50rpm. (3333uS is the time per degree at 50rpm). Largest gap
                                           // between teeth is 90 or 60 degrees depending on decoder.
    BIT_CLEAR(decoderState, BIT_DECODER_HAS_SECONDARY);
...
// variables used to help calculate gap on the physical 44 or 66 teeth we're pretending don't exist in most of the
// speeduino code reusing existing variables to save storage space as these aren't used in the code for their original
// purpose.
    ...
    if (curGap >= triggerFilterTime)
        {
            ...
            if (toothSystemCount == 12 ||
                toothLastToothTime ==
                    0)  // toothLastToothTime used to ensure we set the value so the code that handles the fuel pump in
                        // speeduino.ini has a value to use once the engine is running.
                {
                    toothCurrentCount++;

                    if ((configPage2.nCylinders == 6 &&
                         toothCurrentCount == 7) ||  // 6 Pretend teeth on the 66 tooth wheel, if get to severn rotate
                                                     // round back to first tooth
                        (configPage2.nCylinders == 4 &&
                         toothCurrentCount == 5))  // 4 Pretend teeth on the 44 tooth wheel, if get to five rotate round
                                                   // back to first tooth
                        {
                            toothOneMinusOneTime = toothOneTime;
                            toothOneTime = curTime;
...
void triggerSetEndTeeth_Renix(void)
{
    ...
    // Temp variables are used here to avoid potential issues if a trigger interrupt occurs part way through this
    // function
...
{
    for (toothOneTime = 0; toothOneTime < 10;
         toothOneTime++)  // repurpose variable temporarily to help clear ToothAngles.
        {
    ...
    triggerFilterTime =
        (MICROS_PER_SEC /
         (MAX_RPM / 60U *
          36U));  // Trigger filter time is the shortest possible time (in uS) that there can be between crank teeth (ie
                  // at max RPM). Any pulses that occur faster than this time will be discarded as noise
    ...
    triggerToothAngle =
        360 /
        configPage4.triggerTeeth;  // The number of degrees that passes from tooth to tooth 360 / 36 theortical teeth
    triggerActualTeeth = 36;  // The number of physical teeth on the wheel. Need to fix now so we can identify the wheel
                              // on the first rotation and not risk a  type 1 wheel not being spotted
    toothLastMinusOneToothTime = 0;
    ...
    if (curGap >= triggerFilterTime)  // Pulses should never be less than triggerFilterTime, so if they are it means a
                                      // false trigger. (A 36-1 wheel at 8000pm will have triggers approx. every 200uS)
        {
            if ((toothLastToothTime > 0) &&
                (toothLastMinusOneToothTime > 0))  // have we seen more than 1 tooth so we start processing
                {
                    // Begin the missing tooth detection
                    targetGap = (3 * (toothLastToothTime - toothLastMinusOneToothTime)) >>
                                1;  // Multiply by 1.5 (Checks for a gap 1.5x greater than the last one) (Uses bitshift
                                    // to multiply by 3 then divide by 2. Much faster than multiplying by 1.5)
                    currentStatus.hasSync = true;
                    if (curGap > targetGap)  // we've found a gap
                        {
                            roverMEMSTeethSeen =
                                roverMEMSTeethSeen
                                << 2;  // add the space for the gap and the tooth we've just seen so shift 2 bits
                            ...
                            toothCurrentCount++;  // Increment the tooth counter on the wheel (used to spot a revolution
                                                  // and trigger igition timing)

                            // the missing tooth gap messing up timing as it appears in different parts of the cycle.
                            // Don't update setFilter as it would be wrong with the gap
                            toothCurrentCount++;
                        }
                    else  // Regular (non-missing) tooth so update things
                        {
                            roverMEMSTeethSeen = roverMEMSTeethSeen
                                                 << 1;  // make a space, shift the bits 1 place to the left
                            roverMEMSTeethSeen++;       // add the tooth seen
                            toothCurrentCount++;        // Increment the tooth counter on the wheel (used to spot a
                                                  // revolution)
                            setFilter(curGap);
                        }

                    // reduce checks to minimise cpu load when looking for key point to identify where we are on the
                    // wheel
                    if (toothCurrentCount >= triggerActualTeeth)
                        {  // 12345678901234567890123456789012
                            if (roverMEMSTeethSeen == 0b11111101111111011111111110111111)  // Binary pattern for trigger
                                                                                           // pattern 9-7-10-6- (#5)
                                {
                                    if (toothAngles[ID_TOOTH_PATTERN] != 5)
                                        {
                                            ...
                                            configPage4.triggerMissingTeeth =
                                                4;  // this could be read in from the config file, but people could
                                                    // adjust it.
                                            triggerActualTeeth = 36;  // should be 32 if not hacking toothcounter
                                        }
                                    triggerRoverMEMSCommon();
                                }  // 123456789012345678901234567890123456
                            else if (roverMEMSTeethSeen ==
                                     0b11011101111111111111101101111111)  // Binary pattern for trigger pattern
                                                                          // 3-14-2-13- (#4)
                                {
                                    ...
                                    triggerRoverMEMSCommon();
                                     ...
                                     0b11011011111111111111011101111111)  // Binary pattern for trigger pattern
                                                                          // 2-14-3-13- (#3)
                                {
                                    ...
                                    triggerRoverMEMSCommon();
                                     ...
                                     0b11111101111101111111111110111101)  // Binary pattern for trigger pattern
                                                                          // 11-5-12-4- (#2)
                                {
                                    ...
                                    triggerRoverMEMSCommon();
                                     ...
                                     0b11111111111101111111111111111101)  // Binary pattern for trigger pattern 17-17-
                                                                          // (#1)
                                {
                                    if (toothAngles[ID_TOOTH_PATTERN] != 1)
                                        {
                                            ...
                                            configPage4.triggerMissingTeeth =
                                                2;  // this should be read in from the config file, but people could
                                                    // adjust it.
                                            triggerActualTeeth = 36;  // should be 34 if not hacking toothcounter
                                        }
                                    triggerRoverMEMSCommon();
                                }
                            else if (toothCurrentCount >
                                     triggerActualTeeth + 1)  // no patterns match after a rotation when we only need 32
                                                              // teeth to match, we've lost sync
                                {
                                    currentStatus.hasSync = false;
                                    if (secondaryToothCount > 0)
                                        BIT_SET(currentStatus.status3, BIT_STATUS3_HALFSYNC);
...
static void triggerRoverMEMSCommon(void)
{
    // pattern 1 isn't unique & if we don't have a cam we need special code to identify if we're tooth 18 or 36 - this
    // allows batch injection but not spark to run as we have to be greater than 18 teeth when using the cam this code
    // also works for that.
    ...
    if ((configPage4.sparkMode == IGN_MODE_SEQUENTIAL) || (configPage2.injLayout == INJ_SEQUENTIAL))
        {
            // If either fuel or ignition is sequential, only declare sync if the cam tooth has been seen OR if the
            // missing wheel is on the cam
            if ((secondaryToothCount > 0) || (configPage4.TrigSpeed == CAM_SPEED))
                {
                    currentStatus.hasSync = true;
                    BIT_CLEAR(currentStatus.status3,
                              BIT_STATUS3_HALFSYNC);  // the engine is fully synced so clear the Half Sync bit
                    if (configPage4.trigPatternSec == SEC_TRIGGER_SINGLE)
                        {
                            secondaryToothCount = 0;
            ...
            if (configPage4.trigPatternSec == SEC_TRIGGER_SINGLE)
            ...
            else if (configPage4.trigPatternSec == SEC_TRIGGER_5_3_2)  // multi tooth cam
                {
                    if (curGap2 < targetGap2)  // ie normal tooth sized gap, not a single or double gap
                        ...
                        {
                            // gap either single or double - nb remember we've got the tooth after the gap, so on the 5
                            // tooth pattern we'll see here tooth 6
                            if (secondaryToothCount == 6)
                                {
                                    // if we've got the tooth after the gap from reading 5 teeth we're on cycle 360-720
                                    // & tooth 18-36
                                    revolutionOne = false;
                                ...
                                {
                                    // we've got the tooth after the gap from reading 3 teeth we're on cycle 0-360 &
                                    // tooth 1-18
                                    revolutionOne = true;
                                ...
                                {
                                    // if we've got the tooth after the gap from reading 2 teeth we're on cycle 0-360 &
                                    // tooth 18-36
                                    revolutionOne = true;
                                    if (toothCurrentCount < 19)
                                        {
                            ...
                            secondaryToothCount =
                                1;  // as we've had a gap we need to reset to this being the first tooth after the gap
                        }
                }
        }  // Trigger filter
}
...
{
    triggerToothAngle = 90;  // The number of degrees that passes from tooth to tooth (primary) - set to a value, needs
                             // to be set per tooth
    toothCurrentCount = 99;  // Fake tooth count represents no sync
    ...
    // gap between rising and falling edge of a normal 70 degree tooth is 48 degrees, this means the gap is 70 degrees -
    // 48 degrees = 22 degrees. assume this is constant for all similar sized gaps and teeth sync tooth is 35 degrees -
    // eyeball looks like the tooth is 50% tooth and 50% gap so guess its 17 degrees and 18 degrees.

    // coded every tooth here in case you want to try "change" setting on the trigger setup (this is defined in init.ino
    // and what i've set it to, otherwise you need code to select rising or falling in init.ino (steal it from another
    // trigger)). If you don't want change then drop the 'falling' edges listed below and half the number of edges +
    // reduce the triggerActualTeeth nb as you can edit the trigger offset using rising or falling edge setup below is
    // irrelevant as you can adjust via the trigger offset to cover the difference.

    // not using toothAngles[0] as i'm hoping it makes logic easier

    toothAngles[1] = 0;  // 0 TDC cylinder 1,
    toothAngles[2] =
        170;  // 170 - end of cylinder 1, start of cylinder 3, trigger ignition for cylinder 3 on this tooth
    toothAngles[3] = 240;  // 70 TDC cylinder 3
    toothAngles[4] =
        410;  // 170  - end of cylinder 3, start of cylinder2, trigger ignition for cylinder 2 on this tooth
    ...
    toothAngles[8] = 720;  // 70 - gap to rotation to TDC1. array item 1 and 8 are the same, code never gets here its
                           // for reference only
    ...
    if ((curGap >= triggerFilterTime) || (currentStatus.startRevolutions == 0))
        {
            ...
            // pattern is normally small tooth, big tooth, small tooth, big tooth. The extra tooth breaks the pattern go
            // it goes, big tooth (curGap3), small tooth(curGap2), small tooth(curGap) reuse curGap2 and curGap3 (from
            // secondary and Tertiary decoders) to store previous tooth sizes as not needed in this decoder.

            if ((curGap <= curGap2) && (curGap2 <= curGap3))
                {
                    // cur Gap is smaller than last gap & last gap is smaller than gap before that - means we must be on
                    // sync tooth
            ...
            if ((toothCurrentCount == (triggerActualTeeth + 1)) && currentStatus.hasSync == true)
                {
                    ...
                    toothOneMinusOneTime = toothOneTime;
                    toothOneTime = curTime;
                    currentStatus.startRevolutions = currentStatus.startRevolutions +
                                                     2;  // increment for 2 revs as we do 720 degrees on the the crank
                }
            ...
            switch (toothCurrentCount)
                {
                    ...
                    case 6:
                        ...
                        // eg tooth 3 should be smaller than tooth 2 gap, if its not then we've lost sync and the tooth
                        // 3 we've just seen isn't really tooth 3
                        if (curGap > curGap2)
                            {
                                currentStatus.hasSync = false;
                                currentStatus.syncLossCounter++;
            ...
            // Setup data to allow other areas of the system to work due to odd sized teeth - this could be merged with
            // sync checking above, left separate to keep code clearer as its doing only one function at once % of
            // filter are not based on previous tooth size but expected next tooth size triggerToothAngle is the size of
            // the previous tooth not the future tooth
            if (currentStatus.hasSync == true)
                {
                    switch (toothCurrentCount)  // Set tooth angle based on previous gap and triggerFilterTime based on
...
void triggerSec_SuzukiK6A(void) { return; }
...
int getCrankAngle_SuzukiK6A(void)
{
    ...
    crankAngle = toothAngles[(tempToothCurrentCount)] +
                 configPage4.triggerAngle;  // Perform a lookup of the fixed toothAngles array to find what the angle of
                                            // the last tooth passed was.
    ...
    //  if (crankAngle > CRANK_ANGLE_MAX) { crankAngle -= CRANK_ANGLE_MAX; } not needed, crank angle max gets max from
    //  injection or ignition, we have to be over 720 degrees so can ignore
    if (crankAngle < 0)
        {
            crankAngle += 720;
        }
