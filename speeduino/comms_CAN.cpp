...
int CAN_read() { return Can0.read(inMsg); }

void CAN_write() { Can0.write(outMsg); }
    ...
    if (configPage2.canWBO == CAN_WBO_RUSEFI)
        {
            ...
            outMsg.len = 2;
            outMsg.buf[0] =
                currentStatus.battery10;  // We don't do any conversion since factor is 0.1 and speeduino value is x10
            outMsg.buf[1] = BIT_CHECK(currentStatus.engine, BIT_ENGINE_RUN)
                                ? 0x1
                                : 0x0;  // Enable heater once engine is running (ie. above cranking rpm), this condition
                                        // can be changed to CLT above certain temp and so on.
            Can0.write(outMsg);
            if ((inMsg.id == 0x190 || inMsg.id == 0x192))
                {
                    uint32_t inLambda;
                    inLambda = (word(inMsg.buf[3], inMsg.buf[2]));  // Combining 2 bytes of data into single variable
                                                                    // factor is 0.0001 so lambda 1 comes in as 10K
                    if (inMsg.buf[1] == 0x1)                        // Checking if lambda is valid
                        {
                            switch (inMsg.id)
                                {
                                    case 0x190:
                                        if ((inLambda * configPage2.stoich / 10000) > 250)
                                            {  // Check if we dont overflow the 8bit O2 variable
                                                currentStatus.O2 = 250;
                                                break;
                                            }
                                        currentStatus.O2 =
                                            (unsigned int)(inLambda * configPage2.stoich /
                                                           10000);  // Multiplying lambda by stoich ratio to get AFR and
                                                                    // dividing it by 10000 to get correct value
                                        break;
    ...
    switch (DashMessageID)
        {
            case CAN_BMW_DME1:
                uint32_t temp_RPM;
                temp_RPM = currentStatus.RPM *
                           64;  // RPM conversion is currentStatus.RPM * 6.4, but this does it without floats.
                temp_RPM = temp_RPM / 10;
                outMsg.id = DashMessageID;
                outMsg.len = 8;
                outMsg.buf[0] = 0x05;  // bitfield, Bit0 = 1 = terminal 15 on detected, Bit2 = 1 = the ASC message ASC1
                                       // was received within the last 500 ms and contains no plausibility errors
                ...
                outMsg.buf[4] = 0x0C;  // Indicated Engine Torque in % of C_TQ_STND TBD do torque calculation!! Use same
                                       // as for byte 1
                outMsg.buf[5] = 0x15;  // Engine Torque Loss (due to engine friction, AC compressor and electrical power
                                       // consumption)
            ...
            case CAN_BMW_DME2:
                ...
                temp_CLT = (((currentStatus.coolant - CALIBRATION_TEMPERATURE_OFFSET) + 48) * 4 /
                            3);  // CLT conversion (actual value to add is 48.373, but close enough)
                if (temp_CLT > 255)
                    {
                        temp_CLT = 255;
                    }  // CLT conversion can yield to higher values than what fits to byte, so limit the maximum value
                       // to 255.
                ...
                outMsg.buf[6] =
                    0x00;  // bitfield, Bit0 = 0 = brake not actuated, Bit1 = 0 = brake switch system OK etc...
            ...
            case 0x545:  // fuel consumption and CEl light for BMW e46/e39/e38 instrument cluster
                         // fuel consumption calculation not implemented yet. But this still needs to be sent to get rid
                         // of the CEL and EML fault lights on the dash.
                outMsg.id = DashMessageID;
                outMsg.len = 5;
                outMsg.buf[0] = 0x00;  // Check engine light (binary 10), Cruise light (binary 1000), EML (binary
                                       // 10000).
            ...
            if (inMsg.buf[1] == 0x22)
                {
                    ...
                    obd_response(inMsg.buf[1], inMsg.buf[2],
                                 inMsg.buf[3]);  // get the obd response based on the data in byte2
            ...
            if (inMsg.buf[1] == 0x09)
                {
                    ...
                    if (inMsg.buf[2] == 02)
                        ...
                        {
                            // code 20: send 20 ascii characters with ECU name , "ECU -SpeeduinoXXXXXX" , change the
                            // XXXXXX ONLY as required.
                        }
                }
        }
}

// This routine builds the realtime data into packets that the obd requesting device can understand. This is only used
// by teensy and stm32 with onboard canbus
            ...
            switch (requestedPIDlow)
                {
                    case 0:                    // PID-0x00 PIDs supported 01-20
                        ...
                        outMsg.buf[1] = 0x41;  // Same as query, except that 40h is added to the mode value. So:41h =
                    ...
                    case 5:  // PID-0x05 Engine coolant temperature , range is -40 to 215 deg C , formula == A-40
                        ...
                        outMsg.buf[3] =
                            (byte)(currentStatus.coolant + CALIBRATION_TEMPERATURE_OFFSET);  // the data value A
                    ...
                    case 10:  // PID-0x0A , Fuel Pressure (Gauge) , range is 0 to 765 kPa , formula == A / 3)
                        ...
                        // Fuel pressure is in PSI. PSI to kPa is 6.89475729, but that needs to be divided by 3 for OBD2
                        // formula. So 2.298.... 2.3 is close enough, so that in fraction.
                    ...
                    case 36:  // PID-0x24 O2 sensor2, AB: fuel/air equivalence ratio, CD: voltage ,  Formula ==
                              // (2/65536)(256A +B) , 8/65536(256C+D) , Range is 0 to <2 and 0 to >8V
                        // uint16_t O2_1e ;
                        // int16_t O2_1v ;
                        obdcalcH16 = configPage2.stoich / 10;  // configPage2.stoich(is *10 so 14.7 is 147)
                        obdcalcE32 =
                            currentStatus.O2 / 10;  // afr(is *10 so 25.5 is 255) , needs a 32bit else will overflow
                        obdcalcF32 =
                            (obdcalcE32 << 8) /
                            obdcalcH16;  // this is same as (obdcalcE32/256) / obdcalcH16 . this calculates the ratio
                    ...
                    case 37:  // O2 sensor2, AB fuel/air equivalence ratio, CD voltage ,  2/65536(256A +B)
                              // ,8/65536(256C+D) , range is 0 to <2 and 0 to >8V
                        // uint16_t O2_2e ;
                        // int16_t O2_2V ;
                        obdcalcH16 = configPage2.stoich / 10;  // configPage2.stoich(is *10 so 14.7 is 147)
                        obdcalcE32 =
                            currentStatus.O2_2 / 10;  // afr(is *10 so 25.5 is 255) , needs a 32bit else will overflow
                        obdcalcF32 =
                            (obdcalcE32 << 8) /
                            obdcalcH16;  // this is same as (obdcalcE32/256) / obdcalcH16 . this calculates the ratio
                        obdcalcG16 = (obdcalcF32 * 32768) >> 8;
                    ...
                    case 70:  // PID-0x46 Ambient Air Temperature , range is -40 to 215 deg C , formula == A-40
                        ...
                        obdcalcA =
                            temp_ambientair +
                            40;  // maybe later will be (byte)(currentStatus.AAT + CALIBRATION_TEMPERATURE_OFFSET)
                        outMsg.buf[0] = 0x03;  // sending 3 byte
                        outMsg.buf[1] = 0x41;  // Same as query, except that 40h is added to the mode value. So:41h =
                                               // show current data ,42h = freeze frame ,etc.
                    ...
                    case 92:  // PID-0x5C Engine oil temperature , range is -40 to 210 deg C , formula == A-40
                        ...
                        obdcalcA =
                            temp_engineoiltemp +
                            40;  // maybe later will be (byte)(currentStatus.EOT + CALIBRATION_TEMPERATURE_OFFSET)
                        outMsg.buf[0] = 0x03;  // sending 3 byte
                        outMsg.buf[1] = 0x41;  // Same as query, except that 40h is added to the mode value. So:41h =
                                               // show current data ,42h = freeze frame ,etc.
    ...
    else if (PIDmode == 0x22)
        {
                    ...
                    if ((requestedPIDlow >= 0x01) && (requestedPIDlow <= 0x10))
                        {
                            ...
                            outMsg.buf[1] = 0x62;  // Same as query, except that 40h is added to the mode value. So:62h
                                                   // = custom mode
            ...
            // this allows to get any value out of current status array.
            else if (requestedPIDhigh == 0x78)
                {
                    ...
                    outMsg.buf[0] = 0x06;  // sending 6 bytes
                    outMsg.buf[1] =
                        0x62;  // Same as query, except that 40h is added to the mode value. So:62h = custom mode
                    outMsg.buf[2] = requestedPIDlow;      // PID code
                            ...
                            if (configPage9.caninputEndianess == 1)
                                {
                                    ...
                                    currentStatus.canin[i] =
                                        ((inMsg.buf[configPage9.caninput_source_start_byte[i]]) |
                                         (inMsg.buf[configPage9.caninput_source_start_byte[i] + 1] << 8));
                                }
                            else
                                {
                                    // Gets the two-byte value from the Data Field in Big Endian.
                                    currentStatus.canin[i] =
                                        ((inMsg.buf[configPage9.caninput_source_start_byte[i]] << 8) |
                                         (inMsg.buf[configPage9.caninput_source_start_byte[i] + 1]));
                                }
                        }
                }
        }
