...
#define SERIAL_BUFFER_SIZE \
    (256 + 7 + 1)  // Size of the serial buffer used by new comms protocol. The largest single packet is the O2
                   // calibration which is 256 bytes + 7 bytes of overhead
...
/*#define micros() (((timer5_overflow_count << 16) + TCNT5) * 4) */  // Fast version of micros() that uses the 4uS tick
                                                                     // of timer5. See timers.ino for the overflow ISR
                                                                     // of timer5
#define millis() \
    (ms_counter)  // Replaces the standard millis() function with this macro. It is both faster and more accurate. See
                  // timers.ino for its counter increment.
static inline unsigned long micros_safe();  // A version of micros() that is interrupt safe
#else
#define micros_safe() \
    micros()  // If the timer5 method is not used, the micros_safe() macro is simply an alias for the normal micros()
#endif
...
static inline void FUEL1_TIMER_ENABLE(void)
{
    TIFR3 |= (1 << OCF3A);
    TIMSK3 |= (1 << OCIE3A);
}  // Turn on the A compare unit (ie turn on the interrupt)
static inline void FUEL2_TIMER_ENABLE(void)
{
    TIFR3 |= (1 << OCF3B);
    TIMSK3 |= (1 << OCIE3B);
}  // Turn on the B compare unit (ie turn on the interrupt)
static inline void FUEL3_TIMER_ENABLE(void)
{
    TIFR3 |= (1 << OCF3C);
    TIMSK3 |= (1 << OCIE3C);
}  // Turn on the C compare unit (ie turn on the interrupt)
...
// These have the TIFR5 bits set to 1 to clear the interrupt flag. This prevents a false interrupt being called the
// first time the channel is enabled.
...
#define MAX_TIMER_PERIOD \
    262140UL  // The longest period of time (in uS) that the timer can permit (IN this case it is 65535 * 4, as each
              // timer tick is 4uS)
#define uS_TO_TIMER_COMPARE(uS1) \
    ((uS1) >> 2)  // Converts a given number of uS into the required number of timer ticks until that time has passed
