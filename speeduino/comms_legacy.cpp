...
bool firstCommsRequest = true; /**< The number of times the A command has been issued. This is used to track whether a
                                  reset has recently been performed on the controller */
static byte currentCommand;    /**< The serial command that is currently being processed. This is only useful when
                                  cmdPending=True */
...
static int
    valueOffset; /**< The memory offset within a given page for a value to be read from or written to. Note that we
                    cannot use 'offset' as a variable name, it is a reserved word for several teensy libraries */
...
static bool isMap(void)
{
    ...
    return (currentPage == veMapPage) || (currentPage == ignMapPage) || (currentPage == afrMapPage) ||
           (currentPage == fuelMap2Page) || (currentPage == ignMap2Page);
}
...
- chunkPending = Specifically for the new receive value method where TS will send a known number of contiguous bytes to
    ...
    switch (currentCommand)
        {
            ...
            case 'C':  // test communications. This is used by Tunerstudio to see whether there is an ECU on a given
                       // serial port
                testComm();
                break;
            ...
            // The G/g commands are used for bulk reading and writing to the EEPROM directly. This is typically a
            // non-user feature but will be incorporated into SpeedyLoader for anyone programming many boards at once
            ...
            case 'g':  // Receive a dump of raw EEPROM values from the user
                {
                    // Format is similar to the above command. 2 bytes for the EEPROM size that is about to be
                    // transmitted, a comma and then a raw dump of the EEPROM values
            ...
            case 'H':  // Start the tooth logger
                startToothLogger();
                Serial.write(1);  // TS needs an acknowledgement that this was received. I don't know if this is the
                                  // correct response, but it seems to work
                break;
            ...
            case 'P':  // set the current page
                // This is a legacy function and is no longer used by TunerStudio. It is maintained for compatibility
                // with other systems A 2nd byte of data is required after the 'P' specifying the new page number.
                serialStatusFlag = SERIAL_COMMAND_INPROGRESS_LEGACY;

                if (Serial.available() > 0)
                    {
                        currentPage = Serial.read();
                        // This converts the ASCII number char into binary. Note that this will break everything if
                        // there are ever more than 48 pages (48 = asci code for '0')
            ...
            case 'W':  // receive new VE obr constant at 'W'+<offset>+<newbyte>
                ...
                if (isMap())
                    {
                        if (Serial.available() >=
                            3)  // 1 additional byte is required on the MAP pages which are larger than 255 bytes
                            {
            ...
            case 'Z':  // Totally non-standard testing function. Will be removed once calibration testing is completed.
                       // This function takes 1.5kb of program space! :S
#ifndef SMALL_FLASH_MODE
                ...
                (configPage9.secondarySerialProtocol == SECONDARY_SERIAL_PROTO_REALDASH))
                {
                    if (cmd == 0x30)
                    ...
                    else if (cmd == 0x32)
                        {
                            ...
                            secondarySerial.write(
                                NEW_CAN_PACKET_SIZE);  // send the packet size the receiving device should expect.
                        }
                }
        }
    else
    ...
    for (byte x = 0; x < packetLength; x++)
        {
            ...
            if ((&targetPort == &Serial))
                {
                    // If the transmit buffer is full, wait for it to clear. This cannot be used with Read Dash as it
                    // will cause a timeout
                    if (targetPort.availableForWrite() < 1)
                        {
                            bufferFull = true;
                        }
                }
 ...
 *
 * This is used for testing only (Not used by TunerStudio) in order to see current map and config data without the need
 * for TunerStudio.
 */
    ...
    switch (currentPage)
        {
            ...
            case veSetPage:
                Serial.println(F("\nPg 2 Cfg"));
                // The following loop displays in human readable form of all byte values in config page 1 up to but not
                ...
                // This displays all the byte values between the last array up to but not including the first unsigned
                // int on config page 1
            ...
            case ignSetPage:
                ...
                Serial.println(
                    configPage4
                        .triggerAngle);  // configPage4.triggerAngle is an int so just display it without complication
                // Following loop displays byte values after that first int up to but not including the first array in
                // config page 2
                ...
                Serial.println(
                    configPage4
                        .dwellLimit);  // Little lonely byte stuck between two arrays. No complications just display it.
                serial_print_space_delimited_array(configPage4.dwellCorrectionValues);
                serial_println_range(_end_range_byte_address(configPage4.dwellCorrectionValues),
                                     (byte *)&configPage4 + sizeof(configPage4));
                break;
            ...
            case afrSetPage:
                ...
                serial_println_range(_end_range_byte_address(configPage6.iacCrankBins),
                                     (byte *)&configPage6 + sizeof(configPage6));
                break;
            ...
            for (int x = 0; x < 1024; x++)
                {
                    ...
                    if ((x % 32) == 0)
                        {
                            ((uint8_t *)pnt_TargetTable_values)[(x / 32)] =
                                (byte)tempValue;  // O2 table stores 8 bit values
            ...
            for (uint16_t x = 0; x < 32; x++)
                {
                    ...
                    tempValue = (int16_t)(word(
                        tempBuffer[1], tempBuffer[0]));  // Combine the 2 bytes into a single, signed 16-bit value
                    tempValue = div(tempValue, DIVISION_FACTOR)
                                    .quot;  // TS sends values multiplied by 10 so divide back to whole degrees.
...
{
    // We need TOOTH_LOG_SIZE number of records to send to TunerStudio. If there aren't that many in the buffer then we
    // just return and wait for the next call
    if (BIT_CHECK(currentStatus.status1,
                  BIT_STATUS1_TOOTHLOG1READY))  // Sanity check. Flagging system means this should always be true
        {
            ...
            for (int x = startOffset; x < TOOTH_LOG_SIZE; x++)
                {
                    ...
                    uint32_t inProgressCompositeTime =
                        toothHistory[x];  // This combined runtime (in us) that the log was going for by this record)
                    ...
                    Serial.write(compositeLogHistory[x]);  // The status byte (Indicates the trigger edge, whether it
                                                           // was a pri/sec pulse, the sync status)
                }
            BIT_CLEAR(currentStatus.status1, BIT_STATUS1_TOOTHLOG1READY);
            toothHistoryIndex = 0;
            serialStatusFlag = SERIAL_INACTIVE;
