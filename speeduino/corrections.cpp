...
void initialiseCorrections(void)
{
    ...
    currentStatus.egoCorrection =
        100;  // Default value of no adjustment must be set to avoid randomness on first correction cycle after startup
    AFRnextCycle = 0;
    currentStatus.knockActive = false;
    currentStatus.battery10 = 125;  // Set battery voltage to sensible value for dwell correction for "flying start"
...
uint16_t correctionsFuel(void)
{
    ...
    // The values returned by each of the correction functions are multiplied together and then divided back to give a
    // single 0-255 value.
    ...
    if ((configPage2.aeApplyMode == AE_MODE_MULTIPLIER) ||
        BIT_CHECK(currentStatus.engine,
                  BIT_ENGINE_DCC))  // multiply by the AE amount in case of multiplier AE mode or Decel
        {
    ...
    if (configPage2.battVCorMode == BATTV_COR_MODE_OPENTIME)
        {
            inj_opentime_uS =
                configPage2.injOpen * currentStatus.batCorrection;  // Apply voltage correction to injector open time.
            // currentStatus.batCorrection = 100; // This is to ensure that the correction is not applied twice. There
            // is no battery correction fator as we have instead changed the open time
        }
    ...
    // If we're not cranking, check if if cranking enrichment tapering to ASE should be done
    else if (crankingEnrichTaper < configPage10.crankingEnrichTaper)
        {
            crankingValue =
                table2D_getValue(&crankingEnrichTable, currentStatus.coolant + CALIBRATION_TEMPERATURE_OFFSET);
            crankingValue = (uint16_t)crankingValue * 5;  // multiplied by 5 to get range from 0% to 1275%
            // Taper start value needs to account for ASE that is now running, so total correction does not increase
            // when taper begins
            unsigned long taperStart = (unsigned long)crankingValue * 100 / currentStatus.ASEValue;
            crankingValue = (uint16_t)map(crankingEnrichTaper, 0, configPage10.crankingEnrichTaper, taperStart,
                                          100);  // Taper from start value to 100%
    ...
    if (BIT_CHECK(currentStatus.engine, BIT_ENGINE_CRANK) != true)
        {
            if (BIT_CHECK(LOOP_TIMER, BIT_TIMER_10HZ) || (currentStatus.ASEValue == 0))
                {
                    if ((currentStatus.runSecs <
                         (table2D_getValue(&ASECountTable, currentStatus.coolant + CALIBRATION_TEMPERATURE_OFFSET))) &&
                        !(BIT_CHECK(currentStatus.engine, BIT_ENGINE_CRANK)))
                        {
                            BIT_SET(currentStatus.engine, BIT_ENGINE_ASE);  // Mark ASE as active.
                            ASEValue = 100 + table2D_getValue(&ASETable,
                                                              currentStatus.coolant + CALIBRATION_TEMPERATURE_OFFSET);
                            aseTaper = 0;
                        }
                    else
                        {
                            if (aseTaper < configPage2.aseTaperTime)  // Check if we've reached the end of the taper
                                                                      // time
                                {
                                    BIT_SET(currentStatus.engine, BIT_ENGINE_ASE);  // Mark ASE as active.
                                    ASEValue =
                                        100 + map(aseTaper, 0, configPage2.aseTaperTime,
                                                  table2D_getValue(&ASETable, currentStatus.coolant +
...
uint16_t correctionAccel(void)
{
    ...
    if (configPage2.aeMode == AE_MODE_MAP)
        {
            ...
            currentStatus.mapDOT = (MICROS_PER_SEC / (MAP_time - MAPlast_time)) *
                                   MAP_change;  // This is the % per second that the MAP has moved
    ...
    else if (configPage2.aeMode == AE_MODE_TPS)
        {
            ...
            // currentStatus.tpsDOT = ldiv(MICROS_PER_SEC, (TPS_time - TPSlast_time)).quot * TPS_change; //This is the %
            // per second that the TPS has moved
            currentStatus.tpsDOT =
                (TPS_READ_FREQUENCY * TPS_change) /
                2;  // This is the % per second that the TPS has moved, adjusted for the 0.5% resolution of the TPS
        }
    ...
    if (BIT_CHECK(currentStatus.engine, BIT_ENGINE_ACC) || BIT_CHECK(currentStatus.engine, BIT_ENGINE_DCC))
        {
            // If it is currently running, check whether it should still be running or whether it's reached it's end
            // time
            if (micros_safe() >= currentStatus.AEEndTime)
            ...
            else
                {
                    ...
                    // If the accel amount HAS increased, we clear the current enrich phase and a new one will be
                    // started below
    ...
    if (!BIT_CHECK(currentStatus.engine, BIT_ENGINE_ACC) &&
        !BIT_CHECK(currentStatus.engine,
                   BIT_ENGINE_DCC))  // Need to check this again as it may have been changed in the above section (Both
                                     // ACC and DCC are off if this has changed)
        {
            if (configPage2.aeMode == AE_MODE_MAP)
                {
                            ...
                            if (abs(currentStatus.mapDOT) > configPage2.maeThresh)
                                {
                                    ...
                                    currentStatus.AEEndTime =
                                        micros_safe() +
                                        ((unsigned long)configPage2.aeTime *
                                         10000);  // Set the time in the future where the enrichment will be turned off.
                                                  // taeTime is stored as mS / 10, so multiply it by 100 to get it in uS
                                    // Check if the MAP rate of change is negative or positive. Negative means
                                    // decelarion.
                                    if (currentStatus.mapDOT < 0)
                                        {
                                            BIT_SET(currentStatus.engine,
                                                    BIT_ENGINE_DCC);  // Mark deceleration enleanment as active.
                                            accelValue =
                                                configPage2
                                                    .decelAmount;  // In decel, use the decel fuel amount as accelValue
                                        }                          // Deceleration
                                    // Positive MAP rate of change is acceleration.
                                    else
                                        {
                                            BIT_SET(currentStatus.engine,
                                                    BIT_ENGINE_ACC);  // Mark acceleration enrichment as active.
                                            accelValue = table2D_getValue(
                                                &maeTable,
                                                currentStatus.mapDOT / 10);  // The x-axis of mae table is divided by 10
                                                                             // to fit values in byte.
                                            ...
                                            if (currentStatus.RPM > trueTaperMin)
                                                {
                                                    if (currentStatus.RPM > trueTaperMax)
                                                        {
                                                            accelValue = 0;
                                                        }  // RPM is beyond taper max limit, so accel enrich is turned
                                                           // off
                                                    else
                                                        {
                                                            int16_t taperRange = trueTaperMax - trueTaperMin;
                                                            int16_t taperPercent =
                                                                ((currentStatus.RPM - trueTaperMin) * 100UL) /
                                                                taperRange;  // The percentage of the way through the
                                                                             // RPM taper range
                                                            accelValue = percentage(
                                                                (100 - taperPercent),
                                                                accelValue);  // Calculate the above percentage of the
                                                                              // calculated accel amount.
                                                        }
                                                }

                                            // Apply AE cold coolant modifier, if CLT is less than taper end temperature
                                            if (currentStatus.coolant <
                                                (int)(configPage2.aeColdTaperMax - CALIBRATION_TEMPERATURE_OFFSET))
                                                {
                                                    // If CLT is less than taper min temp, apply full modifier on top of
                                                    // accelValue
                                                    if (currentStatus.coolant <= (int)(configPage2.aeColdTaperMin -
                                                                                       CALIBRATION_TEMPERATURE_OFFSET))
                                                        {
                                                            uint16_t accelValue_uint =
                                                                percentage(configPage2.aeColdPct, accelValue);
                                                            accelValue = (int16_t)accelValue_uint;
                                                        }
                                                    // If CLT is between taper min and max, taper the modifier value and
                                                    // apply it on top of accelValue
                                                    else
                                                        {
                                                            int16_t taperRange = (int16_t)configPage2.aeColdTaperMax -
                                                                                 configPage2.aeColdTaperMin;
                                                            int16_t taperPercent =
                                                                (int)((currentStatus.coolant +
                                                                       CALIBRATION_TEMPERATURE_OFFSET -
                                                                       configPage2.aeColdTaperMin) *
                                                                      100) /
                                                                taperRange;
                                                            int16_t coldPct = (int16_t)100 +
                                                                              percentage((100 - taperPercent),
                                                                                         (configPage2.aeColdPct - 100));
                                                            uint16_t accelValue_uint =
                                                                (uint16_t)accelValue * coldPct /
                                                                100;  // Potential overflow (if AE is large) without
                                                                      // using uint16_t (percentage() may overflow)
                                                            accelValue = (int16_t)accelValue_uint;
                                                        }
                                                }
                                            accelValue = 100 + accelValue;  // In case of AE, add the 100 normalisation
                ...
                {
                    // Check for only very small movement. This not only means we can skip the lookup, but helps reduce
                    // false triggering around 0-2% throttle openings
                            ...
                            if (abs(currentStatus.tpsDOT) > configPage2.taeThresh)
                                {
                                    ...
                                    currentStatus.AEEndTime =
                                        ...
                                        ((unsigned long)configPage2.aeTime *
                                         10000);  // Set the time in the future where the enrichment will be turned off.
                                                  // taeTime is stored as mS / 10, so multiply it by 100 to get it in uS
                                    // Check if the TPS rate of change is negative or positive. Negative means
                                    // decelarion.
                                    ...
                                    // Positive TPS rate of change is Acceleration.
                                    else
                                        {
                                            ...
                                            accelValue = table2D_getValue(
                                                &taeTable,
                                                currentStatus.tpsDOT / 10);  // The x-axis of tae table is divided by 10
                                                                             // to fit values in byte.
                                            // Apply the RPM taper to the above
                                            // The RPM settings are stored divided by 100:
                                            uint16_t trueTaperMin = configPage2.aeTaperMin * 100;
...
@return 100 (not cranking and thus no need for flood-clear) or 0 (Engine cranking and TPS above @ref config4.floodClear
limit).
*/
...
{
    byte IATValue = 100;
    IATValue = table2D_getValue(
        &IATDensityCorrectionTable,
        currentStatus.IAT + CALIBRATION_TEMPERATURE_OFFSET);  // currentStatus.IAT is the actual temperature, values in
                                                              // IATDensityCorrectionTable.axisX are temp+offset

    return IATValue;
}

/** Correction for current barometric / ambient pressure.
 * @returns A percentage value indicating the amount the fuelling should be changed based on the barometric reading. 100
 * = No change. 110 = 10% increase. 90 = 10% decrease
 */
    ...
    if (configPage2.dfcoEnabled == 1)
        {
            if (BIT_CHECK(currentStatus.status1, BIT_STATUS1_DFCO) == 1)
                {
                    DFCOValue = (currentStatus.RPM > (configPage4.dfcoRPM * 10)) &&
...
It then waits egoDelta number of ignition events and compares O2 against the target table again. If it is still
lean/rich then the adjustment is increased to 2%.

This continues until either:
- the O2 reading flips from lean to rich, at which point the adjustment cycle starts again at 1% or
- the adjustment amount increases to egoLimit at which point it stays at this level until the O2 state (rich/lean)
changes
...
byte correctionAFRClosedLoop(void)
{
    ...
    if ((configPage6.egoType > 0) ||
        (configPage2.incorporateAFR ==
         true))  // afrTarget value lookup must be done if O2 sensor is enabled, and always if incorporateAFR is enabled
        {
            currentStatus.afrTarget =
                currentStatus.O2;  // Catch all in case the below doesn't run. This prevents the Include AFR option from
                                   // doing crazy things if the AFR target conditions aren't met. This value is changed
                                   // again below if all conditions are met.

            // Determine whether the Y axis of the AFR target table tshould be MAP (Speed-Density) or TPS (Alpha-N)
            // Note that this should only run after the sensor warmup delay when using Include AFR option, but on
            // Incorporate AFR option it needs to be done at all times
    ...
    if ((configPage6.egoType > 0) && (BIT_CHECK(currentStatus.status1, BIT_STATUS1_DFCO) !=
                                      1))  // egoType of 0 means no O2 sensor. If DFCO is active do not run the ego
                                           // controllers to prevent iterator wind-up.
        {
            AFRValue = currentStatus.egoCorrection;  // Need to record this here, just to make sure the correction stays
                                                     // 'on' even if the nextCycle count isn't ready

            if ((ignitionCount >= AFRnextCycle) || (ignitionCount < (AFRnextCycle - configPage6.egoCount)))
                {
                    AFRnextCycle =
                        ignitionCount + configPage6.egoCount;  // Set the target ignition event for the next calculation
                    ...
                    if ((currentStatus.coolant > (int)(configPage6.egoTemp - CALIBRATION_TEMPERATURE_OFFSET)) &&
                        ...
                        (currentStatus.TPS <= configPage6.egoTPSMax) && (currentStatus.O2 < configPage6.ego_max) &&
                        (currentStatus.O2 > configPage6.ego_min) && (currentStatus.runSecs > configPage6.ego_sdelay) &&
                        (BIT_CHECK(currentStatus.status1, BIT_STATUS1_DFCO) == 0) &&
                        ...
                        (currentStatus.MAP >= (configPage9.egoMAPMin * 2U)))
                        {
                            ...
                            if (configPage6.egoAlgorithm == EGO_ALGORITHM_SIMPLE)
                                    ...
                                    if (currentStatus.O2 > currentStatus.afrTarget)
                                        {
                                            ...
                                            if (currentStatus.egoCorrection <
                                                (100 + configPage6.egoLimit))  // Fuelling adjustment must be at most
                                                                               // the egoLimit amount (up or down)
                                                {
                                                    AFRValue = (currentStatus.egoCorrection +
                                                                1);  // Increase the fuelling by 1%
                                                }
                                            else
                                                {
                                                    AFRValue = currentStatus.egoCorrection;
                                                }  // Means we're at the maximum adjustment amount, so simply return
                                                   // that again
                                        }
                            ...
                            else if (configPage6.egoAlgorithm == EGO_ALGORITHM_PID)
                                {
                                    //*************************************************************************************************************************************
                                    // PID algorithm
                                    egoPID.SetOutputLimits(
                                        (long)(-configPage6.egoLimit),
                                        (long)(configPage6
                                                   .egoLimit));  // Set the limits again, just in case the user has
                                                                 // changed them since the last loop. Note that these
                                                                 // are sent to the PID library as (Eg:) -15 and +15
                                    egoPID.SetTunings(configPage6.egoKP, configPage6.egoKI,
                                                      configPage6.egoKD);  // Set the PID values again, just in case the
                                                                           // user has changed them since the last loop
...
int8_t correctionFlexTiming(int8_t advance)
{
    ...
    if (configPage2.flexEnabled == 1)  // Check for flex being enabled
        {
            ignFlexValue = (int16_t)table2D_getValue(&flexAdvTable, currentStatus.ethanolPct) -
                           OFFSET_IGNITION;                          // Negative values are achieved with offset
            currentStatus.flexIgnCorrection = (int8_t)ignFlexValue;  // This gets cast to a signed 8 bit value to allows
                                                                     // for negative advance (ie retard) values here.
...
int8_t correctionWMITiming(int8_t advance)
{
    if ((configPage10.wmiEnabled >= 1) && (configPage10.wmiAdvEnabled == 1) &&
        !BIT_CHECK(currentStatus.status4, BIT_STATUS4_WMI_EMPTY))  // Check for wmi being enabled
        {
            if ((currentStatus.TPS >= configPage10.wmiTPS) && (currentStatus.RPM >= configPage10.wmiRPM) &&
                (currentStatus.MAP / 2 >= configPage10.wmiMAP) &&
                ((currentStatus.IAT + CALIBRATION_TEMPERATURE_OFFSET) >= configPage10.wmiIAT))
                {
                    return (int16_t)advance + table2D_getValue(&wmiAdvTable, currentStatus.MAP / 2) -
                           OFFSET_IGNITION;  // Negative values are achieved with offset
                }
        }
    return advance;
...
int8_t correctionIdleAdvance(int8_t advance)
{
    ...
    if ((configPage2.idleAdvEnabled >= 1) && (runSecsX10 >= (configPage2.idleAdvDelay * 5)) && idleAdvActive)
        {
            // currentStatus.CLIdleTarget = (byte)table2D_getValue(&idleTargetTable, currentStatus.coolant +
            // CALIBRATION_TEMPERATURE_OFFSET); //All temps are offset by 40 degrees
            ...
            if ((currentStatus.RPM < (configPage2.idleAdvRPM * 100)) &&
                ...
                (((configPage2.idleAdvAlgorithm == 0) && (currentStatus.TPS < configPage2.idleAdvTPS)) ||
                 ((configPage2.idleAdvAlgorithm == 1) &&
                  (currentStatus.CTPSActive == 1))))  // closed throttle position sensor (CTPS) based idle state
                {
                    if (idleAdvTaper < configPage9.idleAdvStartDelay)
                        ...
                        {
                            int8_t advanceIdleAdjust =
                                (int16_t)(table2D_getValue(&idleAdvanceTable, idleRPMdelta)) - 15;
    ...
    When some other mechanism is also present, wait until the engine is no more than 200 RPM below idle target speed on
    first time
    */
...
uint16_t correctionsDwell(uint16_t dwell)
{
    ...
    uint16_t sparkDur_uS =
        (configPage4.sparkDur * 100);  // Spark duration is in mS*10. Multiple it by 100 to get spark duration in uS
    ...
    //**************************************************************************************************************************
    // Dwell error correction is a basic closed loop to keep the dwell time consistent even when adjusting its end time
    // for the per tooth timing. This is mostly of benefit to low resolution triggers at low rpm (<1500)
    ...
    //**************************************************************************************************************************
    /*
    Dwell limiter - If the total required dwell time per revolution is longer than the maximum time available at the
    current RPM, reduce dwell. This can occur if there are multiple sparks per revolution This only times this can occur
    are:
    1. Single channel spark mode where there will be nCylinders/2 sparks per revolution
    2. Rotary ignition in wasted spark configuration (FC/FD), results in 2 pulses per rev. RX-8 is fully sequential
    resulting in 1 pulse, so not required
    */
    ...
    if (((configPage4.sparkMode == IGN_MODE_SINGLE) ||
        ...
        (configPage2.nCylinders > 1))  // No point in running this for 1 cylinder engines
        {
