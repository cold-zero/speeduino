 ...
 * The doUpdates() uses may lower level routines from Arduino EEPROM library and storage.ino to carry out EEPROM storage
 * tasks.
 */
#include "updates.h"
...
#define CURRENT_DATA_VERSION 23
    ...
    // July 2017 adds a cranking enrichment curve in place of the single value. This converts that single value to the
    // curve
    if (readEEPROMVersion() == 4)
        {
            // Some default values for the bins (Doesn't matter too much here as the values against them will all be
            // identical)
    ...
    // September 2017 had a major change to increase the minimum table size to 128. This required multiple pieces of
    ...
    // November 2017 added the staging table that comes after boost and vvt in the EEPROM. This required multiple pieces
            ...
            for (uint8_t x = 1; x < 6; x++)
                {
                    ...
                    int16_t boostAdder = (((configPage2.aeColdTaperMin - (int8_t)configPage2.aeColdPct) * pct) / 100) +
                                         (int8_t)configPage2.aeColdPct;
                    configPage10.flexBoostAdj[x] = boostAdder;

                    uint8_t fuelAdder =
                        (((configPage2.idleUpAdder - configPage2.idleUpPin) * pct) / 100) + configPage2.idleUpPin;
                    configPage10.flexFuelAdj[x] = fuelAdder;

                    uint8_t advanceAdder =
                        (((configPage2.aeTaperMax - configPage2.aeTaperMin) * pct) / 100) + configPage2.aeTaperMin;
    ...
    if (readEEPROMVersion() == 9)
        {
            // October 2018 set default values for all the aux in variables (These were introduced in Aug, but no
            // defaults were set then) All aux channels set to Off
    ...
    if (readEEPROMVersion() == 10)
        {
            ...
            configPage2.primePulse[0] =
                configPage2.aeColdTaperMax /
                5;  // New priming pulse values are in the range 0-127.5 rather than 0-25.5 so they must be divided by 5
            configPage2.primePulse[1] =
                configPage2.aeColdTaperMax /
                5;  // New priming pulse values are in the range 0-127.5 rather than 0-25.5 so they must be divided by 5
            configPage2.primePulse[2] =
                configPage2.aeColdTaperMax /
                5;  // New priming pulse values are in the range 0-127.5 rather than 0-25.5 so they must be divided by 5
            configPage2.primePulse[3] =
                configPage2.aeColdTaperMax /
                5;  // New priming pulse values are in the range 0-127.5 rather than 0-25.5 so they must be divided by 5
            ...
            // March 19 added a tacho pulse duration that could default to stupidly high values. Check if this is the
            // case and fix it if found. 6ms is the maximum allowed value
            if (configPage2.tachoDuration > 6)
                {
                    configPage2.tachoDuration = 3;
                }
    ...
    if (readEEPROMVersion() == 13)
        {
            ...
            if (configPage6.idleKP >= 8)
                {
                    configPage6.idleKP = 255;
                }
            else
                {
                    configPage6.idleKP = configPage6.idleKP << 5;
                }
            if (configPage6.idleKI >= 8)
                {
                    configPage6.idleKI = 255;
                }
            else
                {
                    configPage6.idleKI = configPage6.idleKI << 5;
                }
            if (configPage6.idleKD >= 8)
                {
                    configPage6.idleKD = 255;
                }
            else
                {
                    configPage6.idleKD = configPage6.idleKD << 5;
                }
            if (configPage10.vvtCLKP >= 8)
                {
                    configPage10.vvtCLKP = 255;
                }
            else
                {
                    configPage10.vvtCLKP = configPage10.vvtCLKP << 5;
                }
            if (configPage10.vvtCLKI >= 8)
                {
                    configPage10.vvtCLKI = 255;
                }
            else
                {
                    configPage10.vvtCLKI = configPage10.vvtCLKI << 5;
                }
            if (configPage10.vvtCLKD >= 8)
                {
                    configPage10.vvtCLKD = 255;
                }
            else
                {
            ...
            // there is now option for fixed and relative timing retard for soft limit. This sets the soft limiter to
            // the old fixed timing mode.
...
// Move this #endif to only do latest updates to safe ROM space on small devices.
    ...
    if (readEEPROMVersion() == 18)
        {
            // 202202
            configPage2.fanEnable =
                configPage6.fanUnused;  // PWM Fan mode added, but take the previous setting of Fan in use.
            ...
            if (configPage10.fuel2SwitchVariable == FUEL2_CONDITION_TPS)
                {
                    configPage10.fuel2SwitchValue *= 2;
                }
            if (configPage10.spark2SwitchVariable == SPARK2_CONDITION_TPS)
                {
                    configPage10.spark2SwitchVariable *= 2;
                }
            ...
            if (configPage2.ignAlgorithm == LOAD_SOURCE_TPS)
                {
                    multiplyTableLoad(&ignitionTable, ignitionTable.type_key, 4);
                }
            if (configPage10.fuel2Algorithm == LOAD_SOURCE_TPS)
                {
                    multiplyTableLoad(&fuelTable2, fuelTable2.type_key, 4);
                }
            if (configPage10.spark2Algorithm == LOAD_SOURCE_TPS)
                {
                    multiplyTableLoad(&ignitionTable2, ignitionTable2.type_key, 4);
                }
            multiplyTableLoad(&boostTable, boostTable.type_key,
                              2);  // Boost table used 1.0 previously, so it only needs a 2x multiplier

            if (configPage6.vvtLoadSource == VVT_LOAD_TPS)
                {
                    // NOTE: The VVT tables all had 1.0 as the multiply value rather than 2.0 used in all other tables.
                    // For this reason they only need to be multiplied by 2 when updating
                ...
                {
                    // NOTE: The VVT tables all had 1.0 as the multiply value rather than 2.0 used in all other tables.
                    // For this reason they need to be divided by 2 when updating
    ...
    if (readEEPROMVersion() == 19)
        {
            ...
            if (configPage4.inj4cylPairing > INJ_PAIR_14_23)
                {
                    configPage4.inj4cylPairing = 0;
                }  // Check valid value
            if (configPage2.nCylinders == 4)
                {
                    if (configPage2.injLayout == INJ_SEQUENTIAL)
                        {
                            configPage4.inj4cylPairing = INJ_PAIR_13_24;
                        }  // Since #478 engine will always start in semi, make the sequence right for the majority of
                           // inlie 4 engines
                    else
                        {
                            configPage4.inj4cylPairing = INJ_PAIR_14_23;
                        }  // Force setting to use the default mode from previous FW versions. This is to prevent issues
                           // on any setups that have been wired accordingly
                }
            ...
            // Fill the boostTableLookupDuty with all 50% duty cycle. This is the same as the hardcoded 50% DC that had
            // been used before. This makes the boostcontrol fully backwards compatible.
    ...
    if (readEEPROMVersion() == 20)
        {
            ...
            configPage2.taeMinChange =
                4;  // Default is 2% minimum change to match prior behaviour. (4 = 2% account for 0.5 resolution)
            configPage2.maeMinChange = 2;  // Default is 2% minimum change to match prior behaviour.

            configPage2.decelAmount =
                100;  // Default decel fuel amount is 100%, so no change in fueling in decel as before.
            ...
            for (uint8_t y = 0; y < sizeof(configPage13.outputPin); y++)
                {
                    if ((configPage13.firstDataIn[y] > 22) && (configPage13.firstDataIn[y] < 240))
                        {
                            configPage13.firstDataIn[y]++;
                        }
                    if ((configPage13.firstDataIn[y] > 92) && (configPage13.firstDataIn[y] < 240))
                        {
                            configPage13.firstDataIn[y]++;
                        }
                    if ((configPage13.secondDataIn[y] > 22) && (configPage13.secondDataIn[y] < 240))
                        {
                            configPage13.secondDataIn[y]++;
                        }
                    if ((configPage13.secondDataIn[y] > 92) && (configPage13.secondDataIn[y] < 240))
                        {
                            configPage13.secondDataIn[y]++;
                        }
                }
    ...
    if (readEEPROMVersion() == 22)
        {
            ...
            if (configPage10.wmiMode >= WMI_MODE_OPENLOOP)
                {
                    multiplyTableValue(wmiMapPage, 2);
                }  // Increased PWM resolution from 0-100 to 0-200 to match VVT
    ...
    if (readEEPROMVersion() > CURRENT_DATA_VERSION)
        {
            storeEEPROMVersion(CURRENT_DATA_VERSION);
        }
}
