 ...
 * @brief
 *
 * @param buttonCommand The command number of the button that was clicked. See TS_CommendButtonHandler.h for a list of
 * button IDs
 */
    ...
    switch (buttonCommand)
        {
            ...
            case TS_CMD_INJ4_OFF:  // cmd group is for injector4 off actions
                if (BIT_CHECK(currentStatus.testOutputs, 1))
                    {
                        closeInjector4();
                        BIT_CLEAR(HWTest_INJ_Pulsed, INJ4_CMD_BIT);
                    }
                break;
            ...
            case TS_CMD_IGN1_OFF:  // cmd group is for spark1 off actions
                if (BIT_CHECK(currentStatus.testOutputs, 1))
                    {
                        endCoil1Charge();
                        BIT_CLEAR(HWTest_IGN_Pulsed, IGN1_CMD_BIT);
                    }
                break;
            ...
            case TS_CMD_IGN2_OFF:  // cmd group is for spark2 off actions
                if (BIT_CHECK(currentStatus.testOutputs, 1))
                    {
                        endCoil2Charge();
                        BIT_CLEAR(HWTest_IGN_Pulsed, IGN2_CMD_BIT);
                    }
                break;
            ...
            case TS_CMD_IGN2_PULSED:  // cmd group is for spark2 50%dc actions
                if (BIT_CHECK(currentStatus.testOutputs, 1))
                    {
                        BIT_SET(HWTest_IGN_Pulsed, IGN2_CMD_BIT);
                    }
            ...
            case TS_CMD_IGN3_OFF:  // cmd group is for spark3 off actions
                if (BIT_CHECK(currentStatus.testOutputs, 1))
                    {
                        endCoil3Charge();
                        BIT_CLEAR(HWTest_IGN_Pulsed, IGN3_CMD_BIT);
                    }
                break;
            ...
            case TS_CMD_IGN3_PULSED:  // cmd group is for spark3 50%dc actions
                if (BIT_CHECK(currentStatus.testOutputs, 1))
                    {
                        BIT_SET(HWTest_IGN_Pulsed, IGN3_CMD_BIT);
                    }
            ...
            case TS_CMD_IGN4_OFF:  // cmd group is for spark4 off actions
                if (BIT_CHECK(currentStatus.testOutputs, 1))
                    {
                        endCoil4Charge();
                        BIT_CLEAR(HWTest_IGN_Pulsed, IGN4_CMD_BIT);
                    }
                break;
            ...
            case TS_CMD_IGN4_PULSED:  // cmd group is for spark4 50%dc actions
                if (BIT_CHECK(currentStatus.testOutputs, 1))
                    {
                        BIT_SET(HWTest_IGN_Pulsed, IGN4_CMD_BIT);
                    }
            ...
            case TS_CMD_IGN5_OFF:  // cmd group is for spark5 off actions
                if (BIT_CHECK(currentStatus.testOutputs, 1))
                    {
                        endCoil5Charge();
                        BIT_CLEAR(HWTest_IGN_Pulsed, IGN5_CMD_BIT);
                    }
                break;
            ...
            case TS_CMD_IGN5_PULSED:  // cmd group is for spark4 50%dc actions
                if (BIT_CHECK(currentStatus.testOutputs, 1))
                    {
                        BIT_SET(HWTest_IGN_Pulsed, IGN5_CMD_BIT);
                    }
            ...
            case TS_CMD_IGN6_OFF:  // cmd group is for spark6 off actions
                if (BIT_CHECK(currentStatus.testOutputs, 1))
                    {
                        endCoil6Charge();
                        BIT_CLEAR(HWTest_IGN_Pulsed, IGN6_CMD_BIT);
                    }
                break;
            ...
            case TS_CMD_IGN6_PULSED:  // cmd group is for spark6 50%dc actions
                if (BIT_CHECK(currentStatus.testOutputs, 1))
                    {
                        BIT_SET(HWTest_IGN_Pulsed, IGN6_CMD_BIT);
                    }
            ...
            case TS_CMD_IGN7_OFF:  // cmd group is for spark7 off actions
                if (BIT_CHECK(currentStatus.testOutputs, 1))
                    {
                        endCoil7Charge();
                        BIT_CLEAR(HWTest_IGN_Pulsed, IGN7_CMD_BIT);
                    }
                break;
            ...
            case TS_CMD_IGN7_PULSED:  // cmd group is for spark7 50%dc actions
                if (BIT_CHECK(currentStatus.testOutputs, 1))
                    {
                        BIT_SET(HWTest_IGN_Pulsed, IGN7_CMD_BIT);
                    }
            ...
            case TS_CMD_IGN8_OFF:  // cmd group is for spark8 off actions
                if (BIT_CHECK(currentStatus.testOutputs, 1))
                    {
                        endCoil8Charge();
                        BIT_CLEAR(HWTest_IGN_Pulsed, IGN8_CMD_BIT);
                    }
                break;
            ...
            case TS_CMD_IGN8_PULSED:  // cmd group is for spark8 50%dc actions
                if (BIT_CHECK(currentStatus.testOutputs, 1))
                    {
                        BIT_SET(HWTest_IGN_Pulsed, IGN8_CMD_BIT);
                    }
            ...
            case TS_CMD_VSS_60KMH:
                {
                    if (configPage2.vssMode == 1)
                        {
                            // Calculate the ratio of VSS reading from Aux input and actual VSS (assuming that actual
                            // VSS is really 60km/h).
                            configPage2.vssPulsesPerKm = (currentStatus.canin[configPage2.vssAuxCh] / 60);
                            writeConfig(1);  // Need to manually save the new config value as it will not trigger a burn
                                             // in tunerStudio due to use of ControllerPriority
                            BIT_SET(currentStatus.status3,
                                    BIT_STATUS3_VSS_REFRESH);  // Set the flag to trigger the UI reset
                        }
                    else
                        {
                            ...
                            if (calibrationGap > 0)
                                {
                                    ...
                                    writeConfig(1);  // Need to manually save the new config value as it will not
                                                     // trigger a burn in tunerStudio due to use of ControllerPriority
                                    BIT_SET(currentStatus.status3,
                                            BIT_STATUS3_VSS_REFRESH);  // Set the flag to trigger the UI reset
                                }
                        }
                ...
                if (currentStatus.vss > 0)
                    {
                        ...
                        writeConfig(1);  // Need to manually save the new config value as it will not trigger a burn in
                                         // tunerStudio due to use of ControllerPriority
                        BIT_SET(currentStatus.status3, BIT_STATUS3_VSS_REFRESH);  // Set the flag to trigger the UI
                                                                                  // reset
                    }
                break;
