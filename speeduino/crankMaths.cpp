...
{
    noInterrupts();
    // Still uses a last interval method (ie retrospective), but bases the interval on the gap between the 2 most recent
...
{
    //********************************************************
    // How fast are we going? Need to know how long (uS) it will take to get from one tooth to the next. We then use
    // that to estimate how far we are between the last tooth and the next one We use a 1st Deriv acceleration
    // prediction, but only when there is an even spacing between primary sensor teeth Any decoder that has uneven
    // spacing has its triggerToothAngle set to 0 THIS IS CURRENTLY DISABLED FOR ALL DECODERS! It needs more work.
    if ((BIT_CHECK(decoderState, BIT_DECODER_2ND_DERIV)) && (toothHistoryIndex >= 3) &&
        (currentStatus.RPM < 2000))  // toothHistoryIndex must be greater than or equal to 3 as we need the last 3
                                     // entries. Currently this mode only runs below 3000 rpm
        {
            // Only recalculate deltaV if the tooth has changed since last time (DeltaV stays the same until the next
            // tooth) if (deltaToothCount != toothCurrentCount)
            {
                ...
                uint32_t toothDeltaV = (MICROS_PER_SEC * angle2 / toothHistory[toothHistoryIndex]) -
            ...
            timePerDegreex16 = ldiv(2666656L, currentStatus.RPM + rpmDelta)
                                   .quot;  // This gives accuracy down to 0.1 of a degree and can provide noticeably
                                           // better timing results on low resolution triggers
        }
}
#endif