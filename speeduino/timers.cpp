...
They should not be confused with Schedulers, which are for performing an action once at a given point of time in the
future

Timers are typically low resolution (Compared to Schedulers), with maximum frequency currently being approximately every
10ms
*/
#include "timers.h"
...
#endif
{
    ...
    uint32_t targetOverdwellTime =
        micros() - dwellLimit_uS;  // Set a target time in the past that all coil charging must have begun after. If the
                                   // coil charge began before this time, it's been running too long
    bool isCrankLocked =
        configPage4.ignCranklock &&
        (currentStatus.RPM <
         currentStatus.crankRPM);  // Dwell limiter is disabled during cranking on setups using the locked cranking
                                   // timing. WE HAVE to do the RPM check here as relying on the engine cranking bit can
                                   // be potentially too slow in updating
    ...
    if (tachoOutputFlag == READY)
        {
            ...
            if ((configPage2.tachoDiv == 0) || (currentStatus.tachoAlt == true))
                {
                    TACHO_PULSE_LOW();
                    // ms_counter is cast down to a byte as the tacho duration can only be in the range of 1-6, so no
                    // extra resolution above that is required
            ...
            currentStatus.tachoAlt =
                !currentStatus.tachoAlt;  // Flip the alternating value in case half speed tacho is in use.
        }
    ...
    if (loop100ms == 100)
        {
            ...
            currentStatus.rpmDOT =
                (currentStatus.RPM - lastRPM_100ms) *
                10;  // This is the RPM per second that the engine has accelerated/decelerated in the last loop
            ...
            if ((currentStatus.injPrimed == false) && (seclx10 == configPage2.primingDelay) && (currentStatus.RPM == 0))
                {
    ...
    if (loopSec == 1000)
        {
            ...
            if (BIT_CHECK(currentStatus.engine, BIT_ENGINE_RUN))
                {  // NOTE - There is a potential for a ~1sec gap between engine crank starting and the runSec number
                   // being incremented. This may delay ASE!
                    if (currentStatus.runSecs <=
                        254)  // Ensure we cap out at 255 and don't overflow. (which would reset ASE and cause problems
                              // with the closed loop fuelling (Which has to wait for the O2 to warmup))
                        {
            ...
            mainLoopCount = 0;
            //**************************************************************************************************************************************************
            // increment secl (secl is simply a counter that increments every second and is used to track whether the
            // system has unexpectedly reset
            currentStatus.secl++;
            //**************************************************************************************************************************************************
            ...
            if (currentStatus.fpPrimed == false)
                {
                    // fpPrimeTime is the time that the pump priming started. This is 0 on startup, but can be changed
                    // if the unit has been running on USB power and then had the ignition turned on (Which starts the
                    // priming again)
                    if ((currentStatus.secl - fpPrimeTime) >= configPage2.fpPrime)
                        {
                            ...
                            if (currentStatus.RPM == 0)
                                {
                                    // If we reach here then the priming is complete, however only turn off the fuel
                                    // pump if the engine isn't running
            ...
            //**************************************************************************************************************************************************
            // Set the flex reading (if enabled). The flexCounter is updated with every pulse from the sensor. If
            // cleared once per second, we get a frequency reading
            if (configPage2.flexEnabled == true)
                {
                    byte tempEthPct = 0;
                    if (flexCounter < 50)
                        {
                            tempEthPct = 0;  // Standard GM Continental sensor reads from 50Hz (0 ethanol) to 150Hz
                                             // (Pure ethanol). Subtracting 50 from the frequency therefore gives the
                                             // ethanol percentage.
                            flexCounter = 0;
                        }
                    else if (flexCounter > 151)  // 1 pulse buffer
                        {
                            if (flexCounter < 169)
                                ...
                                {
                                    // This indicates an error condition. Spec of the sensor is that errors are above
                                    // 170Hz)
                                    tempEthPct = 0;
                                    flexCounter = 0;
                                }
                        ...
                        {
                            tempEthPct = flexCounter - 50;  // Standard GM Continental sensor reads from 50Hz (0
                                                            // ethanol) to 150Hz (Pure ethanol). Subtracting 50 from the
                                                            // frequency therefore gives the ethanol percentage.
                            flexCounter = 0;
                        }
                    ...
                    // Continental flex sensor fuel temperature can be read with following formula: (Temperature =
                    // (41.25 * pulse width(ms)) - 81.25). 1000μs = -40C and 5000μs = 125C
