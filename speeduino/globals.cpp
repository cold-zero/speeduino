/** @file
 * Instantiation of various (table2D, table3D) tables, volatile (interrupt modified) variables, Injector (1...8)
 * enablement flags, etc.
 */
#include "globals.h"
...
/// volatile inj*_pin_port and  inj*_pin_mask vars are for the direct port manipulation of the injectors, coils and aux
/// outputs.
...
volatile uint16_t mainLoopCount;  // Main loop counter (incremented at each main loop rev., used for maintaining
                                  // currentStatus.loopsPerSecond)
uint32_t revolutionTime;  // The time in uS that one revolution would take at current speed (The time tooth 1 was last
                          // seen, minus the time it was seen prior to that)
volatile unsigned long timer5_overflow_count =
    0;  // Increments every time counter 5 overflows. Used for the fast version of micros()
volatile unsigned long ms_counter = 0;  // A counter that increments once per ms
uint16_t fixedCrankingOverride = 0;
bool clutchTrigger;
bool previousClutchTrigger;
volatile uint32_t toothHistory[TOOTH_LOG_SIZE];  ///< Tooth trigger history - delta time (in uS) from last tooth
                                                 ///< (Indexed by @ref toothHistoryIndex)
...
int CRANK_ANGLE_MAX_INJ =
    360;  ///< The number of crank degrees that the system track over. Typically 720 divided by the number of squirts
          ///< per cycle (Eg 360 for wasted 2 squirt and 720 for sequential single squirt)
volatile uint32_t runSecsX10;
volatile uint32_t seclx10;
volatile byte HWTest_INJ =
    0; /**< Each bit in this variable represents one of the injector channels and it's HW test status */
volatile byte HWTest_INJ_Pulsed =
    0; /**< Each bit in this variable represents one of the injector channels and it's pulsed HW test status */
volatile byte HWTest_IGN =
    0; /**< Each bit in this variable represents one of the ignition channels and it's HW test status */
...
byte injectorOutputControl =
    OUTPUT_CONTROL_DIRECT; /**< Specifies whether the injectors are controlled directly (Via an IO pin)
...
byte ignitionOutputControl =
    OUTPUT_CONTROL_DIRECT; /**< Specifies whether the coils are controlled directly (Via an IO pin)
or using something like the MC33810. 0 = Direct (OUTPUT_CONTROL_DIRECT), 10 = MC33810 (OUTPUT_CONTROL_MC33810) */
...
struct statuses currentStatus; /**< The master global "live" status struct. Contains all values that are updated
                                  frequently and used across modules */
...
// byte iatCalibrationTable[CALIBRATION_TABLE_SIZE]; /**< An array containing the inlet air temperature sensor
// calibration values */ byte o2CalibrationTable[CALIBRATION_TABLE_SIZE]; /**< An array containing the O2 sensor
// calibration values */
...
bool pinIsOutput(byte pin)
{
    bool used = false;
    bool isIdlePWM =
        (configPage6.iacAlgorithm > 0) && ((configPage6.iacAlgorithm <= 3) || (configPage6.iacAlgorithm == 6));
    bool isIdleSteper = (configPage6.iacAlgorithm > 3) && (configPage6.iacAlgorithm != 6);
    // Injector?
    if ((pin == pinInjector1) || ((pin == pinInjector2) && (configPage2.nInjectors > 1)) ||
    ...
    // Ignition?
    if ((pin == pinCoil1) || ((pin == pinCoil2) && (maxIgnOutputs > 1)) || ((pin == pinCoil3) && (maxIgnOutputs > 2)) ||
        ((pin == pinCoil4) && (maxIgnOutputs > 3)) || ((pin == pinCoil5) && (maxIgnOutputs > 4)) ||
        ((pin == pinCoil6) && (maxIgnOutputs > 5)) || ((pin == pinCoil7) && (maxIgnOutputs > 6)) ||
        ((pin == pinCoil8) && (maxIgnOutputs > 7)))
        {
            used = true;
        }
    // Functions?
    if ((pin == pinFuelPump) || ((pin == pinFan) && (configPage2.fanEnable == 1)) ||
        ((pin == pinVVT_1) && (configPage6.vvtEnabled > 0)) || ((pin == pinVVT_1) && (configPage10.wmiEnabled > 0)) ||
        ((pin == pinVVT_2) && (configPage10.vvt2Enabled > 0)) ||
        ((pin == pinBoost) && (configPage6.boostEnabled == 1)) || ((pin == pinIdle1) && isIdlePWM) ||
        ((pin == pinIdle2) && isIdlePWM && (configPage6.iacChannels == 1)) ||
        ((pin == pinStepperEnable) && isIdleSteper) || ((pin == pinStepperStep) && isIdleSteper) ||
        ((pin == pinStepperDir) && isIdleSteper) || (pin == pinTachOut) ||
