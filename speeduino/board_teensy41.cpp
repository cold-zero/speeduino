...
void initBoard()
{
    ...
    */
    ...
    boost_pwm_max_count =
        (uint16_t)(MICROS_PER_SEC / (2U * configPage6.boostFreq *
                                     2U));  // Converts the frequency in Hz to the number of ticks (at 2uS) it takes to
    ...
    vvt_pwm_max_count =
        (uint16_t)(MICROS_PER_SEC /
                   (2U * configPage6.vvtFreq *
                    2U));  // Converts the frequency in Hz to the number of ticks (at 2uS) it takes to complete 1 cycle
    ...
    */
    // Use the Quad timer
    // Uses the BUS clock speed, which is 1/4 of the CPU clock. Maximum prescaler of 128 is used to give a 0.853333uS
    // tick time @ 600Mhz TMR1 - Fuel 1-4
    // 0
    TMR1_CTRL0 = 0;
    TMR1_CSCTRL0 = 0;
...
void setTriggerHysteresis()
{
    ...
    const uint32_t padConfig =
        IOMUXC_PAD_DSE(1) | IOMUXC_PAD_PKE | IOMUXC_PAD_PUE | IOMUXC_PAD_SPEED(0) | IOMUXC_PAD_HYS;
