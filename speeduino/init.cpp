...
*/
    ...
    flexBoostTable.axisSize =
        SIZE_BYTE;  // Set this table to use byte axis bins (NOTE THIS IS DIFFERENT TO THE VALUES!!)
    ...
    if ((configPage2.pinMapping == 255) ||
        (configPage2.pinMapping == 0))  // 255 = EEPROM value in a blank AVR; 0 = EEPROM value in new FRAM
        {
...
// Must come after setPinMapping() as secondary serial can be changed on a per board basis
    ...
    BIT_CLEAR(currentStatus.engineProtectStatus, PROTECT_IO_ERROR);  // Clear the I/O error bit. The bit will be set in
    ...
    req_fuel_uS =
        configPage2.reqFuel * 100;  // Convert to uS and an int. This is the only variable to be used in calculations
    ...
    if (configPage10.stagingEnabled == true)
        {
            ...
            /*
                These values are a percentage of the req_fuel value that would be required for each injector channel to
               deliver that much fuel. Eg: Pri injectors are 250cc Sec injectors are 500cc Total injector capacity =
               750cc

                staged_req_fuel_mult_pri = 300% (The primary injectors would have to run 3x the overall PW in order to
               be the equivalent of the full 750cc capacity staged_req_fuel_mult_sec = 150% (The secondary injectors
               would have to run 1.5x the overall PW in order to be the equivalent of the full 750cc capacity
            */
    ...
    if (configPage4.trigPatternSec == SEC_TRIGGER_POLL && configPage4.TrigPattern == DECODER_MISSING_TOOTH)
        ...
        }  // set the secondary trigger edge automatically to correct working value with poll level mode to enable cam
           // angle detection in closed loop vvt.
    // Explanation: currently cam trigger for VVT is only captured when revolution one == 1. So we need to make sure
    // that the edge trigger happens on the first revolution. So now when we set the poll level to be low on revolution
    // one and it's checked at tooth #1. This means that the cam signal needs to go high during the first revolution to
    // be high on next revolution at tooth #1. So poll level low = cam trigger edge rising.

    // Begin the main crank trigger interrupt pin setup
    // The interrupt numbering is a bit odd - See here for reference: arduino.cc/en/Reference/AttachInterrupt
    // These assignments are based on the Arduino Mega AND VARY BETWEEN BOARDS. Please confirm the board you are using
    ...
    currentStatus.crankRPM =
        ((unsigned int)configPage4.crankRPM * 10);  // Crank RPM limit (Saves us calculating this over and over again.
                                                    // It's updated once per second in timers.ino)
    currentStatus.fuelPumpOn = false;
    currentStatus.engineProtectStatus = 0;
    triggerFilterTime =
        0;  // Trigger filter time is the shortest possible time (in uS) that there can be between crank teeth (ie at
            // max RPM). Any pulses that occur faster than this time will be discarded as noise. This is simply a
            // default value, the actual values are set in the setup() functions of each decoder
    dwellLimit_uS = (1000 * configPage4.dwellLimit);
    currentStatus.nChannels =
        ((uint8_t)INJ_CHANNELS << 4) + IGN_CHANNELS;  // First 4 bits store the number of injection channels, 2nd 4
                                                      // store the number of ignition channels
    fpPrimeTime = 0;
    ms_counter = 0;
    ...
    // The secondary input can be used for VSS if nothing else requires it. Allows for the standard VR conditioner to be
    // used for VSS. This MUST be run after the initialiseTriggers() function
    if (VSS_USES_RPM2())
        {
            attachInterrupt(digitalPinToInterrupt(pinVSS), vssPulse, RISING);
    ...
    if (configPage2.strokes == FOUR_STROKE)
        {
            // Default is 1 squirt per revolution, so we halve the given req-fuel figure (Which would be over 2
            // revolutions)
            req_fuel_uS = req_fuel_uS /
                          2;  // The req_fuel calculation above gives the total required fuel (At VE 100%) in the full
                              // cycle. If we're doing more than 1 squirt per cycle then we need to split the amount
                              // accordingly. (Note that in a non-sequential 4-stroke setup you cannot have less than 2
    ...
    if (configPage2.divider == 0)
        ...
        }  // The number of squirts being requested. This is manually overridden below for sequential setups (Due to TS
           // req_fuel calc limitations)
    if (currentStatus.nSquirts == 0)
        {
            currentStatus.nSquirts = 1;
        }  // Safety check. Should never happen as TS will give an error, but leave in case tune is manually altered
           // etc.
    ...
    switch (configPage2.nCylinders)
        {
            ...
            case 2:
                ...
                // Sequential ignition works identically on a 2 cylinder whether it's odd or even fire (With the default
                // being a 180 degree second cylinder).
                if ((configPage4.sparkMode == IGN_MODE_SEQUENTIAL) && (configPage2.strokes == FOUR_STROKE))
                    {
                        CRANK_ANGLE_MAX_IGN = 720;
                    }
            ...
            case 3:
                ...
                if (configPage2.engineType == EVEN_FIRE)
                    {
                        // Sequential and Single channel modes both run over 720 crank degrees, but only on 4 stroke
                        // engines.
                ...
                if ((configPage2.injLayout == INJ_SEMISEQUENTIAL) || (configPage2.injLayout == INJ_PAIRED))
                    {
                        ...
                        if (configPage2.injType == INJ_TYPE_PORT)
                            {
                                // Force nSquirts to 2 for individual port injection. This prevents TunerStudio forcing
                                // the value to 3 even when this isn't wanted.
                                currentStatus.nSquirts = 2;
                                if (configPage2.strokes == FOUR_STROKE)
                                    {
                                        CRANK_ANGLE_MAX_INJ = 360;
                ...
                if (configPage2.engineType == EVEN_FIRE)
                    {
                        ...
                        if (configPage4.sparkMode == IGN_MODE_ROTARY)
                            {
                                // Rotary uses the ign 3 and 4 schedules for the trailing spark. They are offset from
                                // the ign 1 and 2 channels respectively and so use the same degrees as them
                                channel3IgnDegrees = 0;
                                channel4IgnDegrees = 180;
                                maxIgnOutputs = 4;

                                configPage4.IgInv =
                                    GOING_LOW;  // Force Going Low ignition mode (Going high is never used for rotary)
                            }
                    }
                else
                    {
...
// Staging with 6 cylinders semi/sequential requires 7 total channels
                                ...
                                // staging No staging output will be active
                                maxInjOutputs = 6;
#endif
                            }
                    }
    ...
    currentStatus.status3 |=
        ...
        << BIT_STATUS3_NSQUIRTS1;  // Top 3 bits of the status3 variable are the number of squirts. This must be done
                                   // after the above section due to nSquirts being forced to 1 for sequential
                ...
                if (configPage2.nCylinders == 4)
                    {
                        if (configPage4.inj4cylPairing == INJ_PAIR_13_24)
                         ...
                         5)  // This is similar to the paired injection but uses five injector outputs instead of four
                    {
                        fuelSchedule1.pStartFunction = openInjector1;
                        fuelSchedule1.pEndFunction = closeInjector1;
                        fuelSchedule2.pStartFunction = openInjector2;
    ...
    switch (configPage4.sparkMode)
        {
            ...
            case IGN_MODE_WASTEDCOP:
                ...
                if (configPage2.nCylinders <= 3)
                    ...
                    {
                        // If the person has inadvertently selected this when running more than 4 cylinders or other
                        // than 6 cylinders, just use standard Wasted spark mode
                        ignitionSchedule1.pStartCallback = beginCoil1Charge;
                        ignitionSchedule1.pEndCallback = endCoil1Charge;
                        ignitionSchedule2.pStartCallback = beginCoil2Charge;
                        ignitionSchedule2.pEndCallback = endCoil2Charge;
    ...
    interrupts();
    readCLT(false);  // Need to read coolant temp to make priming pulsewidth work correctly. The false here disables use
                     // of the filter
    ...
    /* SweepMax is stored as a byte, RPM/100. divide by 60 to convert min to sec (net 5/3).  Multiply by ignition pulses
       per rev. tachoSweepIncr is also the number of tach pulses per second */
 ...
 * The boardID is switch-case compared against raw boardID integers (not enum or defined label, and probably no need for
 * that either) which are originated from tuning SW (e.g. TS) set values and are available in reference/speeduino.ini
 * (See pinLayout, note also that numbering is not contiguous here).
 */
...
// This is NOT correct. It has not yet been tested with this board
            ...
            case 9:
                ...
                // Note that there is no stepper driver output on the PNP boards. These pins are unconnected and remain
                // here just to prevent issues with random pin numbers occurring
...
// Pin mappings for the DIY-EFI CORE4 Module. This is an AVR only module
            ...
            case 55:
                ...
                // The injector pins below are not used directly as the control is via SPI through the MC33810s, however
                // the pin numbers are set to be the SPI pins (SCLK, MOSI, MISO and CS) so that nothing else will set
                // them as inputs
                ...
                */
                ...
                pinTrigger = 19;   // The CAS pin
                pinTrigger2 = 18;  // The Cam Sensor pin
                pinTrigger3 = 22;  // Uses one of the protected spare digital inputs. This must be set or Serial1 (Pin
                                   // 0) gets broken
    ...
    // This is a legacy mode option to revert the MAP reading behaviour to match what was in place prior to the 201905
    // firmware
...
// And for inputs
    ...
    // Each of the below are only set when their relevant function is enabled. This can help prevent pin conflicts that
    // users aren't aware of with unused functions
    if ((configPage2.flexEnabled > 0) && (!pinIsOutput(pinFlex)))
        {
            pinMode(pinFlex, INPUT);  // Standard GM / Continental flex sensor requires pullup, but this should be
                                      // onboard. The internal pullup will not work (Requires ~3.3k)!
        }
 ...
 * - Set Interrupt numbers @ref triggerInterrupt, @ref triggerInterrupt2 and @ref triggerInterrupt3  by pin their
 * numbers (based on board CORE_* define)
 * - Call decoder specific setup function triggerSetup_*() (by @ref config4.TrigPattern, set to one of the DECODER_*
 * defines) and do any additional initialisations needed.
 *
 * @todo Explain why triggerSetup_*() alone cannot do all the setup, but there's ~10+ lines worth of extra init for each
 * of decoders.
 */
void initialiseTriggers(void)
{
    ...
    tertiaryTriggerEdge = 0;   // This is even more optional and may not be changed below, depending on the decoder in
                              // use
    ...
    switch (configPage4.TrigPattern)
        {
            ...
            case DECODER_NON360:
                triggerSetup_non360();
                triggerHandler = triggerPri_DualWheel;  // Is identical to the dual wheel decoder, so that is used. Same
                                                        // goes for the secondary below
                triggerSecondaryHandler = triggerSec_DualWheel;  // Note the use of the Dual Wheel trigger function
                                                                 // here. No point in having the same code in twice.
            ...
            case DECODER_36_2_2_2:
                ...
                getCrankAngle = getCrankAngle_missingTooth;  // This uses the same function as the missing tooth
                                                             // decoder, so no need to duplicate code
            ...
            case DECODER_VMAX:
                ...
                attachInterrupt(triggerInterrupt, triggerHandler,
                                CHANGE);  // Hardcoded change, the primaryTriggerEdge will be used in the decoder to
            ...
            case DECODER_SUZUKI_K6A:
                ...
                triggerHandler =
                    triggerPri_SuzukiK6A;  // only primary, no secondary, trigger pattern is over 720 degrees
...
{
    return fuelSchedule1.Status == RUNNING || fuelSchedule2.Status == RUNNING || fuelSchedule3.Status == RUNNING ||
