/** @file
 * Global defines, macros, struct definitions (@ref statuses, @ref config2, @ref config4, config*), extern-definitions
 * (for globally accessible vars).
 *
 * ### Note on configuration struct layouts
 *
 * Once the struct members have been assigned to certain "role" (in certain SW version), they should not be "moved
 * around" as the structs are stored onto EEPROM as-is and the offset and size of member needs to remain constant. Also
 * removing existing struct members would disturb layouts. Because of this a certain amount unused old members will be
 * left into the structs. For the storage related reasons also the bit fields are defined in byte-size (or multiple of
 * ...) chunks.
 *
 * ### Config Structs and 2D, 3D Tables
 *
 * The config* structures contain information coming from tuning SW (e.g. TS) for 2D and 3D tables, where looked up
 * value is not a result of direct array lookup, but from interpolation algorithm. Because of standard, reusable
 * interpolation routines associated with structs table2D and table3D, the values from config are copied from config*
 * structs to table2D (table3D destined configurations are not stored in config* structures).
 *
 * ### Board choice
 * There's a C-preprocessor based "#if defined" logic present in this header file based on the Arduino IDE compiler set
 * CPU
...
// This is a workaround to avoid having to change all the references to higher ADC channels. We simply define the
// channels (Which don't exist on the 2561) as being the same as A0-A7 These Analog inputs should never be used on any
// 2561 board definition (Because they don't exist on the MCU), so it will not cause any issues
#define A8 A0
#define A9 A1
#define A10 A2
#define A11 A3
...
#define BIT_AIRCON_RPM_LOCKOUT \
    2  // Indicates the A/C is locked out due to the RPM being too high/low, or the post-high/post-low-RPM "stand-down"
       // lockout period
#define BIT_AIRCON_TPS_LOCKOUT \
    3  // Indicates the A/C is locked out due to high TPS, or the post-high-TPS "stand-down" lockout period
#define BIT_AIRCON_TURNING_ON \
    4  // Indicates the A/C request is on (i.e. A/C button pressed), the lockouts are off, however the start delay has
       // not yet elapsed. This gives the idle up time to kick in before the compressor.
...
// note the sequence of these defines which reference the bits used in a byte has moved when the third trigger & engine
...
#define CALIBRATION_TEMPERATURE_OFFSET                                                           \
    40                        /**< All temperature measurements are stored offset by 40 degrees. \
...
#define OFFSET_IGNITION \
    40  ///< Ignition values from the main spark table are offset 40 degrees downwards to allow for negative spark
        ///< timing

#define SERIAL_BUFFER_THRESHOLD \
    32  ///< When the serial buffer is filled to greater than this threshold value, the serial processing operations
        ///< will be performed more urgently in order to avoid it overflowing. Serial buffer is 64 bytes long, so the
        ///< threshold is set at half this as a reasonable figure
...
extern const byte data_structure_version;  // This identifies the data structure when reading / writing. Now in use:
                                           // CURRENT_DATA_VERSION (migration on-the fly) ?
...
extern unsigned long revolutionTime;  // The time in uS that one revolution would take at current speed (The time tooth
                                      // 1 was last seen, minus the time it was seen prior to that)
extern volatile unsigned long
    timer5_overflow_count;  // Increments every time counter 5 overflows. Used for the fast version of micros()
...
extern volatile uint16_t
    ignitionCount; /**< The count of ignition events that have taken place since the engine started */
// The below shouldn't be needed and probably should be cleaned up, but the Atmel SAM (ARM) boards use a specific type
// for the trigger edge values rather than a simple byte/int
...
extern int CRANK_ANGLE_MAX_INJ;      ///< The number of crank degrees that the system track over. 360 for wasted / timed
                                     ///< batch and 720 for sequential
extern volatile uint32_t runSecsX10; /**< Counter of seconds since cranking commenced (similar to runSecs) but in
                                        increments of 0.1 seconds */
extern volatile uint32_t
    seclx10; /**< Counter of seconds since powered commenced (similar to secl) but in increments of 0.1 seconds */
extern volatile byte
    HWTest_INJ; /**< Each bit in this variable represents one of the injector channels and it's HW test status */
extern volatile byte HWTest_INJ_Pulsed; /**< Each bit in this variable represents one of the injector channels and it's
                                           50% HW test status */
extern volatile byte
    HWTest_IGN; /**< Each bit in this variable represents one of the ignition channels and it's HW test status */
extern volatile byte HWTest_IGN_Pulsed; /**< Each bit in this variable represents one of the ignition channels and it's
                                           50% HW test status */
...
extern byte resetControl;  ///< resetControl needs to be here (as global) because using the config page (4) directly can
                           ///< prevent burning the setting
...
#define pinIsInjector(pin)                                                                                       \
    (((pin) == pinInjector1) || ((pin) == pinInjector2) || ((pin) == pinInjector3) || ((pin) == pinInjector4) || \
     ((pin) == pinInjector5) || ((pin) == pinInjector6) || ((pin) == pinInjector7) || ((pin) == pinInjector8))
#define pinIsIgnition(pin)                                                                       \
    (((pin) == pinCoil1) || ((pin) == pinCoil2) || ((pin) == pinCoil3) || ((pin) == pinCoil4) || \
     ((pin) == pinCoil5) || ((pin) == pinCoil6) || ((pin) == pinCoil7) || ((pin) == pinCoil8))
//#define pinIsOutput(pin)    ( pinIsInjector((pin)) || pinIsIgnition((pin)) || ((pin) == pinFuelPump) || ((pin) ==
//pinFan) || ((pin) == pinAirConComp) || ((pin) == pinAirConFan)|| ((pin) == pinVVT_1) || ((pin) == pinVVT_2) || (
//((pin) == pinBoost) && configPage6.boostEnabled) || ((pin) == pinIdle1) || ((pin) == pinIdle2) || ((pin) ==
//pinTachOut) || ((pin) == pinStepperEnable) || ((pin) == pinStepperStep) )
#define pinIsSensor(pin)                                                                                     \
    (((pin) == pinCLT) || ((pin) == pinIAT) || ((pin) == pinMAP) || ((pin) == pinTPS) || ((pin) == pinO2) || \
     ((pin) == pinBat) || (((pin) == pinFlex) && (configPage2.flexEnabled != 0)))
...
{
    volatile bool
        hasSync : 1;                  /**< Flag for crank/cam position being known by decoders (See decoders.ino).
    ...
    uint16_t EMAPADC;
    byte baro;      ///< Barometric pressure is simply the initial MAP reading, taken before the engine is running.
                    ///< Alternatively, can be taken from an external sensor
    byte TPS;       /**< The current TPS reading (0% - 100%). Is the tpsADC value after the calibration is applied */
    byte tpsADC;    /**< byte (valued: 0-255) representation of the TPS. Downsampled from the original 10-bit (0-1023)
                       reading, but before any calibration is applied */
    int16_t tpsDOT; /**< TPS delta over time. Measures the % per second that the TPS is changing. Note that is signed
                       value, because TPSdot can be also negative */
    byte TPSlast;   /**< The previous TPS reading */
    int16_t mapDOT; /**< MAP delta over time. Measures the kpa per second that the MAP is changing. Note that is signed
                       value, because MAPdot can be also negative */
    volatile int rpmDOT; /**< RPM delta over time (RPM increase / s ?) */
    byte VE;     /**< The current VE value being used in the fuel calculation. Can be the same as VE1 or VE2, or a
                    calculated value of both. */
    ...
    int8_t advance;  /**< The current advance value being used in the spark calculation. Can be the same as advance1 or
                        advance2, or a calculated value of both */
    ...
    uint16_t
        AEamount; /**< The amount of acceleration enrichment currently being applied. 100=No change. Varies above 255 */
    ...
    int8_t flexIgnCorrection; /**< Amount of additional advance being applied based on flex. Note the type as this
    ...
    volatile byte spark;     ///< Spark status/control indicator bits (launch control, boost cut, spark errors, See
                             ///< BIT_SPARK_* defines)
    ...
    volatile byte secl; /**< Counter incrementing once per second. Will overflow after 255 and begin again. This is used
                           by TunerStudio to maintain comms sync */
    volatile uint32_t loopsPerSecond; /**< A performance indicator showing the number of main loops that are being
    ...
    uint16_t freeRAM;
    unsigned int clutchEngagedRPM; /**< The RPM at which the clutch was last depressed. Used for distinguishing between
                                      launch control and flat shift */
    bool flatShiftingHard;
    volatile uint32_t
        startRevolutions; /**< A counter for how many revolutions have been completed since sync was achieved. */
    ...
    uint16_t crankRPM =
        400; /**< The actual cranking RPM limit. This is derived from the value in the config page, but saves us
                multiplying it every time it's used (Config page value is stored divided by 10) */
    ...
    byte compositeTriggerUsed;  // 0 means composite logger disabled, 2 means use secondary input (1st cam), 3 means use
                                // tertiary input (2nd cam), 4 means log both cams together
...
struct config2
{
    ...
    byte aeMode : 2; /**< Acceleration Enrichment mode. 0 = TPS, 1 = MAP. Values 2 and 3 reserved for potential future
                        use (ie blended TPS / MAP) */
    byte battVCorMode : 1;
    ...
    byte multiplyMAP : 2;  ///< MAP value processing: 0 = off, 1 = div by currentStatus.baro, 2 = div by 100 (to gain
                           ///< usable value)
    ...
    byte tachoPin : 6;  ///< Custom pin setting for tacho output (if != 0, override copied to pinTachOut, which defaults
                        ///< to board assigned tach pin)
    ...
    // config1 in ini
    byte mapSample : 2;   ///< MAP sampling method (0=Instantaneous, 1=Cycle Average, 2=Cycle Minimum, 4=Ign. event
                          ///< average, See sensors.ino)
    ...
    // config2 in ini
    byte fuelAlgorithm : 3;  ///< Fuel algorithm - 0=Manifold pressure/MAP (LOAD_SOURCE_MAP, default, proven),
                             ///< 1=Throttle/TPS (LOAD_SOURCE_TPS), 2=IMAP/EMAP (LOAD_SOURCE_IMAPEMAP)
    byte fixAngEnable : 1;   ///< Whether fixed/locked timing is enabled (0=disable, 1=enable, See @ref
                             ///< configPage4.FixAng)
    ...
    byte injLayout : 2; /**< Injector Layout - 0=INJ_PAIRED (number outputs == number cyls/2, timed over 1 crank rev),
                         1=INJ_SEMISEQUENTIAL (like paired, but number outputs == number cyls, only for 4 cyl),
                         2=INJ_BANKED (2 outputs are used), 3=INJ_SEQUENTIAL (number outputs == number cyls, timed over
                         full cycle, 2 crank revs) */
    ...
    byte fanWhenOff : 1;  ///< Allow running fan with engine off: 0 = Only run fan when engine is running, 1 = Allow
                          ///< even with engine off
    byte fanWhenCranking : 1;  ///< Set whether the fan output will stay on when the engine is cranking (0=force off,
                               ///< 1=allow on)
    byte useDwellMap : 1;      ///< Setting to change between fixed dwell value and dwell map (0=Fixed value from @ref
                               ///< configPage4.dwellRun, 1=Use @ref dwellTable)
    ...
    byte aseCount[4];    ///< Afterstart enrichment cycles. This is the number of ignition cycles that the afterstart
                         ///< enrichment % lasts for
...
struct config4
{
    ...
    int8_t FixAng;    ///< Fixed Ignition angle value (enabled by @ref configPage2.fixAngEnable, copied to ignFixValue,
                      ///< Negative values allowed, See corrections.ino)
    ...
    byte TrigEdge : 1;     ///< Primary (RPM1) Trigger Edge - 0 - RISING, 1 = FALLING (Copied from this config to
                           ///< primaryTriggerEdge)
    byte TrigSpeed : 1;    ///< Primary (RPM1) Trigger speed - 0 = crank speed (CRANK_SPEED), 1 = cam speed (CAM_SPEED),
                           ///< See decoders.ino
    byte IgInv : 1;        ///< Ignition signal invert (?) (GOING_LOW=0 (default by init.ino) / GOING_HIGH=1 )
    byte TrigPattern : 5;  ///< Decoder configured (DECODER_MISSING_TOOTH, DECODER_BASIC_DISTRIBUTOR, DECODER_GM7X, ...
                           ///< See init.ino)

    byte TrigEdgeSec : 1;  ///< Secondary (RPM2) Trigger Edge (See RPM1)
    byte fuelPumpPin : 6;  ///< Fuel pump pin (copied as override to pinFuelPump, defaults to board default, See:
                           ///< init.ino)
    byte useResync : 1;

    byte sparkDur;            ///< Spark duration in ms * 10
    byte trigPatternSec : 7;  ///< Mode for Missing tooth secondary trigger - 0=single tooth cam wheel
                              ///< (SEC_TRIGGER_SINGLE), 1=4-1 (SEC_TRIGGER_4_1) or 2=poll level mode (SEC_TRIGGER_POLL)
    byte PollLevelPolarity : 1;  // for poll level cam trigger. Sets if the cam trigger is supposed to be high or low
                                 // for revolution one.
    uint8_t bootloaderCaps;  // Capabilities of the bootloader over stock. e.g., 0=Stock, 1=Reset protection, etc.

    byte resetControlConfig : 2; /** Which method of reset control to use - 0=Disabled (RESET_CONTROL_DISABLED),
       1=Prevent When Running (RESET_CONTROL_PREVENT_WHEN_RUNNING), 2=Prevent Always (RESET_CONTROL_PREVENT_ALWAYS),
       3=Serial Command (RESET_CONTROL_SERIAL_COMMAND) - Copied to resetControl (See init.ino, utilities.ino) */
    ...
    byte sparkMode : 3;      /** Ignition/Spark output mode - 0=Wasted spark (IGN_MODE_WASTED), 1=single channel
             (IGN_MODE_SINGLE),      2=Wasted COP (IGN_MODE_WASTEDCOP), 3=Sequential (IGN_MODE_SEQUENTIAL), 4=Rotary
             (IGN_MODE_ROTARY) */
    byte triggerFilter : 2;  // The mode of trigger filter being used (0=Off, 1=Light (Not currently used), 2=Normal,
                             // 3=Aggressive)
    byte ignCranklock : 1;  // Whether or not the ignition timing during cranking is locked to a CAS (crank) pulse. Only
                            // currently valid for Basic distributor and 4G63.
    ...
    byte floodClear;  ///< TPS (raw adc count? % ?) value that triggers flood clear mode (No fuel whilst cranking, See
                      ///< @ref correctionFloodClear())
    ...
    byte taeValues[4];   ///< TPS based acceleration enrichment rates (Unit: % to add), values matched to thresholds of
                         ///< taeBins
    ...
    uint8_t ADCFILTER_MAP;  // This is only used on Instantaneous MAP readings and is intentionally very weak to allow
                            // for faster response
    uint8_t ADCFILTER_BARO;

    byte cltAdvBins[6];   /**< Coolant Temp timing advance curve bins */
    byte cltAdvValues[6]; /**< Coolant timing advance curve values. These are translated by 15 to allow for negative
                             values */
...
struct config6
{
    ...
    byte boostLimit;  ///< Boost limit (Kpa). Stored value is actual (kPa) value divided by 2, allowing kPa values up to
                      ///< 511
    byte boostKP;
    byte boostKI;
    byte boostKD;
    ...
    byte iacAlgorithm : 3;  // Valid values are: "None", "On/Off", "PWM", "PWM Closed Loop", "Stepper", "Stepper Closed
                            // Loop"
    ...
    byte iacPWMdir : 1;  // Direction of the PWM valve. 0 = Normal = Higher RPM with more duty. 1 = Reverse = Lower RPM
                         // with more duty
...
struct config9
{
    ...
    byte secondarySerialProtocol : 4;  // protocol for secondary serial. 0=Generic (Fixed list), 1=Generic (ini based
                                       // list), 2=CAN, 3=msDroid, 4=Real Dash
    byte unused9_0 : 1;
    ...
    byte iacMaxSteps;  // Step limit beyond which the stepper won't be driven. Should always be less than homing steps.
                       // Stored div 3 as per home steps.
...
struct config10
{
    ...
    int16_t flexBoostAdj[6];   // kPa to be added to the boost target @ current ethanol (negative values allowed). Bytes
                              // 39-50
    ...
    uint8_t flexAdvAdj[6];  // Additional advance (in degrees) @ current ethanol (typically 0 @ 0%, 10-20 @ 100%). NOTE:
                            // THIS SHOULD BE A SIGNED VALUE BUT 2d TABLE LOOKUP NOT WORKING WITH IT CURRENTLY! And
                            // another three corn rows die. Bytes 69-74
...
struct cmpOperation
{
    ...
    uint8_t
        secondCompType : 3;  ///< Second cmp. op (0=COMPARATOR_EQUAL,
                             ///< 1=COMPARATOR_NOT_EQUAL,2=COMPARATOR_GREATER,3=COMPARATOR_GREATER_EQUAL,4=COMPARATOR_LESS,5=COMPARATOR_LESS_EQUAL,6=COMPARATOR_CHANGE)
...
struct config13
{
    ...
    uint8_t
        outputPin[8];  ///< Disable(0) or enable (set to valid pin number) Programmable Pin (output/target pin to set)
    ...
    byte onboard_log_storage : 2;  // "sd-card", "INVALID", "INVALID", "INVALID" ;In the future maybe an onboard spi
                                   // flash can be used, or switch between SDIO vs SPI sd card interfaces.
...
extern byte injectorOutputControl;  // Specifies whether the injectors are controlled directly (Via an IO pin) or using
                                    // something like the MC33810
...
extern byte ignitionOutputControl;  // Specifies whether the coils are controlled directly (Via an IO pin) or using
                                    // something like the MC33810
...
// extern byte cltCalibrationTable[CALIBRATION_TABLE_SIZE]; /**< An array containing the coolant sensor calibration
// values */ extern byte iatCalibrationTable[CALIBRATION_TABLE_SIZE]; /**< An array containing the inlet air temperature
// sensor calibration values */ extern byte o2CalibrationTable[CALIBRATION_TABLE_SIZE]; /**< An array containing the O2
...
extern struct table2D
    cltCalibrationTable; /**< A 32 bin array containing the coolant temperature sensor calibration values */
extern struct table2D
    iatCalibrationTable; /**< A 32 bin array containing the inlet air temperature sensor calibration values */
