...
constexpr const char* header_table[] PROGMEM = {
    header_0,  header_1,  header_2,  header_3,  header_4,  header_5,  header_6,  header_7,  header_8,  header_9,
    header_10, header_11, header_12, header_13, header_14, header_15, header_16, header_17, header_18, header_19,
    header_20, header_21, header_22, header_23, header_24, header_25, header_26, header_27, header_28, header_29,
    header_30, header_31, header_32, header_33, header_34, header_35, header_36, header_37, header_38, header_39,
    header_40, header_41, header_42, header_43, header_44, header_45, header_46, header_47, header_48, header_49,
    header_50, header_51, header_52, header_53, header_54, header_55, header_56, header_57, header_58, header_59,
    header_60, header_61, header_62, header_63, header_64, header_65, header_66, header_67, header_68, header_69,
    header_70, header_71, header_72, header_73, header_74, header_75, header_76, header_77, header_78, header_79,
    header_80, header_81, header_82, header_83, header_84, header_85, header_86, header_87, header_88, header_89,
    header_90, /*
               header_91,\
               header_92,\
               header_93,\
...
#define SD_LOG_NUM_FIELDS                                                                                           \
    91 /**< The number of fields that are in the log. This is always smaller than the entry size due to some fields \
          being 2 bytes */

static_assert(sizeof(header_table) == (sizeof(char*) * SD_LOG_NUM_FIELDS),
              "Number of header table titles must match number of log fields");
...
bool createLogFile()
{
    ...
    */
    ...
    if (currentLogFileNumber > MAX_LOG_FILES)
        {
            currentLogFileNumber = 1;
        }  // If we've run out of file numbers, start again from 1
...
bool getSDLogFileDetails(uint8_t* buffer, uint16_t logNumber)
{
    ...
    char filenameBuffer[13];  // 8 + 1 + 3 + 1
    if (logNumber > MAX_LOG_FILES)
        {
            logNumber = MAX_LOG_FILES;
        }  // If we've run out of file numbers, start again from 1
            ...
            for (byte i = 0; i < 12; i++)
                {
                    ...
                    if (i < 8)
                        {
                            buffer[i] = filenameBuffer[i];
                        }  // Everything before the fullstop
                    else if (i > 8)
                        {
                            buffer[i - 1] = filenameBuffer[i];
                        }  // Everything after the fullstop
                }

            // Maintenance check, truncate the file. This will usually do nothing, but in the case where a prior log was
            // interrupted, this will truncate the file Due to overhead, only bother doing this if the engine isn't
            // running
...
void beginSDLogging()
{
    if (SD_status == SD_STATUS_READY)
        {
            SD_status = SD_STATUS_ACTIVE;  // Set the status as being active so that entries will begin to be written.
                                           // This will be updated below if there is an error
    ...
    if (SD_status == SD_STATUS_ACTIVE)
        {
            ...
            logFile.rewind();
            logFile.close();
            logFile.sync();  // This is required to update the sd object. Without this any subsequent logfiles will
                             // overwrite this one
    ...
    if (SD_status == SD_STATUS_ACTIVE)
        {
            ...
            // We write to SD when there is more than 1 sector worth of data in the ringbuffer and there is not already
            // a write being performed
            ...
            if ((logFile.dataLength() - logFile.curPosition()) < SD_SECTOR_SIZE)
                {
                    // Provided the conditions for logging are still met, a new file will be created the next time
...
{
    if (SD_status == SD_STATUS_ERROR_NO_CARD)
        {
            BIT_CLEAR(currentStatus.TS_SD_Status, SD_STATUS_CARD_PRESENT);
        }  // CARD is not present
    else
        {
            BIT_SET(currentStatus.TS_SD_Status, SD_STATUS_CARD_PRESENT);
        }  // CARD present
    ...
    if (SD_status == SD_STATUS_ACTIVE)
        {
            BIT_SET(currentStatus.TS_SD_Status, SD_STATUS_CARD_LOGGING);
        }  // CARD is logging
    else
        {
            BIT_CLEAR(currentStatus.TS_SD_Status, SD_STATUS_CARD_LOGGING);
        }  // CARD is not logging

    if ((SD_status >= SD_STATUS_ERROR_NO_FS))
        {
            BIT_SET(currentStatus.TS_SD_Status, SD_STATUS_CARD_ERROR);
        }  // CARD has an error
    else
        {
            BIT_CLEAR(currentStatus.TS_SD_Status, SD_STATUS_CARD_ERROR);
        }  // CARD has no error
            ...
            if ((configPage13.onboard_log_trigger_RPM) && (SD_status == SD_STATUS_READY))
                {
                    if ((currentStatus.RPMdiv100 >= configPage13.onboard_log_tr2_thr_on) &&
                        (currentStatus.RPMdiv100 <=
                         configPage13.onboard_log_tr2_thr_off))  // Need to check both on and off conditions to prevent
                                                                 // logging starting and stopping continually
                        {
                            beginSDLogging();  // Setup the log file, preallocation, header row
                        }
                }
    ...
    if (SD_status == SD_STATUS_ACTIVE)
        {
            ...
            if ((log_boot == false) && (log_RPM == false) && (log_prot == false) && (log_Vbat == false) &&
                (log_Epin == false) && (manualLogActive == false))
                {
                    endSDLogging();
                }
 ...
 *
 * Log files all have the same name with a 4 digit number at the end (Eg SPD_0001.csv). TS sends the 4 digits as ASCII
 * characters and they are combined here with the logfile prefix
 *
 * @param log1
 * @param log2
 * @param log3
