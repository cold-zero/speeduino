...
*/
integerPID idlePID(&currentStatus.longRPM, &idle_pid_target_value, &idle_cl_target_rpm, configPage6.idleKP,
                   configPage6.idleKI, configPage6.idleKD,
                   DIRECT);  // This is the PID object if that algorithm is used. Needs to be global as it maintains
                             // state outside of each function call
...
void initialiseIdle(bool forcehoming)
{
    ...
    // Pin masks must always be initialised, regardless of whether PWM idle is used. This is required for STM32 to
    // prevent issues if the IRQ function fires on restart/overflow
    ...
    switch (configPage6.iacAlgorithm)
        {
...
#if defined(CORE_AVR)
                idle_pwm_max_count =
                    (uint16_t)(MICROS_PER_SEC / (16U * configPage6.idleFreq *
                                                 2U));  // Converts the frequency in Hz to the number of ticks (at 16uS)
                                                        // it takes to complete 1 cycle. Note that the frequency is
                                                        // divided by 2 coming from TS to allow for up to 512hz
#elif defined(CORE_TEENSY35)
                idle_pwm_max_count =
                    (uint16_t)(MICROS_PER_SEC / (32U * configPage6.idleFreq *
                                                 2U));  // Converts the frequency in Hz to the number of ticks (at 32uS)
                                                        // it takes to complete 1 cycle. Note that the frequency is
                                                        // divided by 2 coming from TS to allow for up to 512hz
#elif defined(CORE_TEENSY41)
                idle_pwm_max_count =
                    (uint16_t)(MICROS_PER_SEC / (2U * configPage6.idleFreq *
                                                 2U));  // Converts the frequency in Hz to the number of ticks (at 2uS)
                                                        // it takes to complete 1 cycle. Note that the frequency is
                                                        // divided by 2 coming from TS to allow for up to 512hz
#endif
                enableIdle();
            ...
            case IAC_ALGORITHM_STEP_CL:
                ...
                idlePID.SetOutputLimits((configPage2.iacCLminValue * 3) << 2,
                                        (configPage2.iacCLmaxValue * 3)
                                            << 2);  // Maximum number of steps; always less than home steps count.
                idlePID.SetTunings(configPage6.idleKP, configPage6.idleKI, configPage6.idleKD);
                idlePID.SetMode(AUTOMATIC);     // Turn PID on
                configPage6.iacPWMrun = false;  // just in case. This needs to be false with stepper idle
...
/*
Checks whether a step is currently underway or whether the motor is in 'cooling' state (ie whether it's ready to begin
another step or not) Returns: True: If a step is underway or motor is 'cooling' False: If the motor is ready for another
step
*/
                    ...
                    if (idleStepper.stepperStatus == STEPPING)
                        {
                            ...
                            if (iacCoolTime_uS > 0)
                                {
                                    idleStepper.stepperStatus =
                                        COOLING;  //'Cooling' is the time the stepper needs to sit in LOW state before
    ...
    if (configPage2.idleUpEnabled == true)
        {
            if (configPage2.idleUpPolarity == 0)
                ...
                {
                    currentStatus.idleUpActive = digitalRead(pinIdleUp);
                }  // Inverted mode (5v activates idleUp)
    ...
    switch (configPage6.iacAlgorithm)
        {
            ...
            case IAC_ALGORITHM_ONOFF:  // Case 1 is on/off idle control
                if ((currentStatus.coolant + CALIBRATION_TEMPERATURE_OFFSET) <
                    configPage6.iacFastTemp)  // All temps are offset by 40 degrees
                    {
                        IDLE_PIN_HIGH();
                        idleOn = true;
                ...
                if (BIT_CHECK(currentStatus.engine, BIT_ENGINE_CRANK))
                    {
                        ...
                        currentStatus.idleLoad =
                            table2D_getValue(&iacCrankDutyTable,
                ...
                else if (!BIT_CHECK(currentStatus.engine, BIT_ENGINE_RUN))
                    {
                        if (configPage6.iacPWMrun == true)
                            {
                                // Engine is not running or cranking, but the run before crank flag is set. Use the
                                // cranking table
                                currentStatus.idleLoad = table2D_getValue(
                                    &iacCrankDutyTable,
                                    currentStatus.coolant +
                                        CALIBRATION_TEMPERATURE_OFFSET);  // All temps are offset by 40 degrees
                                idleTaper = 0;
                ...
                else
                    {
                        if (idleTaper < configPage2.idleTaperTime)
                            {
                                ...
                                currentStatus.idleLoad =
                                    map(idleTaper, 0, configPage2.idleTaperTime,
                                        table2D_getValue(&iacCrankDutyTable,
                                                         currentStatus.coolant + CALIBRATION_TEMPERATURE_OFFSET),
                                        table2D_getValue(&iacPWMTable,
                                                         currentStatus.coolant + CALIBRATION_TEMPERATURE_OFFSET));
                        ...
                        // Add air conditioning idle-up - we only do this if the engine is running (A/C should never
            ...
            case IAC_ALGORITHM_PWM_CL:  // Case 3 is PWM closed loop
                ...
                if (BIT_CHECK(currentStatus.engine, BIT_ENGINE_CRANK))
                    ...
                    {
                        idle_cl_target_rpm =
                            (uint16_t)currentStatus.CLIdleTarget * 10;  // Multiply the byte target value back out by 10
                        ...
                        if (PID_computed == true)
                            {
                                ...
                                // Add an offset to the duty cycle, outside of the closed loop. When tuned correctly,
                                // the extra load from the air conditioning should exactly cancel this out and the PID
                                // loop will be relatively unaffected.
                                if (configPage15.airConIdleSteps > 0 &&
                                    BIT_CHECK(currentStatus.airConStatus, BIT_AIRCON_TURNING_ON) == true)
                                    {
                                        // Add air conditioning idle-up
                                        // We are adding percentage steps, but the loop doesn't operate in percentage
                                        // steps - it works in PWM count
                                        TEMP_idle_pwm_target_value +=
                                            percentage(configPage15.airConIdleSteps, idle_pwm_max_count << 2);
                                ...
                                // Fixed this by putting it here, however I have not tested it. It used to be after the
                                // calculation of idle_pwm_target_value, meaning the percentage would update in
                                // currentStatus, but the idle would not actually increase.
                                if (currentStatus.idleUpActive == true)
                                    {
                                        // Add Idle Up amount if active
                                        // Again, we use configPage15.airConIdleSteps * idle_pwm_max_count / 100 because
                                        // we are adding percentage steps, but the loop doesn't operate in percentage
                                        // steps - it works in PWM count
                                        TEMP_idle_pwm_target_value +=
                                            percentage(configPage2.idleUpAdder, idle_pwm_max_count << 2);
                                        if (TEMP_idle_pwm_target_value > (idle_pwm_max_count << 2))
                                            {
                                                TEMP_idle_pwm_target_value = (idle_pwm_max_count << 2);
                                            }
            ...
            case IAC_ALGORITHM_PWM_OLCL:  // case 6 is PWM Open Loop table as feedforward term plus closed loop.
                ...
                if (BIT_CHECK(currentStatus.engine, BIT_ENGINE_CRANK))
                ...
                else
                    {
                        ...
                        FeedForwardTerm = percentage(
                            table2D_getValue(&iacPWMTable, currentStatus.coolant + CALIBRATION_TEMPERATURE_OFFSET),
                            idle_pwm_max_count << 2);  // All temps are offset by 40 degrees

                        // Add an offset to the feed forward term. When tuned correctly, the extra load from the air
                        // conditioning should exactly cancel this out and the PID loop will be relatively unaffected.
                        if (configPage15.airConIdleSteps > 0 &&
                            BIT_CHECK(currentStatus.airConStatus, BIT_AIRCON_TURNING_ON) == true)
                            {
                                // Add air conditioning idle-up
                                // We are adding percentage steps, but the loop doesn't operate in percentage steps - it
                                // works in PWM count <<2 (PWM count * 4)
                        ...
                        // Fixed this by putting it here, however I have not tested it. It used to be after the
                        // calculation of idle_pwm_target_value, meaning the percentage would update in currentStatus,
                        // but the idle would not actually increase.
                        if (currentStatus.idleUpActive == true)
                            {
                                // Add Idle Up amount if active
                                // Again, we are adding percentage steps, but the loop doesn't operate in percentage
                        ...
                        if ((currentStatus.RPM - idle_cl_target_rpm > configPage2.iacRPMlimitHysteresis * 10) ||
                            ...
                            {  // reset integral to zero when TPS is bigger than set value in TS (opening throttle so
                               // not idle anymore). OR when RPM higher than Idle Target + RPM Histeresis (coming back
                               // from high rpm with throttle closed)
                        ...
                        if (PID_computed == true)
                            {
                                ...
                                currentStatus.idleLoad =
                                    ((unsigned long)(idle_pwm_target_value * 100UL) / idle_pwm_max_count);
                            }
                        idleCounter++;
                    }
            ...
            case IAC_ALGORITHM_STEP_OL:  // Case 4 is open loop stepper control
                // First thing to check is whether there is currently a step going on and if so, whether it needs to be
                // turned off
                if ((checkForStepping() == false) &&
                    (isStepperHomed() == true))  // Check that homing is complete and that there's not currently a step
                                                 // already taking place. MUST BE IN THIS ORDER!
                    {
                        // Check for cranking pulsewidth
                        if (!BIT_CHECK(currentStatus.engine,
                                       BIT_ENGINE_RUN))  // If ain't running it means off or cranking
                            {
                                // Currently cranking. Use the cranking table
                                idleStepper.targetIdleStep =
                                    table2D_getValue(&iacCrankStepsTable,
                                                     (currentStatus.coolant + CALIBRATION_TEMPERATURE_OFFSET)) *
                                    3;  // All temps are offset by 40 degrees. Step counts are divided by 3 in TS.
                                ...
                                if (BIT_CHECK(LOOP_TIMER, BIT_TIMER_10HZ) && (currentStatus.RPM > 0))
                                    {
                                        if (idleTaper < configPage2.idleTaperTime)
                                            {
                                                // Tapering between cranking IAC value and running
                                                idleStepper.targetIdleStep = map(
                                                    idleTaper, 0, configPage2.idleTaperTime,
                                                    table2D_getValue(
                                                        &iacCrankStepsTable,
                                                        (currentStatus.coolant + CALIBRATION_TEMPERATURE_OFFSET)) *
                                                        3,
                                                    table2D_getValue(&iacStepTable, (currentStatus.coolant +
                                        ...
                                        else
                                            {
                                                ...
                                                idleStepper.targetIdleStep =
                                                    table2D_getValue(&iacStepTable, (currentStatus.coolant +
                                                                                     CALIBRATION_TEMPERATURE_OFFSET)) *
                                                    3;  // All temps are offset by 40 degrees. Step counts are divided
                                                        // by 3 in TS. Multiply back out here
                                            }
                                        if (currentStatus.idleUpActive == true)
                                            {
                                                idleStepper.targetIdleStep += configPage2.idleUpAdder;
                                        ...
                                        // Add air conditioning idle-up - we only do this if the engine is running (A/C
                                        // should never engage with engine off).
                                        if (configPage15.airConIdleSteps > 0 &&
                                            BIT_CHECK(currentStatus.airConStatus, BIT_AIRCON_TURNING_ON) == true)
                                            {
                                                 ...
                                                 // already taking place. MUST BE IN THIS ORDER!
                    {
                        if (!BIT_CHECK(currentStatus.engine,
                        ...
                        else
                            {
                                if (BIT_CHECK(LOOP_TIMER, BIT_TIMER_10HZ))
                                    {
                                        idle_cl_target_rpm = (uint16_t)currentStatus.CLIdleTarget *
                                                             10;  // Multiply the byte target value back out by 10
                                        if (idleTaper < configPage2.idleTaperTime)
                                            {
                                                uint16_t minValue = table2D_getValue(&iacCrankStepsTable,
                                                                                     (currentStatus.coolant +
                                                                                      CALIBRATION_TEMPERATURE_OFFSET)) *
                                                                    3;
                                                ...
                                                if (configPage6.iacAlgorithm == IAC_ALGORITHM_STEP_OLCL)
                                                    {
                                                        maxValue = table2D_getValue(&iacStepTable,
                                                                                    (currentStatus.coolant +
                                                                                     CALIBRATION_TEMPERATURE_OFFSET)) *
                                                                   3;
                                                    }
                                                ...
                                                // Tapering between cranking IAC value and running
                                                FeedForwardTerm =
                                                    map(idleTaper, 0, configPage2.idleTaperTime, minValue, maxValue)
                                                    << 2;
                                                idleTaper++;
                                        ...
                                        else if (configPage6.iacAlgorithm == IAC_ALGORITHM_STEP_OLCL)
                                            {
                                                ...
                                                FeedForwardTerm =
                                                    (table2D_getValue(&iacStepTable, (currentStatus.coolant +
                                                                                      CALIBRATION_TEMPERATURE_OFFSET)) *
                                                     3)
                                                    << 2;  // All temps are offset by 40 degrees. Step counts are
                                                           // divided by 3 in TS. Multiply back out here
                                                // reset integral to zero when TPS is bigger than set value in TS
                                                // (opening throttle so not idle anymore). OR when RPM higher than Idle
                                                // Target + RPM Hysteresis (coming back from high rpm with throttle
                                                // closed)
                                                if (((currentStatus.RPM - idle_cl_target_rpm) >
                                                     configPage2.iacRPMlimitHysteresis * 10) ||
                                ...
                                // Add air conditioning idle-up - we only do this if the engine is running (A/C should
                                // never engage with engine off).
                                if (configPage15.airConIdleSteps > 0 &&
                                    BIT_CHECK(currentStatus.airConStatus, BIT_AIRCON_TURNING_ON) == true)
                                    {
                                        idleStepper.targetIdleStep += configPage15.airConIdleSteps;
            ...
            if ((checkForStepping() == false) && (isStepperHomed() == true))
                {
                    ...
                    */
                    idleStepper.targetIdleStep =
                        table2D_getValue(&iacCrankStepsTable,
                                         (currentStatus.coolant + CALIBRATION_TEMPERATURE_OFFSET)) *
                        3;  // All temps are offset by 40 degrees. Step counts are divided by 3 in TS. Multiply back out
                            // here
                    if (currentStatus.idleUpActive == true)
                        {
                            idleStepper.targetIdleStep += configPage2.idleUpAdder;
