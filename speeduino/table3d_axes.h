...
class table_axis_iterator
{
   public:
    ...
    table_axis_iterator(table3d_axis_t* pStart, const table3d_axis_t* pEnd, axis_domain domain)
        : _stride(pEnd > pStart ? stride_inc : stride_dec), _pStart(pStart), _pEnd(pEnd + _stride),
    ...
    table_axis_iterator& operator++(void) { return advance(1); }

    /** @brief Test for end of iteration */
    bool at_end(void) const { return _pStart == _pEnd; }

    /** @brief Dereference the iterator */
    table3d_axis_t& operator*(void) { return *_pStart; }
    /** @copydoc table_axis_iterator::operator*()  */
    const table3d_axis_t& operator*(void) const { return *_pStart; }
...
#define TABLE3D_GEN_AXIS(size, dom)                                                                     \
    ...
    {                                                                                                   \
        ...
        */                                                                                              \
        ...
        table_axis_iterator begin(void) { return table_axis_iterator(axis + (size)-1, axis, domain); }  \
        /** @brief Iterate over the axis elements, from largest to smallest */                          \
        table_axis_iterator rbegin(void) { return table_axis_iterator(axis, axis + (size)-1, domain); } \
    };
