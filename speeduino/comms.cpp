...
constexpr byte codeVersion[] PROGMEM = {
    SERIAL_RC_OK, 's', 'p', 'e', 'e', 'd', 'u', 'i', 'n', 'o', ' ',
    '2',          '0', '2', '4', '0', '5', '-', 'd', 'e', 'v'};  // Note no null terminator in array and statu variable
                                                                 // at the start
constexpr byte productString[] PROGMEM = {SERIAL_RC_OK, 'S', 'p', 'e', 'e', 'd', 'u', 'i', 'n', 'o', ' ',
                                          '2',          '0', '2', '4', '.', '0', '5', '-', 'd', 'e', 'v'};
// constexpr byte codeVersion[] PROGMEM = { SERIAL_RC_OK, 's','p','e','e','d','u','i','n','o','
// ','2','0','2','4','0','2'} ; //Note no null terminator in array and statu variable at the start constexpr byte
// productString[] PROGMEM = { SERIAL_RC_OK, 'S', 'p', 'e', 'e', 'd', 'u', 'i', 'n', 'o', ' ', '2', '0', '2', '4', '.',
// '0', '2'};
...
static uint32_t serialReceiveStartTime =
    0;  //!< The time at which the serial receive started. Used for calculating whether a timeout has occurred */
 ...
 */
static void processTemperatureCalibrationTableUpdate(uint16_t calibrationLength, uint8_t calibrationPage,
                                                     uint16_t *values, uint16_t *bins)
{
                            ...
                            if (incomingCrc == CRC32_serial.crc32(serialPayload, serialPayloadLength))
                                {
                                    ...
                                    BIT_CLEAR(currentStatus.status4,
                                              BIT_STATUS4_ALLOW_LEGACY_COMMS);  // Lock out legacy commands until next
                                                                                // power cycle
                                }
                            else
                                {
    ...
    switch (serialStatusFlag)
        {
            ...
            case SERIAL_TRANSMIT_INPROGRESS:
                ...
                serialStatusFlag = serialBytesRxTx == serialPayloadLength + sizeof(crc_t) ? SERIAL_INACTIVE
    ...
    switch (serialPayload[0])
        {
            ...
            case 'B':  // Same as above, but for the comms compat mode. Slows down the burn rate and increases the defer
                       // time
            ...
            case 'k':  // Send CRC values for the calibration pages
                {
                    uint32_t CRC32_val =
                        reverse_bytes(readCalibrationCRC32(serialPayload[2]));  // Get the CRC for the requested page
            ...
            case 'M':
                {
                    ...
                    // 2 - Length
                    // 1 - 1st New value
                    if (updatePageValues(serialPayload[2], word(serialPayload[4], serialPayload[3]), &serialPayload[7],
            ...
            case 'p':
                {
                    ...
                    serialPayload[0] = SERIAL_RC_OK;
                    loadPageValuesToBuffer(serialPayload[2], word(serialPayload[4], serialPayload[3]),
                                           &serialPayload[1], length);
                            ...
                            if ((SD_arg1 == SD_READ_STAT_ARG1) && (SD_arg2 == SD_READ_STAT_ARG2))
                                {
                                    ...
                                    */
                                    ...
                                    uint16_t numLogFiles = getNextSDLogFileNumber() -
                                                           2;  // -1 because this returns the NEXT file name not the
                                                               // current one and -1 because TS expects a 0 based index
                    ...
                    if (cmd == O2_CALIBRATION_PAGE)
                        {
                            ...
                            sendReturnCodeMsg(SERIAL_RC_OK);
                            Serial.flush();  // This is safe because engine is assumed to not be running during
                                             // calibration
                        }
                    else if (cmd == IAT_CALIBRATION_PAGE)
                        {
                            processTemperatureCalibrationTableUpdate(calibrationLength, IAT_CALIBRATION_PAGE,
                                                                     iatCalibration_values, iatCalibration_bins);
                        }
                    else if (cmd == CLT_CALIBRATION_PAGE)
                        {
                            processTemperatureCalibrationTableUpdate(calibrationLength, CLT_CALIBRATION_PAGE,
                    ...
                    if (cmd == SD_READWRITE_PAGE)
                        {
                            if ((SD_arg1 == SD_WRITE_DO_ARG1) && (SD_arg2 == SD_WRITE_DO_ARG2))
                                ...
                                {
                                    // Read sector Init? Unsure what this is meant to do however it is sent at the
                                    // beginning of a Card Format request and requires an OK response Provided the
                                    // sector being requested is x0 x0 x0 x0, we treat this as a SD Card format request
                                    if ((serialPayload[7] == 0) && (serialPayload[8] == 0) && (serialPayload[9] == 0) &&
                                        (serialPayload[10] == 0))
                                        {
                            ...
                            else if ((SD_arg1 == SD_WRITE_COMP_ARG1) && (SD_arg2 == SD_WRITE_COMP_ARG2))
                                {
                                    ...
                                    SDreadNumSectors = (sectorCount1 << 24) | (sectorCount2 << 16) |
                                                       (sectorCount3 << 8) | sectorCount4;
    ...
    if (logItemsTransmitted == 0)
        {
            ...
            (void)serialWrite(
                (uint16_t)(sizeof(toothHistory) + 1U));  // Size of the tooth log (uint32_t values) plus the return code
            // Begin new CRC hash
...
void sendCompositeLog(void)
{
    if (BIT_CHECK(currentStatus.status1, BIT_STATUS1_TOOTHLOG1READY) == false)
        {
            ...
            while (toothHistoryIndex < TOOTH_LOG_SIZE)
                {
                    toothHistory[toothHistoryIndex] =
                        toothHistory[toothHistoryIndex - 1];  // Composite logger needs a realistic time value to
                                                              // display correctly. Copy the last value
    ...
    if (logItemsTransmitted == 0)
        {
            // Transmit the size of the packet
            (void)serialWrite((uint16_t)(sizeof(toothHistory) + sizeof(compositeLogHistory) +
                                         1U));  // Size of the tooth log (uint32_t values) plus the return code
    ...
    for (; logItemsTransmitted < TOOTH_LOG_SIZE; logItemsTransmitted++)
        {
            ...
            uint32_t transmitted =
                serialWrite(toothHistory[logItemsTransmitted]);  // This combined runtime (in us) that the log was going
                                                                 // for by this record
