...
*/
#ifdef RTC_ENABLED
    // Check if RTC time has been set earlier. If yes, RTC will use LSE_CLOCK. If not, default LSI_CLOCK is used, to
    // prevent hanging on boot.
    if (rtc.isTimeSet())
        {
            rtc.setClockSource(STM32RTC::LSE_CLOCK);  // Initialise external clock for RTC if clock is set. That is the
                                                      // only clock running of VBAT
        }
    ...
    */
    if ((configPage6.iacAlgorithm == IAC_ALGORITHM_PWM_OL) || (configPage6.iacAlgorithm == IAC_ALGORITHM_PWM_CL) ||
        ...
        {
            idle_pwm_max_count =
                (uint16_t)(MICROS_PER_SEC / (TIMER_RESOLUTION * configPage6.idleFreq *
                                             2U));  // Converts the frequency in Hz to the number of ticks (at 4uS) it
                                                    // takes to complete 1 cycle. Note that the frequency is divided by
                                                    // 2 coming from TS to allow for up to 5KHz
        }
...
*/
    ...
    */
    ...
    boost_pwm_max_count =
        (uint16_t)(MICROS_PER_SEC / (TIMER_RESOLUTION * configPage6.boostFreq *
                                     2U));  // Converts the frequency in Hz to the number of ticks (at 4uS) it takes to
    ...
    vvt_pwm_max_count =
        (uint16_t)(MICROS_PER_SEC /
                   (TIMER_RESOLUTION * configPage6.vvtFreq *
                    2U));  // Converts the frequency in Hz to the number of ticks (at 4uS) it takes to complete 1 cycle
    fan_pwm_max_count =
        (uint16_t)(MICROS_PER_SEC /
                   (TIMER_RESOLUTION * configPage6.fanFreq *
                    2U));  // Converts the frequency in Hz to the number of ticks (at 4uS) it takes to complete 1 cycle
...
void jumpToBootloader(
    void)  // https://github.com/3devo/Arduino_Core_STM32/blob/jumpSysBL/libraries/SrcWrapper/src/stm32/bootloader.c
