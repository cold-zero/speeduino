...
#define DIV_ROUND_CLOSEST(n, d, t)                                                   \
    ((((n) < (t)(0)) ^ ((d) < (t)(0))) ? ((t)((n)-DIV_ROUND_CORRECT(d, t)) / (t)(d)) \
...
static inline int16_t div100(int16_t n)
{
    ...
    return libdivide::libdivide_s16_do_raw(n - DIV_ROUND_CORRECT(UINT16_C(100), uint16_t), S16_MAGIC(100),
                                           S16_MORE(100));
#else
...
static inline int div100(int n) { return DIV_ROUND_CLOSEST(n, 100U, int); }
#else
static inline int32_t div100(int32_t n)
{
    ...
    return libdivide::libdivide_s32_do_raw(n + (DIV_ROUND_CORRECT(UINT16_C(100), uint32_t) * (n < 0 ? -1 : 1)),
                                           1374389535L, 5);
#else
...
// This is a common case because it means converting from a standard 10-bit analog input to a byte or 10-bit analog into
// 0-511 (Eg the temperature readings)
