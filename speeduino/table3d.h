...
#define TABLE3D_GEN_GET_TABLE_VALUE(size, xDom, yDom)                                                                 \
    static inline table3d_value_t get3DTableValue(TABLE3D_TYPENAME_BASE(size, xDom, yDom) * pTable, table3d_axis_t y, \
                                                  table3d_axis_t x)                                                   \
    {                                                                                                                 \
        return get3DTableValue(&pTable->get_value_cache, TABLE3D_TYPENAME_BASE(size, xDom, yDom)::value_t::row_size,  \
                               pTable->values.values, pTable->axisX.axis, pTable->axisY.axis, y, x);                  \
    }
...
#define CONCRETE_TABLE_ACTION_INNER(size, xDomain, yDomain, action, ...) \
    case TO_TYPE_KEY(size, xDomain, yDomain):                            \
        action(size, xDomain, yDomain, ##__VA_ARGS__);
