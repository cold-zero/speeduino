...
void initBoard(void)
{
    ...
    */
    ...
    TCCR1A =
        TIMER_MODE_NORMAL;  // Timer1 Control Reg A: Wave Gen Mode normal (Simply counts up from 0 to 65535 (16-bit int)
    ...
    TIFR1 = (1 << OCF1A) | (1 << OCF1B) | (1 << OCF1C) | (1 << TOV1) |
            (1 << ICF1);  // Clear the compare flags, overflow flag and external input flag bits

    boost_pwm_max_count =
        (uint16_t)(MICROS_PER_SEC / (16U * configPage6.boostFreq *
                                     2U));  // Converts the frequency in Hz to the number of ticks (at 16uS) it takes to
                                            // complete 1 cycle. The x2 is there because the frequency is stored at half
                                            // value (in a byte) to allow frequencies up to 511Hz
    vvt_pwm_max_count =
        (uint16_t)(MICROS_PER_SEC /
                   (16U * configPage6.vvtFreq *
                    2U));  // Converts the frequency in Hz to the number of ticks (at 16uS) it takes to complete 1 cycle
    ...
    */
    ...
    TCNT2 = 131;  // Preload timer2 with 131 cycles, leaving 125 till overflow. As the timer runs at 125Khz, this causes
                  // overflow to occur at 1Khz = 1ms
    ...
    TCCR2B =
        (1 << CS22) |
        (1 << CS20);  // Set bits. This timer uses different prescaler values, thus we cannot use the defines above.
    TIFR2 = (1 << OCF2A) | (1 << OCF2B) | (1 << TOV2);  // Clear the compare flag bits and overflow flag bit

    // Enable the watchdog timer for 2 second resets (Good reference:
    // www.tushev.org/articles/arduino/5/arduino-and-watchdog-timer) Boooooooooo WDT is currently broken on Mega 2560
    // bootloaders :( wdt_enable(WDTO_2S);
    ...
    * */
    ...
    TIFR3 = (1 << OCF3A) | (1 << OCF3B) | (1 << OCF3C) | (1 << TOV3) |
            (1 << ICF3);  // Clear the compare flags, overflow flag and external input flag bits

    // Ignition Schedules, which uses timer 5. This is also used by the fast version of micros(). If the speed of this
    // timer is changed from 4uS ticks, that MUST be changed as well. See globals.h and timers.ino
    ...
    TIFR5 = (1 << OCF5A) | (1 << OCF5B) | (1 << OCF5C) | (1 << TOV5) |
            (1 << ICF5);  // Clear the compare flags, overflow flag and external input flag bits
    ...
    TIFR4 = (1 << OCF4A) | (1 << OCF4B) | (1 << OCF4C) | (1 << TOV4) |
            (1 << ICF4);  // Clear the compare flags, overflow flag and external input flag bits
}

/*
  Returns how much free dynamic memory exists (between heap and stack)
  This function is one big MISRA violation. MISRA advisories forbid directly poking at memory addresses, however there
  is no other way of determining heap size on embedded systems.
*/
