 ...
 *
 * Structures @ref FuelSchedule and @ref Schedule describe (from scheduler.h) describe the scheduling info for Fuel and
 * Ignition respectively. They contain duration, current activity status, start timing, end timing, callbacks to carry
 * out action, etc.
 *
...
void initialiseSchedulers()
{
    ...
    ignition1EndAngle = 0;
    channel1IgnDegrees = 0; /**< The number of crank degrees until cylinder 1 is at TDC (This is obviously 0 for
                               virtually ALL engines, but there's some weird ones) */
    ...
    channel1InjDegrees = 0; /**< The number of crank degrees until cylinder 1 is at TDC (This is obviously 0 for
                               virtually ALL engines, but there's some weird ones) */
...
void _setFuelScheduleRunning(FuelSchedule &schedule, unsigned long timeout, unsigned long duration)
{
    ...
    if (timeout > MAX_TIMER_PERIOD)
        ...
        }  // If the timeout is >4x (Each tick represents 4uS on a mega2560, other boards will be different) the maximum
           // allowed value of unsigned int (65535), the timer compare value will overflow when applied causing erratic
    ...
    // The following must be enclosed in the noInterupts block to avoid contention caused if the relevant interrupt
    // fires before the state is fully set
    noInterrupts();
...
void _setFuelScheduleNext(FuelSchedule &schedule, unsigned long timeout, unsigned long duration)
{
    ...
    // This is required in cases of high rpm and high DC where there otherwise would not be enough time to set the
    // schedule
...
void _setIgnitionScheduleRunning(IgnitionSchedule &schedule, unsigned long timeout, unsigned long duration)
{
    ...
    if (timeout > MAX_TIMER_PERIOD)
        {
            timeout_timer_compare = uS_TO_TIMER_COMPARE((MAX_TIMER_PERIOD - 1));
        }  // If the timeout is >4x (Each tick represents 4uS) the maximum allowed value of unsigned int (65535), the
           // timer compare value will overflow when applied causing erratic behaviour such as erroneous sparking.
    else
        {
            timeout_timer_compare = uS_TO_TIMER_COMPARE(timeout);
        }  // Normal case
    ...
    noInterrupts();
    schedule.startCompare =
        schedule.counter + timeout_timer_compare;  // As there is a tick every 4uS, there are timeout/4 ticks until the
                                                   // interrupt should be triggered ( >>2 divides by 4)
    if (schedule.endScheduleSetByDecoder == false)
        {
            schedule.endCompare = schedule.startCompare + uS_TO_TIMER_COMPARE(duration);
        }  // The .endCompare value is also set by the per tooth timing in decoders.ino. The check here is so that it's
           // not getting overridden.
...
void refreshIgnitionSchedule1(unsigned long timeToEnd)
{
    if ((ignitionSchedule1.Status == RUNNING) && (timeToEnd < ignitionSchedule1.duration))
        // Must have the threshold check here otherwise it can cause a condition where the compare fires twice, once
        // after the other, both for the end if( (timeToEnd < ignitionSchedule1.duration) && (timeToEnd >
...
extern void beginInjectorPriming(void)
{
    ...
    if ((primingValue > 0) && (currentStatus.TPS < configPage4.floodClear))
        {
            primingValue =
                primingValue * 100 * 5;  // to achieve long enough priming pulses, the values in tuner studio are
                                         // divided by 0.5 instead of 0.1, so multiplier of 5 is required.
            if (maxInjOutputs >= 1)
                {
                    setFuelSchedule(fuelSchedule1, 100, primingValue);
                }
#if (INJ_CHANNELS >= 2)
            if (maxInjOutputs >= 2)
                {
                    setFuelSchedule(fuelSchedule2, 100, primingValue);
                }
#endif
#if (INJ_CHANNELS >= 3)
            if (maxInjOutputs >= 3)
                {
                    setFuelSchedule(fuelSchedule3, 100, primingValue);
                }
#endif
#if (INJ_CHANNELS >= 4)
            if (maxInjOutputs >= 4)
                {
                    setFuelSchedule(fuelSchedule4, 100, primingValue);
                }
#endif
#if (INJ_CHANNELS >= 5)
            if (maxInjOutputs >= 5)
                {
                    setFuelSchedule(fuelSchedule5, 100, primingValue);
                }
#endif
#if (INJ_CHANNELS >= 6)
            if (maxInjOutputs >= 6)
                {
                    setFuelSchedule(fuelSchedule6, 100, primingValue);
                }
#endif
#if (INJ_CHANNELS >= 7)
            if (maxInjOutputs >= 7)
                {
                    setFuelSchedule(fuelSchedule7, 100, primingValue);
                }
#endif
#if (INJ_CHANNELS >= 8)
            if (maxInjOutputs >= 8)
                {
                    setFuelSchedule(fuelSchedule8, 100, primingValue);
                }
#endif
        }
}
    ...
    if (schedule.Status == PENDING)  // Check to see if this schedule is turn on
        {
            ...
            schedule.Status = RUNNING;  // Set the status to be in progress (ie The start callback has been called, but
                                        // not the end callback)
            SET_COMPARE(schedule.compare,
                        schedule.counter +
                            uS_TO_TIMER_COMPARE(
                                schedule.duration));  // Doing this here prevents a potential overflow on restarts
        }
        ...
        {
            schedule.pTimerDisable();  // Safety check. Turn off this output compare unit and return without performing
                                       // any action
        }
}

/*******************************************************************************************************************************************************************************************************/
/** fuelSchedule*Interrupt (All 8 ISR functions below) get called (as timed interrupts) when either the start time or
 * the duration time are reached. This calls the relevant callback function (startCallback or endCallback) depending on
 * the status (PENDING => Needs to run, RUNNING => Needs to stop) of the schedule. The status of schedule is managed
 * here based on startCallback /endCallback function called:
    ...
    if (schedule.Status == PENDING)  // Check to see if this schedule is turn on
        {
            ...
            if (schedule.endScheduleSetByDecoder == true)
                {
