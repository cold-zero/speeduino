...
#define SD_CONFIG \
    SdioConfig(FIFO_SDIO)  // Set Teensy to use SDIO in FIFO mode. This is the fastest SD mode on Teensy as it offloads
                           // most of the writes
...
#define pinIsReserved(pin)                                                                                \
    (((pin) == 0) || ((pin) == 42) || ((pin) == 43) || ((pin) == 44) || ((pin) == 45) || ((pin) == 46) || \
     ((pin) == 47))  // Forbidden pins like USB
...
The Quad timer (TMR) provides 4 timers each with 4 usable compare channels. The down compare and alternating compares
are not usable FUEL 1-4: TMR1 IGN 1-4 : TMR2 FUEL 5-8: TMR3 IGN 5-8 : TMR4
*/
...
}  // Write 1 to the TCFIEN (Channel Interrupt Enable) bit of channel 0 Status/Control
...
{
    TMR1_CSCTRL0 &= ~TMR_CSCTRL_TCF1EN;
}  // Write 0 to the TCFIEN (Channel Interrupt Enable) bit of channel 0 Status/Control
...
#define uS_TO_TIMER_COMPARE(uS) \
    ((uS * 75UL) >>             \
     6)  // Converts a given number of uS into the required number of timer ticks until that time has passed.
/*
