...
struct table2D
{
    ...
    int16_t lastInput;
    int16_t lastOutput;
    byte cacheTime;  // Tracks when the last cache value was set so it can expire after x seconds. A timeout is required
                     // to pickup when a tuning value is changed, otherwise the old cached value will continue to be
                     // returned as the X value isn't changing.
};
