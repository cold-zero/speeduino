...
#define SERIAL_BUFFER_SIZE \
    517  // Size of the serial buffer used by new comms protocol. For SD transfers this must be at least 512 + 1 (flag)
         // + 4 (sector)
#define FPU_MAX_SIZE 32  // Size of the FPU buffer. 0 means no FPU.
#define micros_safe() \
    micros()  // timer5 method is not used on anything but AVR, the micros_safe() macro is simply an alias for the
              // normal micros()
#define TIMER_RESOLUTION 4

// Select one for EEPROM,the default is EEPROM emulation on internal flash.
//#define SRAM_AS_EEPROM /*Use 4K battery backed SRAM, requires a 3V continuous source (like battery) connected to Vbat
//pin */ #define USE_SPI_EEPROM PB0 /*Use M25Qxx SPI flash on BlackF407VE*/ #define FRAM_AS_EEPROM /*Use FRAM like
//FM25xxx, MB85RSxxx or any SPI compatible */

#ifndef word
#define word(h, l) ((h << 8) | l)  // word() function not defined for this platform in the main library
#endif

#if defined(ARDUINO_BLUEPILL_F103C8) || defined(ARDUINO_BLUEPILL_F103CB) || defined(ARDUINO_BLACKPILL_F401CC) || \
...
// Specific mode for Bluepill due to its small flash size. This disables a number of strings from being compiled into
// the flash
...
inline uint32_t digitalPinToInterrupt(uint32_t Interrupt_pin)
{
...
#if defined(USER_BTN)
#define EEPROM_RESET_PIN \
    USER_BTN  // onboard key0 for black STM32F407 boards and blackpills, keep pressed during boot to reset eeprom
#endif

#if defined(STM32F407xx)
// Comment out this to disable SD logging for STM32 if needed. Currently SD logging for STM32 is experimental feature
// for F407.
#define SD_LOGGING
#endif
...
#define RTC_ENABLED
// SD logging with STM32 uses SD card in SPI mode, because used SD library doesn't support SDIO implementation. By
// default SPI3 is used that uses same pins as SDIO also, but in different order.
...
// Alternatively same SPI bus can be used as there is for SPI flash. But this is not recommended due to slower speed and
// other possible problems. #define SD_CONFIG SdSpiConfig(SD_CS_PIN, SHARED_SPI, SD_SCK_MHZ(50), &SPI_for_flash)
#endif
#define USE_SERIAL3

// When building for Black board Serial1 is instantiated,building generic STM32F4x7 has serial2 and serial 1 must be
// done here
...
#define pinIsReserved(pin)                                                                       \
    (((pin) == PA11) || ((pin) == PA12) || ((pin) == PB3) || ((pin) == PB4) || ((pin) == PB5) || \
     ((pin) == USE_SPI_EEPROM))  // Forbidden pins like USB
#else
#define pinIsReserved(pin)                                                                       \
    (((pin) == PA11) || ((pin) == PA12) || ((pin) == PB3) || ((pin) == PB4) || ((pin) == PB5) || \
     ((pin) == PB0))  // Forbidden pins like USB
#endif
#endif
...
#define MAX_TIMER_PERIOD \
    65535 * 4  // The longest period of time (in uS) that the timer can permit (IN this case it is 65535 * 4, as each
               // timer tick is 4uS)
#define uS_TO_TIMER_COMPARE(uS) \
    (uS >> 2)  // Converts a given number of uS into the required number of timer ticks until that time has passed.
