...
class table_row_iterator
{
   public:
    ...
    table_row_iterator& operator++(void) { return advance(1); }

    /** @brief Test for end of iteration */
    bool at_end(void) const { return pValue == pEnd; }

    /** @brief Dereference the iterator */
    const table3d_value_t& operator*(void) const { return *pValue; }
    /** @copydoc table_row_iterator::operator*() const */
    table3d_value_t& operator*(void) { return *const_cast<table3d_value_t*>(pValue); }
   ...
   public:
    ...
    table_value_iterator(const table3d_value_t* pValues, table3d_dim_t axisSize)
        : pRowsStart(pValues + (axisSize * (axisSize - 1U))),  // cppcheck-suppress misra-c2012-10.4
          pRowsEnd(pValues - axisSize), rowWidth(axisSize)
    {
    ...
    table_value_iterator& operator++(void) { return advance(1U); }

    /** @brief Dereference the iterator to access a row of data */
    const table_row_iterator operator*(void) const { return table_row_iterator(pRowsStart, rowWidth); }
    /** @copydoc table_value_iterator::operator*() const */
    table_row_iterator operator*(void) { return table_row_iterator(pRowsStart, rowWidth); }

    /** @brief Test for end of iteration */
    bool at_end(void) const { return pRowsStart == pRowsEnd; }
...
#define TABLE3D_GEN_VALUES(size, xDom, yDom)                                                                   \
    ...
    {                                                                                                          \
        ...
        */                                                                                                     \
        ...
        table_value_iterator begin(void) { return table_value_iterator(values, row_size); }                    \
                                                                                                               \
        /**                                                                                                    \
        ...
        {                                                                                                      \
            ...
            const table3d_dim_t index =                                                                        \
                (table3d_dim_t)(first_index + (table3d_dim_t)(2U * (linear_index % row_size)) - linear_index); \
            return values[index];                                                                              \
        }                                                                                                      \
    };
