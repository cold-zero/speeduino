            ...
            if (configPage9.hardRevMode == HARD_REV_FIXED)
                {
                    ...
                    if ((currentStatus.RPMdiv100 >= configPage4.HardRevLim) ||
                        ((softLimitTime > configPage4.SoftLimMax) &&
                         (currentStatus.RPMdiv100 >= configPage4.SoftRevLim)))
                        {
                            BIT_SET(currentStatus.spark,
                                    BIT_SPARK_HRDLIM);  // Legacy and likely to be removed at some point
                ...
                {
                    currentLimitRPM = (int16_t)(table2D_getValue(
                        &coolantProtectTable, currentStatus.coolant + CALIBRATION_TEMPERATURE_OFFSET));
    ...
    if (configPage6.engineProtectType != PROTECT_CUT_OFF)
        {
            ...
            if ((configPage6.boostCutEnabled > 0) &&
                (currentStatus.MAP >
                 (configPage6.boostLimit * 2)))  // The boost limit is divided by 2 to allow a limit up to 511kPa
                {
                    boostLimitActive = 1;
                    ...
                    if (currentStatus.oilPressure < oilLimit)
                        {
                            ...
                            if ((uint8_t(div100(millis())) >=
                                 (uint16_t(oilProtStartTime + configPage10.oilPressureProtTime))) ||
                                (alreadyActive > 0))
                                {
...
byte checkAFRLimit(void)
{
    ...
    */
    if (configPage6.engineProtectType != PROTECT_CUT_OFF && configPage9.afrProtectEnabled &&
        configPage6.egoType == EGO_TYPE_WIDE)
        {
            ...
            */
            ...
            switch (configPage9.afrProtectEnabled)
                {
                    case 1:
                        afrCondition = (currentStatus.O2 >= configPage9.afrProtectDeviation) ? true : false;
                        break; /* Fixed value */
                    case 2:
